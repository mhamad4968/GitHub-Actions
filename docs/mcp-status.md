...
| 項目 | 状態 |
|------|------|
| **mintlify** | **DEL-1 完了**（2026-07-11 削除 · 浜田追認 GO 2026-07-15）· 再追加禁止 |
| **cyber-news** | **DEL-2 完了**（mcp.json 不在済 · 浜田正式クローズ GO 2026-07-15 · 代替=cve-search+DDG）· 再追加禁止 |
| **台帳本数** | **26** · `verify:mcp-deleted-refs` / `cio:mcp:gate` 緑 |
| **下記の過去スナップ** | 当日の事実記録 · mintlify 言及は **当時の正しい監査結果** |
...
| 見送り | **brave-search / exa / firecrawl** — 浜田判断 **不要**（DDG + context7 + WebFetch で足りる） |
...
| `verify:desktop-ai-emergency-sync` | OK（sync 前） |
| `cio:mcp:env:extended` | **markdownify** = SKIP（Windows・TSB-029 意図 — doc レーンは python フォールバック可） |

**GHA 注意**: `security-next-kintone` run **27012420004** — Gemini **403** dunning（MCP 外・API 課金要確認）
...
- **`npm run cio:mcp:env`** … 上記のあと **`verify:deepseek-mcp-v4`**（DeepSeek が `mcp-deepseek-v4` ラッパ／`deepseek-v4-flash` 既定であること。upstream `mcp-deepseek@latest`＝`deepseek-chat` 既定は **API 400**）→ **`.env`＋`.env.proxy` 注入**で `cio-mcp-quickprobe`（kintone + 3AI + memory + sequential-thinking）を **JSON-RPC initialize** まで実測。**CIO 合格線**: **3AI・kintone のキーは `.env` が空でも `%USERPROFILE%\.cursor\mcp.json`（＋リポ `.cursor/mcp.json`）の `server.env` を自動補完**し、**SKIP なしで全件 OK**（不足なら **exit 2**＝未整備）。
- **DeepSeek モデル（2026-07-25）**: 公式 API は **`deepseek-v4-flash` / `deepseek-v4-pro` のみ**。正本起動は **`scripts/mcp-deepseek-v4/entry.mjs`**（Windows sync・WSL `~/.cursor/mcp.json` とも）。検査: **`npm run verify:deepseek-mcp-v4`**。
- **DeepSeek 空応答 #S-DS-EMPTY-01（2026-07-25 夕）**: v4 **thinking 既定 ON** + `max_tokens≈400` → reasoning 全消費で `content=""`（旧ラッパは「无响应」）。lab 既定は **`DEEPSEEK_THINKING_DEFAULT=disabled`**。実 chat 検査: **`npm run verify:deepseek-mcp-chat -- --require-live`**（`cio:mcp:env` 内包）。MCP 再接続後に CallMcpTool で確認。
...
| 6 | memory | ✅ **active 化済** | **26 回** | セッション横断記憶 (現在 10 entities + 11 relations) | PC 台帳 PJ で実戦投入後判断（5/13 頃）|
| 7 | ~~fetch~~ | 🗑 **削除済 2026-05-06** | — | （除去）Cursor **WebFetch**／`user-fetch` で代替 | — |
| 8 | sequential-thinking | ✅ active | **15 回** | 段階的思考 | PC 台帳 PJ で実戦投入後判断 |
| 9 | **kintone (公式)** | ✅ active | **47 回** | kintone API CRUD | 5/13 後（本番運用後）|
| 10 | **kintone-dev (自作)** | ✅ active | **12 回** | API 仕様参照 (アプリ作成 MCP ではない / 4/23 早朝訂正済) | **4/26 PC 台帳 Day 4 後判断** |
...
| 13 | playwright | ✅ active (4/23 21:30 Chrome 147.0.7727.116 install + 実 call 動作確認済) | **4 回** | ブラウザ自動操作 / E2E | 4/26 PC 台帳 customize テスト時 |
