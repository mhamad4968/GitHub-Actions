...
| 6 | memory | ✅ **active 化済** | **7 回** | セッション横断記憶 (現在 10 entities + 11 relations) | PC 台帳 PJ で実戦投入後判断（5/13 頃）|
| 7 | ~~fetch~~ | 🗑 **削除済 2026-05-06** | — | （除去）Cursor **WebFetch**／`user-fetch` で代替 | — |
| 8 | sequential-thinking | ✅ active | **6 回** | 段階的思考 | PC 台帳 PJ で実戦投入後判断 |
| 9 | **kintone (公式)** | ✅ active | **69 回** | kintone API CRUD | 5/13 後（本番運用後）|
