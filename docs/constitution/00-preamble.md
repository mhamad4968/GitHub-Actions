...
- **doc-lane（資料作成）**（**PPTX / DOCX / マニュアル**・`C:\tmp\` 正本）と **kintone customize/deploy** は **別レーン**。資料作成中は **deploy 混在禁止**（`docs/runbooks/doc-lane-autonomous-governance.md` **R-DOC-01**）。**経営会議セキュリティ**は **R-DOC-11**（`docs/runbooks/keiei-kaigi-security-report.md`）。
