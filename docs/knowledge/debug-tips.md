...
<!-- errors: npm run verify:session-close-git-warn` **exit 0** → **その後** 項番 -0 | npm run cio:session:cold-start` | npm run session:bootstrap`**（**Read より前** `verify:constitution-handoff` / **`man -->
