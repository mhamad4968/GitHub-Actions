...
**手順**: `npm run cio:session:cold-start` → `npm run cio:report-verify-response -- --file <下書き>`**（または `--stdin`）を **実行し exit 0 を確認**する（`package.json` の `cio:report-` → `npm run cio:report-verify-response -- --file <下書き>`**（または `--stdin`）を **実行し exit 0 を確認**する（`package.json` の `cio:report-`
...
<!-- errors: npm run cio:session:cold-start` → `session:bootstrap` — **浜田依頼待ち**（項番 -0） | npm run cio:session:cold-start` | npm run cio:report-verify-response -- --file <下書き>`**（または `--stdin`）を **実行し exit -->
