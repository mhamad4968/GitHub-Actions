...
<!-- errors: npm run cio:session:cold-start` → `session:bootstrap` — **浜田依頼待ち**（項番 -0） | npm run cio:session:cold-start` | npm run session:bootstrap`**（**Read より前** `verify:constitution-handoff` / **`man -->
