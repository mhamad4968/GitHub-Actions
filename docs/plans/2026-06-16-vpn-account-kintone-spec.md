...
> **起票**: 2026-06-16 (火)  
> **状態**: **v1 完成 — CLOSED**（2026-06-17）／ **v1.1 3ドメイン統合 — 実装完了**（2026-06-20）／ **v1.2 PC台帳連携 + アプリ名変更 — 実装完了**（2026-06-20 浜田目視 OK）／ **v1.3 リスト出力 + VPN接続情報 — 実装完了**（2026-06-21 浜田目視 OK）  
...
| **VPN ID の入力ミス・訂正** | ダッシュ **編集**で `vpn_id` を変更 | **2026-08-01**: 読取専用を解除。変更時は備考へ `[VPN ID修正] YYYY-MM-DD 旧 → 新` を自動追記し、PC台帳674の個人・利用中 PC へ `vpn_id`/`vpn_pw` を同期。重複・ドメイン形式チェックは維持 |
| **削除済み ID の再利用** | **禁止** | 採番カウンタは削除で戻さない（§6）。訂正は「編集」で行い、誤 ID を削除して同じ文字列を再採番しない |
...
| **作成後** | **編集で訂正可**（2026-08-01）。備考自動履歴＋674 同期。削除後の同一 ID 再利用は禁止 |
...
| 3 | VPN ID | `vpn_id` | 文字列(1行) | ○ | ○ | unique。編集で訂正可（履歴→備考・674同期） |
...
| **編集** | アカウント名・所属・VPN ID（**訂正可・確認ダイアログ＋備考履歴**）・パスワード・登録日・備考 |
...
| 台帳 BUILD（最終） | 734=`2026-06-21-vpn-conn-pw-updated-date` **rev 28** / fileKey `8d2a21e0-d570-43c9-b908-1c2bab1d8c1b` |
| DB BUILD（最終） | 733=`2026-06-20-vpn-db-rename-message` **rev 11** |
| PC台帳 BUILD（連携） | 674=`2026-06-20-674-vpn-readonly-dom-lock` **rev 245** |
| 状態 | **v1.3 完成 — 運用中**（§19–§21） |
...
- **編集**: VPN ID は **訂正可**（2026-08-01）。変更時は備考へ `[VPN ID修正] YYYY-MM-DD 旧 → 新` を自動追記し PC台帳674へ同期。所属変更でドメイン整合が取れない場合は **保存不可**  
...
## §19. v1.3 リスト出力（Excel + 印刷）— 2026-06-21 浜田 GO

| 項目 | 内容 |
|------|------|
| **背景** | 管理者向けに所属・ドメインで絞った一覧を Excel / 印刷で出力したい |
| **UI** | ツールバー **「リスト出力」** → モーダル（**所属**=複数選択リスト+一括ボタン **全選択/全解除/本社/支店・営業所**／**ドメイン**=「すべて」または個別複数選択） |
| **Excel** | **`.xlsx` のみ**（SheetJS 同梱・CSV なし）。列: 登録日・アカウント名・所属・ドメイン・VPN ID・パスワード・備考 |
| **印刷** | A4 横向き・同一列構成・条件サマリ付き |
| **絞込** | 所属 OR × ドメイン OR（両方 AND）。並び: DEPT_ORDER → アカウント名 → VPN ID |
| **BUILD** | `2026-06-21-vpn-list-export-dept-honsha` |
| **§1.3 更新** | 「管理者向け一覧印刷」は **v1.3 で追加**（利用者渡し用行印刷は維持） |

---

## §20. v1.3 VPN接続情報アコーディオン — 2026-06-21 浜田 GO

| 項目 | 内容 |
|------|------|
| **位置** | 一覧 UI・**ライセンス集計アコーディオンの直上** |
| **UI** | `<details>` アコーディオン・**初期は閉**・見出し「VPN接続情報（必要時のみ開く）」・本文見出し **VPNアカウント管理画面URL**（文字大きめ） |
| **内容** | RADIUS 管理 URL・3 ドメイン各 **管理者 ID/PW**・**VPN 接続先 URL**（ユーザー指定文面どおり） |
| **PW 編集** | 3 ドメインの **接続パスワード**をアコーディオン内で編集可。**733 設定レコード**（`record_kind=設定`）の `password` / `note` に保存。変更時 `note` に **`yyyy/mm/dd`** を保存し、PW 横に **`(yyyy/mm/dd更新)`** を表示 |
| **BUILD** | `2026-06-21-vpn-conn-pw-updated-date` |

---

## §21. v1.3 完成サマリ — 2026-06-21 浜田目視 OK

| 項目 | 内容 |
|------|------|
| **URL** | [734 VPNアカウント台帳](https://jbis-kintone.cybozu.com/k/734/) |
| **追加機能** | §19 リスト出力（xlsx + 印刷）／ §20 VPN接続情報アコーディオン（接続 PW 編集・更新日表示） |
| **bundle** | SheetJS 同梱 — `npm run vpn-account:bundle-dash` → `customize/vpn-account-dash/desktop.js` |
| **733 設定レコード** | ドメイン別 3 件 — 採番 `next_user_num` に加え **接続管理者 PW**（`password`）と **更新日**（`note` = `yyyy/mm/dd`）を保持 |
| **判定** | **v1.3 完成** — CLOSED 維持（拡張のみ・§19–§21） |
| **BUILD / rev** | 734=`2026-06-21-vpn-conn-pw-updated-date` **rev 28** |

---

## §22. UX 改善（R61 addendum）— 2026-08-16

**状態**: `closed-v1` を維持したまま UX のみ改善。DB 733・フィールド・CRUD・集計計算は変更しない。浜田目視待ち。

| ID | 実装内容 |
|----|----------|
| 1 | ツールバーを **登録 / 検索・絞込 / 出力** の fieldset に整理 |
| 2 | 一覧 sticky を `border-collapse: separate` + ヘッダー影で安定化 |
| 3 | 次 VPN ID バナーを維持し、非操作の **全件 / 表示件数** チップを追加 |
| 4 | アカウント名・VPN ID・パスワードのクリックコピーを明示（空のアカウント名は対象外） |
| 5 | 一覧のドメインを表示専用ピル化 |
| 6 | ツールバーと行操作ボタンを 36px 化（モーダル・接続・集計ボタンは対象外） |
| 8 | 管理者向け一覧印刷だけに「本紙は機密性の高い内容を含みます。」を追加。利用者単票の既存【ご注意】は維持 |

**Live**: BUILD=`2026-08-16-dashboard-ux-polish` / rev **32** / fileKey `47bbc59a-9dbd-4e02-8a04-4e46c4e2deeb`

---

## §23. ライセンス集計 UX（1–4）— 2026-08-16

**状態**: CLOSED 維持。ライセンス UX 1–4・案5 浜田目視 **OK**（2026-08-16）。改善レーンクローズ。

| ID | 実装内容 |
|----|----------|
| 1 | 比較基準を **当月未満の最新確定** に変更（当月確定直後の自己比較を解消）。列名「前月比」→「**確定比**」 |
| 2 | 所属空欄・所属マスター外・未登録ドメインの **ソフト警告**（集計は続行可） |
| 3 | 対象月の **未確定/確定済**・保存時刻・比較対象月のステータスカード |
| 4 | 所属表の表示切替 **全表示（既定） / 1口以上 / 差分ありのみ**。合計行は常に全件 |

**Live**: BUILD=`2026-08-16-license-compare-ux` / rev **33** / fileKey `80a07af4-3f35-4dbd-b513-802e03e1d5fe`

---

## §24. ライセンス口数クリック一覧（案5）— 2026-08-16

**状態**: CLOSED 維持。案5 浜田目視 **OK**（2026-08-16）。改善レーンクローズ。
| 非対象 | 前回確定・差分・金額・確定比 |
| モーダル列 | アカウント名 / 所属 / ドメイン / VPN ID / パスワード |
| データ | 読込済 `state.records` のみ（追加 API なし） |
