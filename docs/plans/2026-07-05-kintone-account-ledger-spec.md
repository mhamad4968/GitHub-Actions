...
| アカウント余剰数 | — | 表示のみ | **総契約数 − 利用数** |
| 月額利用料合計 | — | 表示のみ | **利用数 × 1アカウント月額利用料** |

**UI 要件**:

- 数値入力欄 + **保存** ボタン（または blur 時に設定 REST 保存）。
- 保存後 **「集計を更新」** で月次表の金額列を再計算。
- ヒント文言例: 「Kintone の価格改定時は **月額単価** と **総契約数** を更新してください。」
- xlsx / 印刷出力にも **当時の単価・総契約数** をヘッダ注記として含める。
...
| — | 権限 | **admin のみ** |
| — | ポータル | **浜田手動** |
| — | 契約・月額 | **月次費用集計表直上で admin 編集可**（総契約数・1アカウント月額）。Kintone 価格改定対応。**コード固定禁止** |
