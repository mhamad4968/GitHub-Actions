...
2. **アカウント集計**（所属グループ × 所属）を **745 型・月列**で表示（**所属グループ単位小計** — §5.1）。
...
### 5.0 集計共通（月ベース — 浜田確定 2026-07-05）

**すべての集計表は月ベース**（745 型）。列 = **YYYY-MM**（指定期間の各月）。

**期間 UI**（アカウント集計・月次利用費用 **共通**）:

| 項目 | 内容 |
|------|------|
| 期間 | **YYYY-MM ～ YYYY-MM**（1 か月 / 複数月）。**当年通年**ボタン |
| 更新 | **集計を更新** ボタン（条件変更後に押す） |
| リセット | **条件クリア** — 期間を当年通年・チップをすべて ON |
| 出力 | xlsx / 印刷（対象月列をそのまま出力） |

**月末稼働判定**（745 同型 — `start_date` / `end_date` ベース）:
...
> **登録日（`start_date`）** は稼働判定に使用するが、集計の粒度は **月単位の月末スナップショット**。新規登録数だけを数える列は **作らない**。

### 5.1 アカウント集計（745 型 — 月次・所属グループ × 所属）

**目的**: 所属グループ × 所属 の **月末稼働数**を **月列** で表示（§5.0）。
...
- **出力**: xlsx / 印刷
- **初期表示**: 集計表を開くと **当月** 列が視認しやすい位置（745 v16 同型）
...
> v1 注意: **総契約数**は設定レコードの **現在値** を各月の余剰算出に使用（契約数の月次履歴は v1 では保持しない）。
...
| アカウント利用数 | — | 表示のみ | **当月** の月末稼働数（全社） |
| アカウント余剰数 | — | 表示のみ | **当月** の余剰（**本社配賦** — §5.2.2） |
| 月額利用料合計 | — | 表示のみ | **当月** の全社小計 × 単価 |
...
- **余剰分**は DB レコードを持たない。**各月 M** の §5.0 判定 + **総契約数（現在設定）** から算出。
...
| **利用終了** | 「利用終了」ボタン |
| **アカウント集計** | §5.1 — **月列**（745 型）— アコーディオン |
| **月次利用費用** | §5.2 + **§5.2.1 集計表直上の契約数・月額編集** — **月列** — アコーディオン |
...
| Q | 内容 | 回答 |
|---|------|------|
| Q1 | 集計の種類 | **両方とも月ベース**（§5.0）。**アカウント**=所属グループ×所属の **月末稼働数×月列** / **費用**=支払箇所× **月末件数×月列**（余剰は本社） |
...
| 13 | 集計粒度 | **月ベース（745型）** | アカウント・費用とも月列。浜田確定 2026-07-05 |
