...
### 5.3 ダッシュ上部サマリー（Excel 行2–5 相当）

| 項目 | 初期値 | 編集 |
|------|--------|------|
| 1アカウント月額 | 1,800 | admin 手入力可 |
| 総契約数 | 77 | admin 手入力可 |
| アカウント利用数 | **稼働中件数から自動** | 表示のみ |
| アカウント余剰数 | **契約数 − 利用数** | 表示のみ |
| 月額利用料合計 | **利用数 × 単価** | 表示のみ（契約変動時は手入力契約数を優先） |

保存先: 台帳アプリの **設定レコード**（VPN 734 `record_kind=設定` 同型）または localStorage + 設定 REST。**実装時に 734 パターンを踏襲**。
