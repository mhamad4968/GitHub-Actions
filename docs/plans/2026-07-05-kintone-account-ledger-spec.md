...
> **起票**: 2026-07-05 (日)  
> **状態**: **v1.3 本番運用中（CLOSED）** — Excel 廃止・リンク設置済・契約数/月額 **752 DB 保存**（2026-07-05）  
> **App ID**: **752**（DB）/ **753**（台帳）— `scripts/data/kintone-account-app-ids.json`  
> **最終 BUILD**: DB fields rev6 / 台帳 `2026-07-05-kintone-account-dash-v20-fee-settings-kintone`  
> **移行元 Excel**: `Kintoneアカウント一覧.xlsx`（**2026-07-05 廃止** — 履歴参照のみ。正本は 752/753）  
...
4. AI 実行: kintone アプリ作成 → フィールド deploy → Excel **74 件移行** → ダッシュ customize → 集計・出力 → 浜田 **目視 OK** → **kintone のみ正本運用**（**Excel 廃止済 — 2026-07-05**）。
...
- 社内 **Kintone ライセンス ID**（ログイン ID）を **Excel** で管理していた（**2026-07-05 廃止 → kintone 正本**）。
...
| Space 48 ポータルリンク | **設置済**（2026-07-05 浜田） |
...
| **② 表示期間の月別設定** | 表示期間（YYYY-MM ～ YYYY-MM）の **全月を読み取り専用表**で一覧（スクロールなし）。`2026年7月` 形式。**月別設定を保存** で **752 DB** に確定 |
...
| 項目 | 内容 |
|------|------|
| **保存先（v1.3）** | **752 DB** — 734 型 `record_kind` レコード（**全端末共通**） |
| **既定値** | `record_kind=設定` · `login_id=__kac_fee_defaults__` · 総契約数 / 月額 |
| **月別** | `record_kind=月次設定` · `snapshot_month=YYYY-MM` · 総契約数 / 月額 |
| **保存** | **月別設定を保存** → REST PUT/POST（752） |
| **ドラフト** | **表に反映** は画面内ドラフトのみ。**保存** まで DB には書かない |
| **移行** | 初回ロード時、localStorage にのみデータがある場合 **自動で 752 へ移行** 後 LS 削除 |
...
**保存先（v1.3）**: **752 DB** — `record_kind` 設定レコード（734 型）。**全端末共通**。
...
| — | Space | **48** |
| — | 権限 | **admin のみ** |
| — | ポータル | **Space 48 リンク設置済**（2026-07-05） |
| — | Excel | **廃止済** — 正本は 752/753 のみ（2026-07-05） |
...
| 日付 | 内容 |
|------|------|
| 2026-07-05 | **v1.3 CLOSED** — 契約数・月額を **752 DB 設定レコード**へ移行（localStorage 廃止・自動移行）BUILD v20 |
| 2026-07-05 | **本番運用開始** — 浜田: Space 48 ポータルリンク設置・Excel 廃止。§0 完了 |
