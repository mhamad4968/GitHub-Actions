...
- 風速・降雨と **地点不一致時は警告**（猛暑日のみ取込は可）
- **観測地点（`obs_location`）は変更しない**（風速/降雨で確定済みの場合）
- **UI 正本**: 取込完了ダイアログに注記（`console.warn` 禁止）
- 例: 「WBGTの地点（さいたま）は観測地点（東京）と異なります。猛暑日参考のみに使用します。」
...
## 8. 部分 GO — 触らないコード（implement 厳守）

| 区分 | パス / 対象 | 理由 |
|------|-------------|------|
| **稼働計算コア** | `scripts/workdays-calc-core.mjs` | 猛暑日は参考表示のみ · `calc-gate` 不変 |
| **heat 以外の 688 UI** | `customize/688/desktop.ui.js` の calc/再算出ロジック | 猛暑日は post-render・CSV 取込・印刷のみ |
| **677–679** | 予実管理レーン全体 | 保留 |
| **688 保留範囲** | 猛暑日以外の新機能・仕様変更 | 浜田 部分 GO スコープ外 |
| **結果フィールド** | `result_scaffold_days` / `result_paint_days` / 不稼働率・雨休率の式 | 猛暑日を含めない |

**gate**: `workdays:calc-gate` + `workdays:heat-reference-gate` が緑であること。
