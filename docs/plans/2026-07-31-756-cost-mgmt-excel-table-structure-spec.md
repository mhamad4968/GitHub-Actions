...
**Excel費目枠オーバーライド（#R-EXCEL-UI-09）**: コード表の constructionMenu（材料費・外注費…）より **原価管理明細の費目枠が正**。例: **10200 → 塗装工事**、**10300 → 足場工事**、**10400 → 塗装・足場工事**、**10600 → 修繕等工事**、**10700 → 塗装附帯工事**、および **軌道工事・調査設計費・外注試験費・交通規制費・追加工事①〜⑤**（いずれも種別なし・詳細2セル＝その他材料費と同型）。`JY2_COST_MGMT_HIMOKU_OVERRIDE` / `OVERRIDE_BY_NAME` ＋ `TYPELESS`。空のときは操作列＋で最初の詳細を追加。共通仕様（SUM/色/太字/揃え）は #R-EXCEL-UI-06〜08 を継承し、差分だけ足す。**区分もコード表 resolve（`JY2_COST_CATEGORY_BY_WORK_TYPE_CODE`）。ENSURE で施工固定しない**（例: 11100〜＝保安）。#C-EXCEL-01 / S-CAT-01。

**配置順（#R-EXCEL-PLACE-01 / S-ORDER-01）**: 意図順は **10700 → 名称枠群（軌道〜追加工事）→ 10800 → 10900 → オペレーター → その他コード枠**（11000〜11300・11600 以降はオペレーター直後のコード枠群）。並び替えは業務クラスタ単位。**コード番号順だけで名称枠を押し下げない**。

**同一工種コードの重複非表示（S-DEDUP-01）**: 同一システム工種コードのブロックが複数あるとき、原価管理では **正規1件のみ表示**（名称一致を優先）。内訳 App757 は非破壊。`jy2CostMgmtDuplicateCodedBlockIdSet`。
...
**コード空の名称枠（#R-EXCEL-UI-11）**: Excelでシステム工種番号が空の枠も表に出す（例: 軌道工事等）。費目枠＝名称。レイアウトは **10700 と同型（TYPELESS・詳細2セル）**。`HIMOKU_OVERRIDE_BY_NAME`。追加工事はコード 14100〜14500 があればコード側 OVERRIDE も可。**内訳にブロックが無いときは原価管理表示時に空ブロックを自動追加**し **10700 直後（名称枠群）へ並べる**。その後ろに 10800／10900（#R-EXCEL-PLACE-01）。一時保存で App757 へ。廃止済みは再作成しない。内訳連動の詳細は月曜以降。

**費目→昼間／夜間→詳細（#R-EXCEL-UI-12）**: 例 **10900 工事管理者賃金**（費目＝出向工事管理者／その他工事管理者）、**11000 工事安全専任管理者賃金**、**11100 線閉責任者賃金**、**11200 列車見張員賃金**、**11300 交通整理員賃金**、および **建設機械オペレーター**（システム工種コードなし・費目＝建設機械オペレーター／その他労務）。コード表の「労務費」「外注労務費」や旧種別名は原価管理では使わない。各種別枠は **昼間／夜間**。その下に詳細を操作列＋で追加。`HIMOKU_OVERRIDE` / `OVERRIDE_BY_NAME` ＋ `TYPES_OVERRIDE` / `TYPES_OVERRIDE_BY_HIMOKU`。オペレーター枠は内訳に無ければ自動追加し **10900 直後**へ並べる。共通仕様（SUM/色/太字/揃え）は #R-EXCEL-UI-06〜08 を継承。
...
|--|------------------------|-----------------|
| 行の見せ方 | 親＝工種合計、`＋`で明細 | Excel同型の階層（費目枠＋種別行）。**費目単位▶／▼開閉**（#R-EXCEL-UI-16）。閉じ時は費目SUMのみ。空詳細は出さない |
...
| 2026-08-02 | 費目単位▶／▼開閉（#R-EXCEL-UI-16）。閉じ時は費目名＋計画数量/実行予算SUM。空詳細は出さない。操作＋は費目を開いて詳細追加。BUILD `2026-08-02-ver02-actual-himoku-fold` |
| 2026-08-02 | 費目▶は詳細があるときだけ表示。既定はすべてクローズ。#R-EXCEL-UI-16。BUILD `2026-08-02-ver02-actual-himoku-fold-closed` |
| 2026-08-02 | 表上に「すべて展開／すべて閉じる」（▶対象＝詳細あり費目のみ）。#R-EXCEL-UI-16。BUILD `2026-08-02-ver02-actual-himoku-fold-all` |
...
| 2026-08-01 | 10800鎌ヶ谷資材使用料を名称枠群（無ければ10700）の直後へ。#R-EXCEL-UI-09。BUILD `2026-08-01-ver02-actual-excel-10800-after-10700` |
| 2026-08-01 | 10900工事管理者賃金（出向工事管理者）を10800鎌ヶ谷の直後へ。#R-EXCEL-UI-09。BUILD `2026-08-01-ver02-actual-excel-10900-after-10800` |
...
| 2026-08-01 | 同一システム工種コードの重複枠は正規1件のみ表示（11100二重など）。ENSURE区分はコード表。#R-EXCEL-UI-09。BUILD `2026-08-01-ver02-actual-excel-dedupe-coded` |
| 2026-08-01 | 夕反省全承認: 配置順固定・PLACE-01・受入ミニチェック・ENSURE区分針。GO `2026-08-01-evening-reflection-hamada-go`。#R-EXCEL-PLACE-01/S-ORDER-01/C-EXCEL-01 |
