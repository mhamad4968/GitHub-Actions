...
> **App ID**: **744**（DB）/ **745**（台帳）— `scripts/data/jre-cloud-account-app-ids.json`  
> **最終 BUILD**: DB `2026-06-26-jre-cloud-account-db-block-v1` rev5 / 台帳 `2026-06-27-jre-cloud-account-dash-v16-list-filter-clear` rev22  
...
| 所属組織 | **チップ toggle**（緑=ON / 灰=OFF）。**すべて**リンクで全選択 |
| 部署 | **チップ toggle**（同上） |
| 更新 | **集計を更新**ボタン（条件変更後に押す） |
| リセット | **条件クリア** — 期間を当年通年・所属/部署をすべて ON に戻して再集計 |
...
### 7.6 一覧キーワード検索（745 — 2026-06-26 浜田 GO / R-JRE-01）

| 項目 | 内容 |
|------|------|
| **対象** | `user_id` / `user_name` / `org` / `dept` / `formatDeptLabel(org,dept)` / `phone` / `mail` / `note` |
| **結合 haystack** | `org + dept`（スペース・連結・`/`・`・` 等）— **DB は分離のまま**（集計キー不変） |
| **スペース区切り** | **AND** — 各トークンが haystack 内の部分一致 |
| **全角スペース** | 半角スペースに正規化してから分割 |
| **例: `東京支店`** | 所属 `org=東京支店` の全レコード（施工部等を含む） |
| **例: `東京支店 施工部`** | 東京支店 **かつ** 施工部 |
| **例: `施工部`** | 全部署の施工部 |
| **実装** | `buildRecordSearchHaystack` / `recordMatchesSearch`（`desktop.src.js`） |

**目視確認（R-SESS-06）**: `docs/runbooks/kintone-dash-first-visual-checklist.md`
...
| 機能 | 内容 |
|------|------|
| **一覧** | Excel 風テーブル・キーワード検索（§7.6）・**クリア**（§4.3） |
| **フィルタバッジ** | 稼働中 / すべて / 退職・無効 |
| **新規・編集** | モーダル。595 検索（**クリア**でキーワード・結果を消去） |
| **退職** | 「利用終了（退職）」ボタン |
| **集計** | アコーディオン（初期 **閉**）。§5 — チップ絞込・**条件クリア** |
...
| Q11 | 一覧検索 | **org/dept 単独 + AND + 結合表現**（§7.6）。DB は org/dept 分離維持 |
...
| 日付 | 内容 |
|------|------|
| 2026-06-27 | **完了** — 集計条件チップ UI・条件クリア・検索クリア（表示→稼働中）。BUILD v16 |
| 2026-06-26 | v1 deploy（744/745）・Excel 99 件移行 |
| 2026-06-26 | §7.6 一覧検索（R-JRE-01 GO）・Q11 追記 |
