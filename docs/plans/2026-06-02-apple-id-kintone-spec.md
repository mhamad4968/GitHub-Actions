...
| 領域 | 内容 |
|------|------|
| **タイトル** | **Apple ID管理台帳**（3群の枠の外・左） |
| **ツールバー A** | 枠＋見出し「再読込・採番」— **再読込**・**採番**（新規作成 / このIDを割当。次採番 ID はメタバーに残す）。3群の枠の高さと、再読込／採番／クリア／印刷のボタン高さを揃える |
| **ツールバー B** | 枠＋見出し「絞込・検索」— すべて / **利用中** / **廃止** / **未割当**＋検索＋クリア。幅を伸ばして印刷を右端に寄せない |
| **ツールバー C** | 枠＋見出し「印刷」— **印刷**（選択行の単票）。B の直後（右端固定しない） |
| **メタバー** | **全件**の件数（絞込に依らない）を色付きチップで表示: **全件**（灰）/ **利用中**（緑）/ **未割当**（琥珀）/ **廃止**（灰）。数字は大きく、単位は「件」。0件は薄く残す。＋ 次採番 |
| **表** | §9.2 の列。**thead sticky**。`status` はピル。未割当・廃止は行色、iPhone/iPad は行ティント（廃止優先）。`apple_id` もクリックコピー |
| **行操作** | **編集** / **廃止** / **削除** / 行チェック（印刷対象） |

### 9.2 表の列（左→右）

`legacy_no` | `status` | `device_type` | `registered_date` | `device_exchange_date` | `mdm_name` | `user_name` | `phone_number` | `apple_id` | `password`* | `lock_passcode`* | 操作
...
| **編集** | **登録区分（新規/交換ラジオ）**・**Apple ID**（登録ミス修正）・**パスワード**・**ロックパス**（空なら標準値）・登録日（表示のみ）・**端末交換日（交換時のみ表示・必須）**・MDM名・氏名・回線・端末種別・メモ。新規保存時は交換日をクリア。MDM名変更時は区分を交換へ寄せ登録日を当日更新 |
| **廃止** | 確認（Apple ID・氏名表示）→ `status=廃止`（**退職・使用停止**） |
| **削除** | 確認（Apple ID 表示・利用者情報ありなら追加警告）→ REST DELETE（**誤登録**） |

### 9.4 印刷

- **対象**: 選択行（**利用中**推奨）。**PC台帳 674 と同型**の別ウィンドウ帳票（緑テーマ・グリッド表）。**A4 縦**・文字は画面より大きめ。
- **見出し**: **業務携帯アカウント（印刷用）**。
- **注意書き**: **「本紙は機密性の高い内容を含みます。」のみ**（指紋認証禁止・パスコード変更禁止などの箇条書き、利用者責任の長文は出さない。2026-08-16 浜田）。
- **グリッド**: **端末登録日または端末交換日（どちらか一方）/利用者名** → **回線/iCloud/パスワード** → **Apple ID/パスワード/ロックパス**。MDM行は印刷しない（2026-08-11）。端末種別はバッジのみ。
...
- **表示面**: 画面＝ツールバー・件数・ピル等。単票印刷＝上記見出し＋注意1行。一覧印刷・Excel は対象外。
...
| 2026-08-16 | §9.1 ツールバー3群・全件件数・sticky/ピル/行色。§9.4 印刷見出し・注意1行・A4縦大きめ文字 |
| 2026-08-16 | §9.1 3群を枠＋legend。印刷は右端固定せず絞込の直後 |
| 2026-08-16 | §9.1 3枠の高さを揃える。§9.2 端末列を状態の直後へ |
| 2026-08-16 | §9.1 再読込／採番／クリア／印刷のボタン高さを揃える |
| 2026-08-16 | §9.1 メタバー件数を色チップ（全件／利用中／未割当／廃止） |
