...
| **登録日** | — | 251 | `registered_date` | 移行時のみ Excel 値。**新規はレコード作成日**（§8） |
...
| 3 | 登録日 | `registered_date` | 日付 | ○ | — | **新規レコード作成日**（POST 当日を自動セット。割当操作では更新しない） |
| 4 | MDM名 | `mdm_name` | 文字列(1行) | — | — | ダッシュ入力 |
| 5 | 氏名 | `user_name` | 文字列(1行) | — | — | **`姓　名`（全角スペース）**（§8.2） |
| 6 | 回線番号 | `phone_number` | 文字列(1行) | — | — | ダッシュ入力 |
| 7 | Apple ID | `apple_id` | 文字列(1行) | ○ | ○ | 採番時自動 |
| 8 | パスワード | `password` | 文字列(1行) | ○ | — | 自動 |
| 9 | ロックパス | `lock_passcode` | 文字列(1行) | ○ | — | 自動 |
| 10 | 端末種別 | `device_type` | ドロップダウン | — | — | `iPhone` / `iPad` / `その他` |
| 11 | メモ | `note` | 文字列(複数行) | — | — | 任意 |
...
### 8.3 登録日（`registered_date`）

- **新規採番**: API POST 時に **当日（JST）** をセット = **レコード登録日**。
- **移行**: Excel **登録日**があればその日付、**空の行**は移行 POST 日（= レコード作成日）と同値でよい。
- **利用者情報の後追い入力**では **`registered_date` を更新しない**。

### 8.4 バリデーション

- `apple_id`: 新規採番は `^jbis\.\d{3}@icloud\.com$`。移行 `kent.*` 等は `^[\w.-]+@icloud\.com$` を許容。
