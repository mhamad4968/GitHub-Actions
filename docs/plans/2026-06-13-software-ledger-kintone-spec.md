...
- [ ] Space 21 に **ソフトウエア台帳DB** 作成（§7 フィールド **18 項目**）
- [ ] **アクセス権限** — Space 21 + **`system` グループ**（694/674 同帯・§3.1）
- [ ] **ソフトウエア管理台帳ver.1**（ダッシュ）作成
- [ ] **ソフトウエア台帳DB**: 保存・削除禁止 customize（693 / `apple-id-db` 型・§4.1）
- [ ] `kintone-apps.md` に App ID 追記

### 12.2 customize

- [ ] `customize/software-ledger/` — 694 `desktop.js` ベース（**ソフトウエア管理台帳ver.1**）
- [ ] `customize/software-ledger-db/` — DB 保存・削除ブロック（**ソフトウエア台帳DB**・§4.1）
...
- [ ] DB 標準 UI から保存・削除できないこと（**ソフトウエア台帳DB**・§4.1）
...
| 2026-06-14 | §4.1 — ダッシュアプリ名 typo 訂正 **vre.01 → ver.1**（浜田） |
