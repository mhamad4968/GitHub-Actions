...
| 入力項目 | 必須 | 検索方式 | 例 |
|----------|------|----------|-----|
| **所属名** | — | **複数選択**（680 全件＋台帳追加値。括弧内は台帳件数・0 件も選択可。**2 列グリッド**（長い所属名は折返し）。全選択/全解除/件数ありのみ。同一項目内 OR 完全一致・未選択は指定なし。関越支店施工部・東京支店施工部・東京支店橋りょうリペア部を含む。SKYSEA 除外リストは使わない） | 仙台営業所・関越支店施工部・東京支店施工部 |
