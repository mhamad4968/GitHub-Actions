...
| 14 | 社員番号 | `emp_id` | 文字列(1行) | 個人時 ○（kintone 必須オフ） | — | 595 連携。共有は空 |
| 15 | 氏名 | `user_name` | 文字列(1行) | 個人時 ○（kintone 必須オフ） | — | 595 連携。共有は空 |
...
- **2026-09-10**: 新規 VL 保存成功後の連続登録 UI — `docs/plans/2026-09-10-715-vl-serial-continue-spec.md`
...
| 項目 | 状態 | 内容 |
|------|------|------|
| **674 設置先連携（個人/共有 + PC 必須）** | **v1.2 実装済**（2026-08-19 deploy） | `docs/plans/2026-08-19-715-pc-install-target-spec.md` — 715 BUILD `2026-08-19-715-install-target-pc` rev 25 |
...
| 2026-08-19 | **v1.2 実装** — 714 フィールド 6 追加・715 customize deploy rev25・595 は 715 共有行をミラー対象外。714 `emp_id`/`user_name` kintone 必須オフ（共有保存） |
