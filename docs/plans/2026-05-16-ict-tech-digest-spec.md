...
> **最終更新**: 2026-05-19（CIO）— **v2.2** AI・LLM を収集対象外（§2.4）。**v2.1** 国内優先・DX国内限定は §2.3。将来改善のみ §9 バックログ。  
...
`インフラ・通信・端末` / `開発トレンド` / `Box・SaaS・文書管理` / `DX人材・IT資格・組織` / `セキュリティ製品・技術` / `その他`

- **685**: ドロップダウンは上記6種（`scripts/update-685-category-dropdown.mjs` で更新可）
- **686**: 旧17種・旧 `AI・LLM` レコードは **表示・フィルタのみ** 現行6種へマッピング（`AI・LLM` → `その他`）
- **686 BUILD**: `2026-05-19-686-ict-digest-no-ai-llm`

### 2.4 v2.2 AI・LLM 除外（2026-05-19・CEO 指示）

| 項目 | 内容 |
|------|------|
| **方針** | **AI・LLM を主題とする記事は収集しない**（カテゴリからも削除） |
| **Gemini** | プロンプトで除外。`ai-exclusion.ts` で候補・厳選結果を二重フィルタ |
| **既存685** | `category=AI・LLM` の過去レコードは残る。686 では **その他** として表示・検索 |
| **686** | カテゴリフィルタに `AI・LLM` チップなし |
...
`インフラ・通信・端末` / `開発トレンド` / `Box・SaaS・文書管理` / `DX人材・IT資格・組織` / `セキュリティ製品・技術` / `その他`（**AI・LLM は v2.2 で廃止**・§2.4）

**旧17種の既存レコード**: kintone 上は旧値のまま。686 は `LEGACY_CATEGORY_TO_NEW` で表示・フィルタを現行6種に読み替え（`AI・LLM` → `その他`）。
