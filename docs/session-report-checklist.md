...
> **運用開始**: **2026-05-06 JST** — 本チェックリスト＋**`every-turn-rules-confirm.mdc` §1e**＋**hooks 自動検証**を、報告・日終わり・状況まとめのターンから **常時適用**（浜田 CEO 指示）。**報告時は本チェックの □ 本文をチャットに貼付**し、CEO が **チャットだけで目視検収**できるようにする（ファイル参照のみ禁止）。
