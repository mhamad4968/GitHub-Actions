...
>
> **報告・締め・GO 前の一発ゲート（2026-05-09 抜本）**: まず **`npm run cio:report:draft -- --out <path>`** で機械検証可能な下書きを作り、`要記入` を実際の事実へ置換することを推奨する。その後、下書き全文に対し **`npm run cio:report-verify-response -- --file <path>`** を実行し **exit 0**（**CEO 最低基準の全行含有**＋**§1 四行先頭ウィンドウ**＋**V2**＋**□A1 最小**＋**禁止語**）。**verifier が常に正**であり、draft の生成成功だけでは報告可としない。hooks は **Cursor が `.cursor/hooks.json` を起動した経路に限り**機械検証するため、**本 npm を hooks と独立に必須**とする（`CEO-MINIMUM-ABSOLUTE-BASELINE.txt` §1 報告違反ゼロ 節）。
>
