...
**会計年度・長期連休（Claude 指示）**: 週次・月次の要約プロンプト（`user683-sync-summaries-to-kintone.mjs` / `user683_claude_relay.py`）では、**期末＝4月末・期首＝5月**の会計年度位置づけ（暦月との対応はモデル指示内の例に従う）と、**年末年始・GW・秋分前後の連休（シルバーウィーク）**を跨ぐ週／月では稼働日減を考慮するよう求めています（コーパスに根拠が無い断定は禁止）。
