...
| フィールド | `**最終更新**:` / `**次の1手**:` / `**Git**:` |
| **レーン変更**（#I-SESSION-LANE-01） | 本題切替時に `**レーン変更**:` 1 行（例: `736 仕様草案 → 688 UI 実装`）· **次の1手** を即上書き |
...
**禁止（浜田 2026-07-28）**: **完了済**の件を `GO待ち` / `次の1手` / 浜田への質問に載せない。履歴ブロックに残っていても **現行待ちと混同しない**（浜田の勘違い防止）。完了したら現行プレアンブルから消し、履歴側に「不要確定」を残す。

**D-CHKPT-DONE-01（2026-08-06 GO）**: 浜田が完了／削除／不要と裁定した件は **同一ターン**で `**次の1手**:`／`GO待ち` から外す（遅延更新禁止）。
