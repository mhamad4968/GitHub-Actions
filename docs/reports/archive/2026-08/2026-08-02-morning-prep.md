# 🌅 朝のブリーフィング — 2026-08-02 (Sun) 06:00
...
- 課金日: 毎月 15 日 / 次回リセット **2026-08-15** (残 **13** 日)
...
### 🤖 自律判断ログ（2026-08-01 / autonomy scan）
...
**朝報生成時刻**: 2026-08-02T06:00:16.727+09:00 (この時刻以降の編集は翌朝報で確認)
...
- L1678: - [ ] 5タブが存在し、役割が混在していない（工事基本情報は総括と別）。
- L1679: - [ ] 代表工事で基本情報から内訳、総括へ追跡できる。
- L1680: - [ ] 内訳を工種ブロックで確認できる。
- L1681: - [ ] 内訳で工種ブロックを追加でき、総括へ反映できる。
- L1682: - [ ] 施工・保安・給与手当の3区分を区別して集計できる。
- L1683: - [ ] 内訳予算が予実管理へ一意に供給される。
- L1684: - [ ] 税抜実績を手入力できる。
- L1685: - [ ] 実績が予算版ではなく工事に1回だけ所属する。
- L1686: - [ ] 実績登録時予算版を監査参照できる。
- L1687: - [ ] 実績、残予算、消化率を工種別・施工/保安別に表示できる（**給与手当は予実表の対象外**＝Y4/RY-10）。
- ...他 11 件
...
- L187: - [ ] 材料費ブロック: 費目枠＋種別（塗料等）＋明細行が見える
- L191: - [ ] 旅費交通費・交際費: コード表どおりの種別が出る／行挿入できる
- L192: - [ ] 月次灰色が合計で、明細だけ編集できる
- L193: - [ ] 原価累計・予算との差が自動で、手入力できない
- L194: - [ ] 備考が編集・保存できる
- L195: - [ ] 実行予算額が手入力できる（暫定）
- L196: - [ ] 既存工事のレガシー実績が壊れない（またはフォールバック表示）
...
- NVM v24 スタック（cron・MCP・朝 prep 正本）:
  ...
  - **NVM v24 整合**: ✅ 100%
- Disk (`~`): 275.2G used / 184.3G free on C: — ✅
  - npm cache: 2.1G / npx cache: 1.7G
- Memory: 11526/15894 MiB (73%) — ✅
...
- `docs\plans\2026-07-19-jikkou-yosan-ver02-redesign-spec-draft.md` （更新: 2026-08-01 11:52）
- `docs\plans\2026-07-31-756-cost-mgmt-excel-table-structure-spec.md` （更新: 2026-08-01 11:52）
- `docs\plans\2026-06-16-vpn-account-kintone-spec.md` （更新: 2026-08-01 01:37）
