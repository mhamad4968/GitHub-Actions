# 🌅 朝のブリーフィング — 2026-08-31 (Mon) 06:00
...
- 直近消費: 🟢 6% (2026-08-30) — OK / 通常運用継続
- 月予算: L1 $200 (Ultra) + L2 $1000 (On-Demand cap) = **$1200**
- 課金日: 毎月 15 日 / 次回リセット **2026-09-15** (残 **15** 日)
- 線形回帰予測 枯渇日: **2027-07-22** ✅ リセット日以降
- AI 助言: 通常運用継続 OK
- 履歴件数: 8 日分
...
### 🤖 自律判断ログ（2026-08-30 / autonomy scan）
...
@kintone/cli   1.21.0  1.21.1  1.21.1  node_modules/@kintone/cli  kintone-ai-lab
eslint         10.8.1  10.9.1  10.9.1  node_modules/eslint        kintone-ai-lab
nodemailer      9.0.3   9.0.6   9.0.6  node_modules/nodemailer    kintone-ai-lab
```
...
**朝報生成時刻**: 2026-08-31T06:00:25.701+09:00 (この時刻以降の編集は翌朝報で確認)
...
> **136 件の未完了項目を 21 ファイルから検出**

#### 2026-04-18-skysea-installer.md

- L119: - [ ] PowerShell スクリプト雛形（突合 → 起動確認 → リモートインストール → 結果CSV） → **2026-04-25/26 持ち越し**
- L125: - [ ] 「📌 SKYSEA未導入」絞り込みを 674 で使う運用／UI を8月計画で確定する

#### 2026-04-21-new-pc-ledger-spec.md

- L777: - [ ] 他アプリからの 627/594/626/667 ルックアップ参照を grep で洗い出し
- L778: - [ ] 595/656/657 等が 627/594/626/667 を参照してないか確認
- L779: - [ ] kintone API で 627/594/626/667 を叩くスクリプト全件特定
- L780: - [ ] 削除直前に CSV 全件エクスポート → リポジトリ保存
- L781: - [ ] 削除直前の最後の JSON Snapshot 取得 (`data/snapshots/{627,594,626,667}-final-<date>.json`)
- L817: - [ ] 627 スキャン（個人 259 件に紐付くアカウント実態 / status / 重複紐付け確認）
- L818: - [ ] 廃棄 1 件の種別特定 → B-1/B-2 除外確定
- L819: - [ ] CSV マッピング表（594 + 627 + 595 → 新・PC台帳ver.1 の各フィールド）
- L820: - [ ] CSV 整形ロジック実装（PW 自動算出 / バリデーション 2 系統対応 / etc）
- L821: - [ ] CSV 出力 → 浜田レビュー → 浜田 import → 検証（件数一致 + 抜け漏れゼロ）
- ...他 7 件
...
- L355: - [ ] kintone レコード数 ≒ **1,146**
- L356: - [ ] 割当済行（氏名あり）≒ **251** 前後
- L357: - [ ] ダッシュで **採番・編集・廃止・削除** が動作
- L358: - [ ] 浜田 **目視 GO**
...
- L1698: - [ ] 5タブが存在し、役割が混在していない（工事基本情報は総括と別）。
- L1699: - [ ] 代表工事で基本情報から内訳、総括へ追跡できる。
- L1700: - [ ] 内訳を工種ブロックで確認できる。
- L1701: - [ ] 内訳で工種ブロックを追加でき、総括へ反映できる。
- L1702: - [ ] 施工・保安・給与手当の3区分を区別して集計できる。
- L1703: - [ ] 内訳予算が予実管理へ一意に供給される。
- L1704: - [ ] 税抜実績を手入力できる。
- L1705: - [ ] 実績が予算版ではなく工事に1回だけ所属する。
- L1706: - [ ] 実績登録時予算版を監査参照できる。
- L1707: - [ ] 実績、残予算、消化率を工種別・施工/保安別に表示できる（**給与手当は予実表の対象外**＝Y4/RY-10）。
- ...他 11 件
...
- L188: - [ ] 材料費ブロック: 費目枠＋種別（塗料等）＋明細行が見える
- L192: - [ ] 旅費交通費・交際費: コード表どおりの種別が出る／行挿入できる
- L193: - [ ] 月次灰色が合計で、明細だけ編集できる
- L194: - [ ] 原価累計・予算との差が自動で、手入力できない
- L195: - [ ] 備考が編集・保存できる
- L196: - [ ] 実行予算額が手入力できる（暫定）
- L197: - [ ] 既存工事のレガシー実績が壊れない（またはフォールバック表示）

#### 2026-08-05-skysea-vendor-inquiry-draft.md

- L46: - [ ] 上記本文をフォームへ貼付（会社名・部署があれば冒頭に追記）
- L47: - [ ] 文字数カウンタが 3000 以内
- L48: - [ ] 秘密情報（内部PC名・社員個人・社内URL）を書いていない
- L49: - [ ] 送信後、チャットに「送信済」と一言（AIが段Bへ進む合図）

#### 2026-08-08-evening-improvements-spec.md

- L56: - [ ] 仕様・approved-changes・runbook・mdc・用語がリポにある
- L57: - [ ] `npm run cio:674:inventory-hub-diag` が件数を出す
- L58: - [ ] 674 棚卸状況一覧メタにキャンペーン／年次の二系統表示
- L59: - [ ] commit + push + GHA 緑
...
#### 2026-08-23-jr-ipad-721-p0-p1-ux-spec.md

- L102: - [ ] `customize/jr-ipad-dash/desktop.src.js` BUILD 更新
- L103: - [ ] `scripts/jr-ipad-bundle-dash.mjs` SheetJS 連結
- L104: - [ ] `npm run cio:preflight:721` → `npm run deploy:721`
- L105: - [ ] `kintone-apps.md` / live-builds / 完成報告 R61 addendum
- L106: - [ ] closures `reclosed`（目視 OK 後）

#### 2026-08-29-jikkou-yosan-v2-master-g0-decisions.md

- L466: - [ ] 736 不触
- L467: - [ ] 756／757／758 を使い続ける
- L468: - [ ] 工事原価管理タブ非表示（データ・758・予実コードは残す）
- L469: - [ ] フィールド追加カタログ（ヘッダDD・事務所・休日・工種説明・材料列）
- L470: - [ ] `desktop.js` と `desktop.ui.js` を同じ変更
- L471: - [ ] 既存値が新リスト外のときの扱い（表示維持／マスタ追加／マップ）を実装中に決めて落とさない
...
> ℹ️ **constitution-aide**: 月曜/強制のため **フル smoke（ingest）**（`MORNING_PREP_RAG_AIDE=1` でフル）。月曜フル後は MCP で2クエリ目視。
...
[constitution-aide-trial] mirror docs/runbooks/session-lifecycle-v2.md → .rag\extra-docs\constitution-aide-trial\session-lifecycle-v2.md
[constitution-aide-trial] mirror docs/runbooks/push-deploy-quality-gates-v2.md → .rag\extra-docs\constitution-aide-trial\push-deploy-quality-gates-v2.md
[constitution-aide-trial] mirror docs/runbooks/checkpoint-handoff-template-v2.md → .rag\extra-docs\constitution-aide-trial\checkpoint-handoff-template-v2.md
[constitution-aide-trial] mirror docs/runbooks/ai-team-tool-routing-v2.md → .rag\extra-docs\constitution-aide-trial\ai-team-tool-routing-v2.md
[constitution-aide-trial] mirror docs/runbooks/rag-constitution-aide-trial.md → .rag\extra-docs\constitution-aide-trial\rag-constitution-aide-trial.md
[constitution-aide-trial] mirror chat-sessions/checkpoint-latest.md → .rag\extra-docs\constitution-aide-trial\checkpoint-latest.md
[constitution-aide-trial] mirror docs\reports\2026-08-19-evening-reflection.md → .rag\extra-docs\constitution-aide-trial\2026-08-19-evening-reflection.md
[constitution-aide-trial] mirror docs\reports\2026-08-21-evening-reflection.md → .rag\extra-docs\constitution-aide-trial\2026-08-21-evening-reflection.md
[constitution-aide-trial] mirror docs\reports\2026-08-22-evening-reflection.md → .rag\extra-docs\constitution-aide-trial\2026-08-22-evening-reflection.md
[constitution-aide-trial] mirror docs\reports\2026-08-23-evening-reflection.md → .rag\extra-docs\constitution-aide-trial\2026-08-23-evening-reflection.md
[constitution-aide-trial] mirror docs\reports\2026-08-24-evening-reflection.md → .rag\extra-docs\constitution-aide-trial\2026-08-24-evening-reflection.md
[constitution-aide-trial] mirror docs\reports\2026-08-30-evening-reflection.md → .rag\extra-docs\constitution-aide-trial\2026-08-30-evening-reflection.md
[constitution-aide-trial] 18 files → .rag\extra-docs\constitution-aide-trial
[rag-ingest-path] npx mcp-local-rag ingest C:\Users\mhamada202408224\kintone-ai-lab\.rag\extra-docs\constitution-aide-trial
[rag-aide-smoke] OK rag-constitution-aide-trial.md ∋ 夕反省 明日やること 禁止
[rag-aide-smoke] OK ai-team-tool-routing-v2.md ∋ cio:tool:route
---
...
npm warn EBADENGINE }
Found 18 file(s) to ingest.
VectorStore: Opened existing table "chunks"
VectorStore initialized: C:\Users\mhamada202408224\kintone-ai-lab\.rag\lancedb
Parsed MD: C:\Users\mhamada202408224\kintone-ai-lab\.rag\extra-docs\constitution-aide-trial\00-rule-hierarchy.md (5392 characters)
Embedder: First use detected. Initializing model (downloading ~90MB, may take 1-2 minutes)...
Embedder: Setting cache directory to "C:\Users\mhamada202408224\kintone-ai-lab\.rag\models"
```
...
**総合**: 正常 31 / 異常 0 / 警告 0 / スキップ 1
**健全性スコア**: 100%（判定対象 31 項目・スキップ 1 は別枠）
...
- NVM v24 スタック（cron・MCP・朝 prep 正本）:
  ...
  - **NVM v24 整合**: ✅ 100%
- Disk (`~`): 206.7G used / 252.8G free on C: — ✅
  - npm cache: 2.6G / npx cache: 1.8G
- Memory: 11138/15894 MiB (70%) — ✅
- cron: ✅ morning:prep 登録済み

### 🛡 自己診断強化 (S9 + S12 wiring)

- **node_modules 完全性 (S9)**: ✅ node_modules 完全性 OK
- **MCP 死蔵検知 (S12)**: ✅ 0/22 active (22 exempt) (過去 7 日)
...
- `docs\plans\2026-08-29-jikkou-yosan-v2-master-g0-decisions.md` （更新: 2026-08-30 09:45）
- `docs\plans\2026-08-30-jikkou-yosan-v2-s0-existing-value-scan.md` （更新: 2026-08-30 08:21）
- `docs\plans\2026-07-19-jikkou-yosan-ver02-redesign-spec-draft.md` （更新: 2026-08-30 07:52）

**AI への指示例**:
```
「2026-08-29-jikkou-yosan-v2-master-g0-decisions.md の続きを進めて」
```
