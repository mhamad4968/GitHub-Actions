...
**§1-N 今日の結論（1行）**: 688 customize は **修正＝同一セッション内デプロイ必須**（浜田 2026-07-11 再確認）。憲法正式クローズは観測後。
...
- 依頼 compose で 688 WBGT 依頼ブロック → OK → 調査着手の流れが機能
...
| # | 失敗（事実） | 根本原因 |
|---|-------------|----------|
| F1 | WBGT 修正後、最初にデプロイせず目視依頼した | customize 修正完了＝デプロイまでがセットという運用を守らなかった |
...
- A1: kintone customize（688 含む）の **コード修正後は preflight→deploy まで同一ターンで完走**（浜田への「確認してください」より先にデプロイ）
...
|---|---|---|---|---|
| #R-688-DEPLOY-01 | R | `AGENTS.md` または 688 runbook に「customize 修正＝`deploy:688` 必須（目視前）」を 1 行追記 | 低 | ○ |
| #S-WBGT-RELOAD-01 | S | `workdays:heat-reference-gate` の再読込 assert を維持（本日追加済・継続） | 低 | 済 |
| #R-REQUEST-COMPOSE-01 | R | kintone 依頼は `cio:request:compose` → 確認 A → 着手を標準化（v0.1 運用） | 低 | 手動 |
| #D-OBS-CLOSE-01 | D | 観測期間（≈7/25）を checkpoint 先頭に維持し、早期クローズ宣言を禁止 | 低 | 済 |
