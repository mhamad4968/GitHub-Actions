...
| **A6** | 674 関連 **Git commit** | **本夜コミット**（`desktop.js`・674 scripts・正本ドキュ） |
...
| **O-5 クレジット監視**（handoff-log） | **2026-05-14 JST リセット日**。`npm run credit:status` を朝イチで確認 |
...
2. 674 残タスクがあれば **`npm run cio:preflight:674`** → **`deploy:674`** の順（GO 後）。
