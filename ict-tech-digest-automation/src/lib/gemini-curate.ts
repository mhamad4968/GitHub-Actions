...
/** 情報源の地域（v2.2・リンク先は国内サイトのみ） */
export const SOURCE_REGION_POLICY = `
【情報源の地域（リンク先は国内サイトのみ・全カテゴリ共通）】
- **記事 URL は国内サイトのみ採用可**（.jp / .go.jp、Qiita/Zenn/ITmedia/日経 xTECH/IPA/JPCERT/Impress 等）。**海外公式・海外メディア（Microsoft / MSRC / NVD / blogs.windows 等）は1件も選ばない**
- パッチ・CVE も **国内メディアの解説** または **国内公式（IPA/JPCERT 等）** がリンク先のものだけ採用可
- **カテゴリ「DX人材・IT資格・組織」**: 日本の IT 資格・リスキリング・DX 人材・組織・働き方・政府/業界団体の国内発表に限る
`;
