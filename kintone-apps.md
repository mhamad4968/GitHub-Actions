...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-focus-assist-composed-click`**（**入力支援**: `composedPath`＋**click 補助**＋所属の **record/DOM 併用の空欄判定**／種別 **DOM select 優先**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ §4.2.0b／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`fc3afadb-1915-4b72-a199-9ab0e1faa366`** / preview revision **`104`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
