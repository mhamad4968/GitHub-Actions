...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-shared-autogen-internal-disabled-fix`**（**共有自動生成** `record.set` 時 **内部メタ子（`pc_serial_no` 等）の disabled 一時解除**＋**`m365_master_record_id` 解除**／個人自動生成も同様／§4.2.2 mail/gb/sb 等）/ fileKey **`42d601e1-c163-4b4b-ab09-522c6d0c8dc0`** / preview revision **`125`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
