...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-input-assist-hai-iie-modal`**（**§4.2.0b**: **個人**＝利用者名・所属名・所属グループクリック→**はい／いいえ**→595／**共有・JR**＝共有PCのため**所属名・所属グループのみ**同様→680／**ヘッダ・直下ボタン**は確認省略／**フォーカスでは起動なし**。**`pc_status`=保管**: 種別横断で新規・編集ヘッダは **全フィールドリセットのみ**／**閲覧×保管**はカスタムバーなし／**非保管**は従来どおり種別別＋PC買替・印刷／**595** **`isPersonal595AssistEnabled674`**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`9982a7d2-2780-490e-82bd-b18e794b2442`** / preview revision **`113`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
