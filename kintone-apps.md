...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-input-assist-in-header-strip`**（**新規 create.show**: **`layout-gaia` 先頭**＋**`kintone.app.getHeaderMenuSpaceElement`** マウントフォールバック・ヘッダ種別 **`readAccountTypeLive674`**／**§4.2.0b**: **`#new-pc-ledger-buttons`** に **「📋 入力支援（595で検索）」**（個人・非保管）・共有・JR **「📋 所属候補を開く（680）」**（緑 **M365 自動生成**と併用）・**`m365_master_record_id` set 前 disabled 解除**／**JR** ヘッダ **「M365 を自動反映（JR）」**・**673** JR 非呼出／**`document` capture**・**所属ヘルプ**撤去／**`pc_status`=保管** ヘッダ最小／**595** **`isPersonal595AssistEnabled674`**／**JBIS GET** 等）/ fileKey **`ebea2a15-eb6e-458d-9350-7393c46716b5`** / preview revision **`121`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
