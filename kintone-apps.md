...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-personal-windowsname-validate`**（**個人 `windows_name`**: §4.2.2 どおり `logon+mail_acct`・**672 `logon_name` を jbm＋4桁（§4.3.2 新規発番）で検証**／共有 **内部メタ disabled 一時解除**／mail/gb/sb 等）/ fileKey **`a7365f2a-a573-42c2-802f-e5b947f2ea79`** / preview revision **`126`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
