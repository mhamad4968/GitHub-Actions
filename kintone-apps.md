...
| 2026-04-20 22:00 | **ops-guide v7 表示バグ修正 + M365ライセンス管理基盤構築**: ① **shell 上端 86px が永久に kintone ヘッダー直下に隠れていた表示バグ** (4/19 以前から潜在) を修正。`customize/ops-guide/desktop.js` の `injectShell` に `adjustShellOffset()` を追加し、shell の `getBoundingClientRect().top` を実測 → kintone ヘッダー高さより上にあれば `marginTop` で物理的に押し下げる (50ms / 300ms / 1000ms の 3 タイミング)。これで「📌 主要メニュー」「📊 データ品質ダッシュボード」が確実に viewport 内に表示されるようになり、4/19 v5 化以降ずっと「ガイドから消えた」と言われていた WindowsID 重複ダッシュ等のリンクが復活。② **M365管理台帳ビュー (627 view=13459663)** に WindowsID (logon_name) + 紐付けPC台数 (pc_link_count) を追加。③ **新ビュー「📧 PC台数順 (Office5台制限管理)」 (id=13459675)** を新設し、Microsoft 365 の「1 アカウント = 5 台インストール制限」を超過しているアカウント (5台以上紐付き) を即座に把握可能に。④ **スキーマ追加**: `pc_ledger_links` サブテーブル内に `pc_link_count_unit` (NUMBER, default=1, 範囲 0-1) + メインに `pc_link_count` (CALC, expression=`SUM(pc_link_count_unit)`, 単位「台」) を 2 段階デプロイで追加。⑤ **既存データ全件バックフィル**: 296 アカウント中 71 件 (サブ行を持つ全件) に対し `pc_link_count_unit=1` を一括設定 (data/snapshots/627-2026-04-20T22-07.json で事前スナップショット → 100% 成功 / 失敗 0 件)。⑥ **検出結果**: 5 台超過は 1 件のみ ($id=810「東京管理者」共有アカウント・**8 台**紐付け) → 入替対象として明日以降の管理対象に確定。⑦ ops-guide 黒帯に「⚠ PC台数順 (Office 5台制限管理)」リンク追加。デプロイ: 627 revision 144 / 668 revision 36 |
