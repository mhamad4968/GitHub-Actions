...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-input-assist-doc-delegate`**（**§4.2.0b**: **`document` capture** でフィールドクリック→**はい／いいえ**→595／680・確認 UI **z-index 2000000**・ヘッダ「社員名検索／所属候補」**削除**・直下 **入力支援（595で検索）／入力支援（所属候補）** も同フロー・共有は所属のみ）／**`pc_status`=保管**: 種別横断で新規・編集ヘッダは **全フィールドリセットのみ**／**閲覧×保管**はカスタムバーなし／**非保管**は従来どおり種別別＋PC買替・印刷／**595** **`isPersonal595AssistEnabled674`**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`49a7accd-e531-4ea2-bda2-0f8d398afeee`** / preview revision **`115`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
