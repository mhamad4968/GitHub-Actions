...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-shared-autogen-m365-disabled-fix`**（**§4.2.0b**: 共有・JR は **所属名の直前**に **M365 取込＋680** ボタン帯・個人は直下 **兄弟 DOM**・**種別／ステータス**は **record 優先**／**共有用自動生成**: **`m365_master_record_id` を `record.set` 前に disabled 解除**（入力内容エラー回避）／**JR**: ヘッダ **「M365 を自動反映（JR）」**・**PC名は自動生成しない**・**673** JR 時 **非呼出**／**`document` capture**→はい／いいえ→595／680・ヘッダ旧検索廃止・**所属ヘルプ `<details>` 撤去**／**`pc_status`=保管**: ヘッダ **全フィールドリセットのみ**／**閲覧×保管**はカスタムバーなし／**595** **`isPersonal595AssistEnabled674`**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`decc79e0-fdf3-4256-841c-8b0b596a181f`** / preview revision **`119`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
