...
| **PC台帳簡単ガイドライン**（事務向けクイックガイド・短文＋図中心・**一覧上段に Web 風リーダー**） | **681** | `customize/681/desktop.js` \| `npm run deploy:681` | **Space 21 / thread 23**・[スペース 21](https://jbis-kintone.cybozu.com/k/#/space/21)・[アプリ](https://jbis-kintone.cybozu.com/k/681/)。**2026-05-06**: REST `scripts/create-pc-ledger-quick-guide-app.mjs`（`npm run pc-ledger:quick-guide:create-app`）でアプリ作成。**表示名**は `pc-ledger:quick-guide:set-app-name`。**一覧**: `app.record.index.show` で **カード型リーダー**。**既定では kintone 一覧は非表示**・「一覧を表示（編集・追加）」で編集モードに切替（679 と同趣旨）。**2026-05-06（追記）**: MCP で **9 レコード**投入済み。フィールド: `sort_no`・`midashi`・`honbun`・`gazou_1`〜`3`。利用者向け文言に **アプリ番号や IT 用語を出さない**運用。**2026-05-06**: customize deploy **SUCCESS** / fileKey **`1da37163-dde8-43a6-8c73-f1763abdab9b`** / preview revision **`9`** / **BUILD=`2026-05-06-681-reader-hide-list-toggle`**。 |
