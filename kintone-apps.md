...
- **本番 live**（**2026-05-12** `npm run cio:preflight:678 -- --note "678実績Enter修正削除プリfill本番反映"` → `npm run deploy:678` **Deploy SUCCESS**）: fileKey **`f0902f20-8cc8-4a94-844e-d58f335cbe11`** / preview revision **`129`** / **`var BUILD`** = **`2026-05-12-678-enter-editdelete-prefill`**（リポ **`customize/678/desktop.js` HEAD** と一致）。**変更内容**: 実績モーダル **Enter で保存**（`TEXTAREA` は **Ctrl+Enter** のみ同効果）、既存支払一覧から **修正／削除**（サブテーブル行 **`id` 維持**で PUT）、固定費かつ支払種別 **月額／年額**で **金額初期値＝当該月 `month_budget`+`month_budget_revision`**（正のときのみ）。新規明細モーダルも **備考 `textarea` 以外は Enter で保存**。
