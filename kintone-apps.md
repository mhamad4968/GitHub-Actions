...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-dept-help-on-demand`**（**所属ヘルプ**: 既定は **閉じた `<details>`**・必要時のみ展開 §4.2.0b）／**一覧**: §4.8a＋**URL `query` 復元**／SKYSEA は計画後に再検討）／**PC名**必須・**個人 JBIS 重複**室長確認モーダル／閲覧=detail **PC買替・印刷のみ**／共有・JR **最小表示**＋所属モーダル（**680**）／**671** 手入力不可／固定IP NAS のみ任意／備考任意）/ fileKey **`0af67d36-d231-4a45-b837-3ca38b06055e`** / preview revision **`100`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
