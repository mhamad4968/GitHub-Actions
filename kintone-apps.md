...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23** / customize **BUILD=`2026-05-05-pc-ledger-jbis-dup-warn`**（**PC名＝管理番号・全種別保存必須**／**個人: JBIS+4桁が他レコードと重複時は赤バナー＋保存前に室長確認モーダル（はい/いいえ）**／閲覧=detail **PC買替・印刷のみ**／共有・JR **最小表示**＋所属モーダル（**680**）／**671 行番号は新規・編集で手入力不可**／固定IPはNASのみ任意／備考任意／モバイル想定なし）/ fileKey **`0cf0edb5-c118-4d8f-a5ff-a9db3315e824`** / preview revision **`97`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
