...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-input-assist-label-style`**（**新規 create.show**: **`layout-gaia` 先頭**＋**`kintone.app.getHeaderMenuSpaceElement`** マウントフォールバック・ヘッダ種別 **`readAccountTypeLive674`**／**§4.2.0b**: **`#new-pc-ledger-buttons`** に **「入力支援利用」**（個人→595／共有・JR→680・`aria-label` で区別・インディゴ系グラデ）・**`m365_master_record_id` set 前 disabled 解除**／**JR** ヘッダ **「M365 を自動反映（JR）」**・**673** JR 非呼出／**`document` capture**・**所属ヘルプ**撤去／**`pc_status`=保管** ヘッダ最小／**595** **`isPersonal595AssistEnabled674`**／**JBIS GET** 等）/ fileKey **`f73c2e00-2695-40ec-a0b9-353956aa666e`** / preview revision **`122`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
