...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-595-assist-spec-gate`**（**595 入力支援の正本**: **`isPersonal595AssistEnabled674`**＝§4.1a・§4.4・§4.2.0＝個人かつ **pc_status≠保管**（`readAccountTypeLive674`＋`readPcStatusLive674`）でモーダル・ボタン・利用者名候補・自動生成・保存前 user_name 検証・595 双方向リンクを統一／共有・JR は未入力所属フォーカスで **680**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ §4.2.0b／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`e6374d94-049d-4714-a79c-e31e041178a3`** / preview revision **`109`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
