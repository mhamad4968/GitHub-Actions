...
| **681（運用終了・旧 PC台帳簡単ガイドライン）** | **681**（**テナント上は削除済**） | `customize/681/desktop.js`（**リポに参照用で残置**・**deploy 対象外**） | **2026-05-06**: CEO 方針で **アプリ削除済**（`/k/681/` は無効）。担当者向け案内は **別手段を後日設計**。`scripts/*quick-guide*`・`npm run pc-ledger:quick-guide:*` は **当面存置**（再利用前提なし）。旧 deploy 連記は **git 履歴**および本行に集約。 |
