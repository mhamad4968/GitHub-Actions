...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-focus-assist-field-wire`**（**入力支援**: **各フィールド getFieldElement に pointerdown/click 直接配線**（AbortController で show/種別変更ごと付け替え）＋`composedPath`／種別 **select 多段シャドウ**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ §4.2.0b／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`7b31574e-8c41-4cf8-bf65-e5a6e8428dde`** / preview revision **`105`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
