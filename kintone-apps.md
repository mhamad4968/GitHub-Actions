...
| **新しいPC台帳 かんたん案内**（事務向けクイックガイド・短文＋図中心・標準アプリのみ） | **681** | （customize なし） | **Space 21 / thread 23**・[スペース 21](https://jbis-kintone.cybozu.com/k/#/space/21)・[アプリ](https://jbis-kintone.cybozu.com/k/681/)。**2026-05-06**: REST `scripts/create-pc-ledger-quick-guide-app.mjs`（`npm run pc-ledger:quick-guide:create-app`）で作成。フィールド: `sort_no`（表示順）・`midashi`（見出し）・`honbun`（本文）・`gazou_1`〜`gazou_3`（FILE・サムネ 250）。一覧は **表示順** 昇順推奨。利用者向け文言に **アプリ番号や IT 用語を出さない**運用。 |
