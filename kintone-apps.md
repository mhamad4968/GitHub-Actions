...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-personal-autogen-mail-gb-sb`**（**個人用自動生成** §4.2.2 **`mail_pw`・`gb_id`/`gb_pw`・`sb_id`/`sb_pw`**＋§4.3.1 **`pc_name`/`pc_serial_no`**＋**`windows_name`**／**共有** **`S-JBIS####-YYYYMM`**／**入力支援利用**／**`m365_master_record_id` disabled 解除**／**JR** M365 のみ 等）/ fileKey **`9136afb6-6fe8-4332-b1e5-34459ea3a554`** / preview revision **`124`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
