...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-personal-stored-header-min`**（**個人×保管の新規・編集**: ヘッダ **`#new-pc-ledger-buttons` は全フィールドリセットのみ**（PC買替・印刷も非表示・§4.1a 最小 UI）／**595 正本** **`isPersonal595AssistEnabled674`**／共有・JR は未入力所属で **680**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ §4.2.0b／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`659b0f75-1710-4afc-bd8a-f748a7e7efe0`** / preview revision **`110`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
