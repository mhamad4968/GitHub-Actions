...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-jr-m365-button-no-pc-name-autogen`**（**JR**: ヘッダ **「M365 を自動反映（JR）」**・**PC名は自動生成しない**（§4.3.1）・`runSharedAutoGen` は **673 非呼出**／**§4.2.0b**: **`document` capture**→はい／いいえ→595／680・直下 **入力支援**・ヘッダ旧検索ボタン廃止・**所属ヘルプ `<details>`（入れ方・コピー一覧）撤去**／**`pc_status`=保管**: 種別横断で新規・編集ヘッダは **全フィールドリセットのみ**／**閲覧×保管**はカスタムバーなし／**非保管**は従来どおり種別別＋PC買替・印刷／**595** **`isPersonal595AssistEnabled674`**／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`8608c454-aeb2-409d-b116-2f7349340310`** / preview revision **`117`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
