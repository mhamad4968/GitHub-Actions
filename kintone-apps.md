...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-windowsname-bracket-noplus`**（**個人 `windows_name`**: 仕様 **`docs/plans/2026-04-21-new-pc-ledger-spec.md` §4.2.2【正本】**・**`jbm####[mail@前]`**（`+` なし・例 `jbm0065[y-sasaki]`）・672 **jbm4桁**検証／共有 **内部メタ disabled 一時解除**／mail/gb/sb 等）/ fileKey **`51b8d33f-b4e0-468e-9c78-760876e87eb9`** / preview revision **`127`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
