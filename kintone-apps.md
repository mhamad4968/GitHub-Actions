...
| **PC台帳簡単ガイドライン**（事務向けクイックガイド・短文＋図中心・**一覧上段に Web 風リーダー**） | **681** | `customize/681/desktop.js` \| `npm run deploy:681` | **Space 21 / thread 23**・[スペース 21](https://jbis-kintone.cybozu.com/k/#/space/21)・[アプリ](https://jbis-kintone.cybozu.com/k/681/)。**2026-05-06**: REST `scripts/create-pc-ledger-quick-guide-app.mjs`（`npm run pc-ledger:quick-guide:create-app`）でアプリ作成。**表示名**は `pc-ledger:quick-guide:set-app-name`。**一覧**: `app.record.index.show` で **カード型リーダー**。**既定では kintone 一覧は非表示**・「一覧を表示（編集・追加）」で編集モードに切替（679 と同趣旨）。**2026-05-06（追記）**: 初期 **9 章**（メタ章2件は **2026-05-06** `prune-meta` で削除・**7 章**）。フィールド: `sort_no`・`midashi`・`honbun`・`gazou_1`〜`3`。利用者向け文言に **アプリ番号や IT 用語を出さない**運用。**2026-05-06**: customize deploy **SUCCESS** / fileKey **`1da37163-dde8-43a6-8c73-f1763abdab9b`** / preview revision **`9`** / **BUILD=`2026-05-06-681-reader-hide-list-toggle`**。**2026-05-06（続）**: **横スクロール目次**（`#pcqg-{レコード番号}` で章へジャンプ）・**広い画面はカード多列グリッド**・**イラスト欄が空の章は内蔵 SVG アイコン**。**deploy SUCCESS** / fileKey **`de67464b-78fd-418b-973b-0fbf9ce2b7e8`** / preview revision **`11`** / **BUILD=`2026-05-06-681-toc-grid-inline-svg`**。**2026-05-06（続・自部署向け）**: 見出しに「このガイドの読み方」「いつから使う」を含む章は **リーダー非表示**＋当時は `prune-meta:apply` で削除済（**現行** `prune-meta` は **既定 dry-run のみ**／本削除は **`prune-meta:apply`**）。**deploy SUCCESS** / fileKey **`2180a413-e074-4692-8e98-564f869e3322`** / preview revision **`12`** / **BUILD=`2026-05-06-681-prune-meta-chapters`**。**2026-05-06（続）**: **「検索の使い方」**章は **`npm run pc-ledger:quick-guide:upsert-search`**（`scripts/pc-ledger-quick-guide-upsert-search-chapter.mjs`）。**見出し別 SVG サムネ**を各章で常時表示し **`gazou_1`〜`3`** と併用可。**deploy SUCCESS** / fileKey **`e7a45078-816e-4931-be8a-a2210adbe5aa`** / preview revision **`13`** / **BUILD=`2026-05-06-681-search-chapter-themed-thumbs`**。**再デプロイ** fileKey **`ad5eda2a-4c89-40a2-a279-25f880287513`** / preview revision **`14`** / **BUILD=`2026-05-06-681-search-themed-thumbs-subnote`**（冒頭案内に検索・章絵の説明）。**2026-05-06（続）**: **リスト一覧／社員マスタ／異動・退職／PC買替／PC保管**の5章は **`npm run pc-ledger:quick-guide:upsert-five-topics`**。表示順 **1,2,3,4,5,6,7,8,9** で既存章と挟み込み。**deploy SUCCESS** / fileKey **`f2420f33-fb7c-45a4-8cac-e26d1f1473b8`** / preview revision **`15`** / **BUILD=`2026-05-06-681-five-topics-thumbs`**。**再デプロイ**（**見出し横の「表示順」バッジ非表示**）fileKey **`38438b02-e3f3-465b-ade4-1aaafbb8e1ba`** / preview revision **`16`** / **BUILD=`2026-05-06-681-hide-sort-badge`**。**2026-05-06（続・運用ゲート）**: **`npm run app:fields 681`＝API 14 件**（業務: `sort_no`・`midashi`・`honbun`・`gazou_1`〜`3` ＋システム）。**レコード DELETE**: `docs/kintone-destructive-operations.md`・**`pc-ledger:quick-guide:prune-meta` は既定 dry-run のみ**・本削除は **`prune-meta:apply`**（チャットに dry-run 貼付＋「この一覧で削除 GO」後）。Cursor: **`.cursor/rules/kintone-destructive-rest-guard.mdc`**。**計画**: 画像一括 `docs/plans/2026-05-06-681-bulk-image-upload.md`。 |
...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-shared-autogen-set-fix`**（**個人 `windows_name`**: §4.2.2【正本】・**`jbm####[mail@前]`**（`+` なし）／**共有自動生成**: REST 後 **`api.get()` 直後のみ `set`**・**`withWritableInternalMeta674` 強化**・672 **jbm4桁**／mail/gb/sb 等）/ fileKey **`6589b3b7-1b3a-4082-8e2e-09c95008700b`** / preview revision **`128`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
