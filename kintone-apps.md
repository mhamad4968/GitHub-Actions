...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-595-on-demand-only`**（**入力支援**: 共有・JR のみ未入力所属欄フォーカスで **680** 自動／**個人の 595**はフォーカス自動なし・**明示ボタンのみ**／field-wire＋種別 **DOM 優先→record**／wire **0/400/800/1200/2000ms**＋初回 **2.6s**／**個人（非保管）**は **user_name / dept_name / group_name 直下「595で氏名・所属を検索」**＋ヘッダ「社員名を検索（595）」／**JBIS GET**: `in("個人")`・`like` **`%` 禁止**）／**所属ヘルプ** `<details>` 既定閉じ §4.2.0b／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`18c7d9cd-f01c-44b1-bc04-3bd4611910b1`** / preview revision **`108`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
