...
- **部署予実（予算・実績・修正）**: 仕様は `templates/yojitsu-budget-lite/SPEC.md`（**§6c**＝`支払内訳` サブテーブル等のたたき台）。**入力**と **ダッシュ**の **2 アプリ**＋任意マスタ（`SPEC.md` §6b）。一覧表は **アプリごと 1 行**（下表参照）。`shin-format-excel-layout.md`・`excel-column-draft-2026-04-28.md`・`yojitsu-spec-session-checklist.md`。
...
| **部署予実・入力**（明細のたたき台・`支払内訳`・月次サブテーブル） | **675** | （未） | [k/675](https://jbis-kintone.cybozu.com/k/675/) ・2026-04-28 MCP 作成・deploy **SUCCESS**・`npm run app:fields 675` で正本更新。フィールド: `kosyu_meisho`…`biko`・`shiharai_lines`・`monthly_block`（**消費率 CALC**は API 式エラーのため未追加→要追補）。`templates/yojitsu-budget-lite/SPEC.md` §6c への完全追従は未 |
| **部署予実・ダッシュ**（集計用のたたき台） | **676** | （未） | [k/676](https://jbis-kintone.cybozu.com/k/676/) ・2026-04-28 MCP 作成・`dash_readme` のみ・集計 UI は今後 |
