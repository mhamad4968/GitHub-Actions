...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-06-pc-ledger-personal-autogen-pcname-windowsfmt`**（**個人用自動生成**: §4.3.1 **`pc_name`/`pc_serial_no`**・**`windows_name`=`jbm####+mail_acct`**／**共有**: **`S-JBIS####-YYYYMM`**（空欄時）／**新規 create.show** マウントフォールバック・**§4.2.0b** **「入力支援利用」**・**`m365_master_record_id` set 前 disabled 解除**／**JR** M365 のみ 等）/ fileKey **`d0dca7e6-a2bc-4f5a-84f0-3ed22710cc49`** / preview revision **`123`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
