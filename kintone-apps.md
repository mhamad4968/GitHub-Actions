...
| **新・PC台帳 所属候補マスタ**（674 共有・JR の「所属候補から入力」モーダル。未作成時は674 JS の埋め込み一覧にフォールバック） | **675** | （customize なし） | **手動でアプリ作成**（Space 21 想定）→ フィールド **`dept_name`**（所属名）**`group_name`**（所属グループコード）を **1 レコード＝1 行**。任意 **`sort_no`**（数値・並び）。674 の `APP_DEPT_MASTER_674` と **アプリ番号を一致**させる。シード例: `npm run pc-ledger:675:seed-records`（レコード投入のみ・フィールド先行必須） |
...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23** / customize **BUILD=`2026-05-05-pc-ledger-shared-jr-visibility-dept-modal`**（閲覧=detail **PC買替・印刷のみ**／共有・JR **最小フィールド表示**＋**所属候補モーダル**＋保存前必須・**675** 未作成時は埋め込み候補／個人＋保管は個人と同表示・VPN は個人のみ・NAS/その他は全表示）/ fileKey **`1389cdbf-ccee-4d6a-92f3-c0b3ba4a1ba1`** / preview revision **`89`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
