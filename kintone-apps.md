...
| **PC台帳簡単ガイドライン**（事務向けクイックガイド・短文＋図中心・**一覧上段に Web 風リーダー**） | **681** | `customize/681/desktop.js` \| `npm run deploy:681` | **Space 21 / thread 23**・[スペース 21](https://jbis-kintone.cybozu.com/k/#/space/21)・[アプリ](https://jbis-kintone.cybozu.com/k/681/)。**2026-05-06**: REST `scripts/create-pc-ledger-quick-guide-app.mjs`（`npm run pc-ledger:quick-guide:create-app`）でアプリ作成。**表示名**は `pc-ledger:quick-guide:set-app-name`。**一覧**: `app.record.index.show` でレコードを取得し **カード型で表示**（下段は従来の一覧のまま編集可）。**2026-05-06（追記）**: MCP で **9 レコード**投入済み。フィールド: `sort_no`・`midashi`・`honbun`・`gazou_1`〜`3`。利用者向け文言に **アプリ番号や IT 用語を出さない**運用。**2026-05-06**: customize deploy **SUCCESS** / fileKey **`5549c95d-3e0c-4e13-8399-008e58288fbf`** / preview revision **`7`** / **BUILD=`2026-05-06-681-web-reader-copyfix`**。 |
