...
| （Cursor）**`mcp-frontend-shadcn-chrome.mdc`** | **alwaysApply: true** — **Shadcn UI MCP 優先**（コンポーネント新規・改修）／**Chrome DevTools MCP** で FE を事実確認／不明時は関連 MCP で先に調査／**`mcp:sync-cursor-windows`**（WSL 正本変更後）。**チェックシート報告**は **`SESSION-BOOTSTRAP-CHECKLIST.md` フェーズ 7 の 4a** |
