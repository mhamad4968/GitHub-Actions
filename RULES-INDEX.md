...
| ルール | 役割 |
|---|---|
| §1 / §1-2 / §1-2-2 | 役割定義 + **モデル前提: Cursor は Claude Opus 4.7 のみ**（別モデルへの常時切替・レビュー用サブエージェント禁止 / 例外は §1-2）+ **API 制限到達時の自動フォールバック禁止**（§1-2-2 / 2026-04-26 / N-3 / `Switched to Composer 2 after reaching API limit.` 検知時は即時中断 → 浜田 GO 待ち / TSB-018 連動）|
| §39 | 発言前の日時確認（絶対遵守）|
| §34 | 人間尊重プロトコル |
| §42 | セッション冒頭の過去ログ確認義務 |
...
| ルール | 役割 |
|---|---|
| §1 | 役割定義（§1-2 = Opus 4.7 単一モデル固定 / §1-2-2 = API 制限到達時の自動フォールバック禁止）|
