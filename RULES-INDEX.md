...
| ルール | 役割 |
|---|---|
| §1 / §1-2 / §1-2-2 / §1-2-3 / §1-2-3-1 / §1-2-4 | 役割定義 + **モデル前提: Cursor は Claude Opus 4.7 のみ**（別モデルへの常時切替・レビュー用サブエージェント禁止 / 例外は §1-2）+ **API 制限到達時の自動フォールバック禁止**（§1-2-2 / 2026-04-26 / N-3 / N-4 で 4 択 A-D 提示の枠組み + §1-2-2-1 Cursor IDE 必須設定 = **Q1 で 4 → 8 項目に拡張 / Browser Protection ON + MCP Tools Protection ON / TSB-019 連動**）+ **Opus 内モデル使い分け**（§1-2-3 / N-5 / 既定 Extra High / Max Thinking は §47-A 100% 証明・設計判断・複雑バグ修正のみ）+ **AI 自己宣言義務**（§1-2-3-1 / P5-5 / タスク冒頭で `[§1-2-3 ティア判定: Extra High/Max Thinking]` を 1 行明示 = 形骸化対策 / F-13 教訓）+ **クレジット予算管理 改定**（§1-2-4 / P5-5 / 月予算 $200+$1000 引上げ / 3 系統 (Total/API/On-Demand) / 70-80-85-95% 4 段階自発警告 / Spending スクショ抽出 / 朝報 §0 統合 / TSB-018/TSB-021 連動）|
| §52 / §52-3 / §52-8 / §52-8-1 | **RACI Tier A/B 自律レベル**（§52-3 6 問自己診断）+ **§52-8 高リスク shell 暴走防止**（Q1 / TSB-019 連動 / rm -rf・git push --force・npm install (新規)・chmod -R・sudo・.env 編集 等は事前報告 → 浜田 GO 待ち）+ **§52-8-1 物理 block 層**（P5-1/R1 / TSB-019 構造的根本対策 / `~/.cursor/hooks/dangerous-shell-blocker.sh` で OS レベル deny / 三層防御確立）|
| §39 | 発言前の日時確認（絶対遵守）|
| §34 | 人間尊重プロトコル |
| §42 | セッション冒頭の過去ログ確認義務 |
...
| ルール | 役割 |
|---|---|
| §1 | 役割定義（§1-2 = Opus 4.7 単一モデル固定 / §1-2-2 = API 制限到達時の自動フォールバック禁止 + §1-2-2-1 Cursor IDE 必須設定 / §1-2-3 = Opus 内 Max Thinking vs Extra High 使い分け + §1-2-3-1 AI 自己宣言義務 / §1-2-4 = クレジット予算管理 月 $200+$1000 + 3 系統 70-80-85-95% 警告）|
