...
### §50-3 CTO運用規定（外部推理 MCP・コスト最適化）（2026-04-28 制定 / 浜田指示）

**目的**: チャット上の Opus（本体）のトークンと課金を抑えつつ開発速度を上げるため、**Kimi / DeepSeek / OpenRouter** 等の MCP を **計画・可視化・検収**つきで使う。本条は **§50 想起儀式**・**§1-2-3 最適モデル原則**を補完する。**矛盾する場合は §1-2・§35-1・§56-1a を優先**する。
...
- MCP が当該セッションから **呼べない・切断されている**と判断した場合、次を **プランB**（**ブラウザ版 Kimi 等で実行できるプロンプトをチャットに生成して提示**し、浜田の手を止めない）の候補とする:
  - **当ターンの自分の出力見込みがおおよそ 1,000 トークンを超える**作業
  - **複数ファイルにまたがる大規模修正**
...
- **新規タスクの着手 1 ターン目**に、**どの MCP（または Cursor 標準ツール）をどの順序で使うか**を、**箇条書きまたは短い表**でチャットに必ず示す（**隠し思考のみ**としない）。
- 浜田が **CTO 判断を追える**ことを最優先とする。

#### §50-3-3 安価 MCP 試行のループ上限

- **格安ルート**（DeepSeek 等の外部 MCP）による試行錯誤は、**回数で最大 3 回**、または **経過時間で累計 5 分以内**の **いずれか早く到達した時点**を上限とする。
- 上限超過後も未解決なら、**自律的に Opus に切替**（§1-2-3 に従い Extra High / Max Thinking を選択）し、**最短距離で収束**させる（安価ルートのワニ口に時間だけ投入しない）。

#### §50-3-4 MCP 送信用データのサニタイズ（一方向・現実運用）
...
#### §50-3-5 MCP 成果物の憲法検収と証跡

- 外部 MCP の出力を取り込む前に、**該当する lint / verify / §9 チェックリスト** 等で憲法・品質に適合することを確認する。
- チャットで **「憲法適合済み」**と書く場合は、**実際に実行した検収コマンド名**（例: `npm run smoke:quiet`、`npm run verify:constitution-handoff`）を **同一ターンで併記**する（**ラベルのみ**禁止）。

#### §50-3 関連

- `docs/plans/2026-gw-improvement-backlog.md`（GW 週のセキュリティ・MCP 整理メモ）
...
- 改訂日: 2026-04-28（[FEAT] v23.20: 第14章 **§50-3 CTO運用規定** 新設。浜田指示（外部推理 MCP・コスト最適化）。プランB閾値（おおよそ 1,000 トークン超の見込み・複数ファイル大規模 / 迷いは §41）/ 新規タスク着手 1 ターン目のパイプライン可視化 / 安価 MCP 試行上限（最大 3 回または累計 5 分の早い方）→ Opus 切替 / MCP 送信の一方向サニタイズ・グレーは要約のみ / 「憲法適合済み」は検収コマンド併記義務。§50-2 関連の「将来 §50-3」を本条参照に更新。RULES-INDEX.md 同期。）
