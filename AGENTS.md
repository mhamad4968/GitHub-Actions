...
- 改訂日: 2026-04-28（[FEAT] v23.21: 第14章 **§50-3 CTO運用規定** 新設＝浜田承認版。PlanB 閾値（**次の 1 回の出力のみ**／~1,000 トークンまたは日本語 ~1,500〜2,000 文字・複数ファイル大規模／迷いは §41）/ パイプライン可視化＋**§51 航海図と実行の分離**（1 ターン 1 コール維持）/ 安価 MCP 試行上限（3 回または累計 5 分）→ Opus 切替 / MCP 送信の一方向サニタイズ / 「憲法適合済み」は検収コマンド併記。`.cursorrules` に憲法・§50-3 優先の 1 行を追加。RULES-INDEX.md 同期。）
