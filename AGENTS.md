...
> **2026 年 4 月版 — 運用基盤完全統合済み**
>
> Phase 1（632 Idempotency・エビデンス）、Phase 2（631 メタデータ・例外枠・品質フラグ）、Phase 3（ランブック・統合テスト）を経て、4 つの防衛線（§7〜§10）が確立された。本憲法の全ルールは恒久的に適用される。変更には設計レビューと本ファイルの改訂を要する。
>
> 最終アーキテクチャ: [`docs/final-architecture.md`](docs/final-architecture.md)
