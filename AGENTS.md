...
- Tier A (必読推奨): §52 Tier A/B / §53 第二意見 / §53-7 Synthesis / §54-1 BREAKING / §54-3 廃止経緯
- Tier B (時間あれば): その他
...
##### §42-2-7 セーフモード (Sonnet 反 4 シナリオ C 反映)

- AGENTS.md Read 失敗 (権限 / ファイル消失) → 緊急 Tier B 投入 + 浜田相談 (R10 §52-2 連動)
- `.session-state/agents-md-hash.txt` 不在 (初回起動 / リカバリー後) → ハッシュ比較スキップ + 全文 Read のみ実施 + 「初回扱い」とチャット出力
- RAG MCP 不調 → §42-2-4 スキップ + 「RAG 不調 / Tier S 抽出後日」とチャット出力
