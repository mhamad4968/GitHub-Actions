...
**背景**: 本日制定ルール 9 件 (R10/R11/§53-7/§53-7-G 1-6/§54/§54-3 廃止/§54-4/§54-5) を新セッション AI が忘れたら全て無意味 → 「忘却 / 退行予防」が最優先課題。メイン AI 原案「唱和 + Sonnet 検閲 + クリティカル重み付け」 → Sonnet 反定立 21:28 で「唱和 = 記憶を記憶で検証する自己循環 / Sonnet 検閲 = 同一ファミリー馴れ合い (§53-7-G-1 違反) / §47-B-2 ルール疲労違反」5 件致命的指摘 → 「**ファイル直読方式 (No-Recitation Read-First)**」転換で合導出 → 浜田 21:30 A 案 GO。
...
1. **AGENTS.md 全文 Read** (`Read /home/mhamada202408224/kintone-ai-lab/AGENTS.md` / 全行)
   - 記憶ベースの「唱和」は廃止 (Sonnet 反 1: 自己循環論理排除)
   - ファイル直読 = 偽記憶混入ゼロ
...
- Tier S (絶対遵守 / 起動時必読): §52-3 Q1 / Q6 / §53-7-G-1 / §54-2-1 / §44
- Tier A (必読推奨): §52 Tier A/B / §53 第二意見 / §53-7 Synthesis / §54-1 BREAKING / §54-3 廃止経緯 / **§55 異常時セーフモード** / **§56 RACI**
- Tier B (時間あれば): その他

##### §42-2-6 Sonnet 検閲廃止 + 異ファミリー検閲オプション

- Sonnet 検閲は **廃止** (Sonnet 反 2 反映 / 同一 AI ファミリー馴れ合い回避 = §53-7-G-1 完全遵守)
- 代わりに **R11 §53-3 経路 B (外部 AI / Codex/Gemini)** で異ファミリー検閲を **将来オプション** (§54-5 月次審査と統合 / 5/1 R22 月次レビュー後検討)
- 起動時の Sonnet 起動は **不要** (ハッシュ比較 + RAG クエリで完結)
...
- **RAG MCP 不調** → §42-2-4 スキップ + 「RAG 不調 / Tier S 抽出後日」出力（**RAG 単独では §55 発動しない** / G-6 可用性）
...
##### §42-2-9 Sonnet 反定立 5 件指摘解消

| Sonnet 指摘 | 解消 |
|---|---|
| 1. 唱和 = 記憶依存自己循環 | ✅ 唱和全廃 / ファイル直読のみ |
| 2. Sonnet 検閲 = 同一ファミリー馴れ合い | ✅ 検閲廃止 / 外部 AI 検閲オプション (将来) |
...
- **§53-7-G-5 (2026-04-24 19:55 制定 / 浜田指示)** との完全整合: §51 = 1 ターン 1 ツール call (AI 操作レベル) / §53-7-G-5 = 実行直列 + シングル・エージェント・コントロール + エラー回避最優先 (Synthesis Logic 文脈での再強化)
...
- 改訂日: 2026-04-24（[FEAT] v19: 第18章 自己統治能力 §54 R12 新設。§54-1 意味論的バージョニング (BREAKING/FEAT/FIX ラベル必須 + 3 質問判定フローチャート) + §54-2 Negative Log (棄却案永続化 + Sonnet 書き込み権限 = 自己申告バイアス排除) 浜田 20:13 提案 + Sonnet 5 件レビュー全反映）
- 改訂日: 2026-04-24（[FEAT] v20: 第19章 §55 R13 異常時セーフモード。浜田 #2 GO / Tier A 縮小 + §52-6 合成但書 + G-6 読取継続 + 解除は手動 health-check 必須（cron のみ禁止）/ §42-2-7 を §55 へ委譲）
- 改訂日: 2026-04-25（[FEAT] v21: 第20章 §56 R14 責任の所在 (RACI) + §53-7-H 第二意見経路不能時 + 朝ブリーフィング §55/autonomy スキャン連動 / 浜田 D1–P1 一括承認バッチ）
...
**背景**: 2026-04-24 20:13 浜田提案「明日からの激しい構築フェーズで『AI が良かれと思って勝手に行う微調整』が後から巨大な負債になるリスクを完全封じ込めたい」 → 「3 つの深層ルール」(§54-1 意味論的バージョニング + §54-2 Negative Log + 第 3 ルール = 後日提案予定) を順次制定。本日 20:18 浜田 GO + Sonnet 厳格レビュー (5 件改善提案) を全件反映して §54-1 / §54-2 を完成させる。
...
##### §54-1-1 BREAKING 判定フローチャート (Sonnet 20:21 レビュー反映 / AI 主観揺れ防止)
...
#### §54-2 不作為の記録 (Negative Log) - Sonnet 書き込み権限必須

**目的**: §53-7 検証付 Synthesis で棄却された案を後日復元可能化 / AI の忘却防止 / 数日後「あの案が良かったかも」発見可能 / **AI の自己都合隠蔽防止**

##### §54-2-1 致命的盲点への対策 (Sonnet 20:21 指摘 / Sonnet 書き込み権限必須)

**盲点**: Negative Log を書くのが AI 自身だと、AI は「自分に都合の悪い棄却案」を記録しない選択が可能 = §53-7-G-1 馴れ合い厳禁の精神に真逆 = AI が不利な証拠を隠蔽できる構造的欠陥。

**対策 (必須)**: Synthesis で「反 (Sonnet 反定立)」が発生したら、**Sonnet 自身が直接 Archives に書き込む** (メイン AI を経由しない):
- Sonnet 反定立 Task tool 完了直後に Sonnet が `docs/archives/synthesis-graveyard/<日付>/<トピック>.md` に追記
- メイン AI は Sonnet 書込内容を編集禁止 (read-only)
- Sonnet 書込ファイルの commit 権限はメイン AI にあるが、内容改変は §47 違反扱い

##### §54-2-2 記録対象

- §53-7 検証付 Synthesis で「反 (Sonnet 反定立)」として却下された主張 (全件記録 / 選択不可)
- 「合 (Synthesis 試案)」に統合されなかった元案の特定要素
- 採用されなかった代替案 (浜田・AI どちらが提案者でも記録対象)
- BREAKING 級の議論で棄却された案 (永続保存対象 = `permanent/`)

##### §54-2-3 ディレクトリ構造 (Sonnet 20:21 蓄積膨張対策反映)
...
**日付**: 2026-04-25
**Synthesis 元 commit**: <ハッシュ>
**棄却された案 (反 / Sonnet 反定立)**:
[Sonnet が直接書き込み / メイン AI 編集不可]

**棄却理由**:
[Sonnet が直接書き込み]

**元提案者**: メイン AI (Opus) / Sonnet / 浜田 のいずれか

**合 (採用された Synthesis)**: <commit ハッシュ + 概要>

**振り返り想起 trigger**: <RAG 検索クエリの例>
```

##### §54-2-5 「振り返る人」不在問題への対策 (Sonnet 20:21 指摘 / 能動的サーフェシング)
...
- **§44 evening-reflect cron に月次レビュータスク追加** (S20 拡張 / 4/27 適用予定):
  ...
  - 結果を翌月初の Synthesis 冒頭に提示
- **Synthesis 開始時フック**: 今回トピックに関連する棄却案を RAG で自動検索し、反定立候補として提示 (浜田が振り返らなくても AI が引き出す)
- 「浜田が振り返らない場合」を前提にした設計でなければ §54-2 は形骸化確定 (Sonnet 警告反映)

##### §54-2-6 適用範囲

- §53-7 検証付 Synthesis 全件 (棄却案ゼロ件でもログに「該当なし」を記録)
- 浜田・AI どちらの提案でも記録対象
...
- Sonnet 反定立 (Cursor Ultra 定額誤解) は正しかったが、浜田の「自律優先 = cost 気にしない」も汲み取れなかった
...
#### §54-5 Weekly Self-Critique with External Audit (週次自己批判 + 外部 AI 月次審査 / 候補 5 / 2026-04-24 21:18 制定)

**目的**: AI 自身の「失敗を矮小化する傾向」を構造的に防ぐ / §53-7-G-1 馴れ合い厳禁の自己適用 / メタ認識の強化 / 同一 AI ファミリー内自己審判の閉ループ断ち切り

**背景**: メイン AI 原案「Daily Self-Critique + Sonnet 検証」 → Sonnet 反定立 21:15 で「同一 AI ファミリー内自己審判 = 馴れ合い構造化 = §53-7-G-1 自己矛盾」+ 4 件他指摘 → 「日次 → 週次降格 + Sonnet 検証廃止 + 外部 AI 月次審査 (R11 §53-3 経路 B)」合導出 → 浜田 A 案 GO 21:18。
...
4. **当週の Sonnet 反定立採用率** (低すぎる = 馴れ合いシグナル / 高すぎる = メイン AI 提案質低下シグナル)
5. **翌週の改善目標** 3 件 (具体的アクション / 測定指標明示)

##### §54-5-2 外部 AI 月次審査 (毎月 1 日 21:00 cron / R11 §53-3 経路 B 連動)

毎月 1 日 21:00 cron で外部 CLI (Codex / Gemini) に当月の自己批判ログを審査依頼:
- 入力: 当月 4 週分の §54-5-1 自己批判レポート (`docs/reports/<月>-self-critique-monthly.md`)
- プロンプト: 「メイン AI (Claude Opus) の当月自己批判は誠実か / 矮小化していないか / 馴れ合い構造あるか / 改善目標は妥当か」
- 出力: `docs/reports/<月>-external-audit.md` に外部 AI 評価結果 (浜田が翌月初に確認)
- **同一 AI ファミリー閉ループ断ち切り** (Sonnet 申し送り反映)

##### §54-5-3 浜田任意採点 (強制せず)

浜田が週次レポート / 月次外部審査結果を読んだ際に、**任意で**「OK / 要見直し / 緊急改訂シグナル」のチェック記入可:
- 浜田時間制約 = 任意 (浜田負担最小化)
- チェック記入なくても §54-5 は機能 (AI 自己批判 + 外部 AI 月次審査が主軸)
- 浜田採点 = 抜き取り検査の効果 (Sonnet 案反映 / 馴れ合い破壊力)

##### §54-5-4 §54-2 高次元進化ログとの分担

| ログ種別 | 内容 | 配置 |
|---|---|---|
| §54-2 Negative Log | Synthesis で**棄却された案** (反定立 / 元案など) の永続記録 | `docs/archives/synthesis-graveyard/` |
| §54-5 Self-Critique | AI 自身の**判断ミス / 矮小化傾向 / 改善目標** の自己批判 | `docs/reports/<月>-self-critique-monthly.md` |

**境界**: 棄却 = §54-2 / 自己反省 = §54-5 (重複なし)

##### §54-5-5 ログ仕様事前確定 (Sonnet 申し送り反映 / 4/27 cron 適用前提条件)

`logs/autonomy-decisions.log` の JSON Lines 仕様を 4/27 朝 cron 適用前に確定:
- 必須フィールド (本日確定): `time / operation / tier / reason / q1-q6 / second_opinion / label`
...
シグナル発火時 = R11 §53-7 検証付 Synthesis で R10/R11/§54 全体見直し (浜田裁定必須)。

##### §54-5-7 Sonnet 反定立 5 件指摘の解消

| Sonnet 指摘 | 解消 |
|---|---|
| 1. 既存 §44/§54-2 重複 | ✅ §54-2 = 棄却 / §54-5 = 自己反省で分担明示 (§54-5-4) |
| 2. 同一 AI ファミリー馴れ合い | ✅ Sonnet 検証廃止 + 外部 AI (R11 §53-3 経路 B) 月次審査で閉ループ断ち切り |
| 3. 既存ルール衝突 / 二重アラート | ✅ R10 §52-6 monitor 強化として統合 (§54-5-6 / 重複なし) |
| 4. 4/25 朝破綻 3 件 | ✅ 日次廃止 (連続 3 日誤発火消失) + ログ仕様確定 (空振り回避) + §44 内拡張 (cron 競合解消) |
| 5. 代替案 (浜田週次 + 外部 AI) | ✅ 全面採用 |
...
**背景**: ツール連続失敗・観測基盤欠損・憲法ファイル不能読取など、**判断材料が壊れた状態**では Tier A 自律が逆に事故率を上げる。一方で §53-7-G-6 の通り **調査停止によるフリーズ**も最大リスク。本節は両立のため「**副作用は締め、読取と診断は止めない**」を機械的に強制する。

#### §55-1 目的

1. **被害半径の縮小**: 不確実な環境で kintone / deploy / mcp.json / 強 push 等を止める
2. **可用性の維持**: Read / 診断 script / 記録 / §54-4 snapshot は継続し、浜田判断を止めない
3. **R10/R11 との整合**: セーフモード中は §52-1 の通常 Tier A 例示に**上書き**（縮小優先）。§52-6 emergency は §55-4 但書で限定的に維持
...
**§52-6 emergency との合成（§53-7-G-1 馴れ合い禁止の範囲で明示）**:
- 「待つと被害拡大」が **1-2 文で具体的に述べられる**場合のみ、上記禁止の例外として Tier A 実行可
- 判断ログ必須フィールド: `safe_mode_emergency_override:true`, `harm_if_wait:"<具体>"`
- 濫用検知: §54-5 週次自己批判 + 月次外部審査の対象。**連続 2 セッションで emergency override** なら次回セッション開始時に浜田へ必ず明示報告
...
#### §55-6 §53-7-G-6（可用性最優先）との関係

- セーフモード中も **第二意見タイムアウト → ダウングレード**は適用。ただし **ダウングレード＝副作用実行許可ではない**（§55-4 の禁止は維持）。調査・単一診断・報告のみ継続

#### §55-7 制定時メモ（外部第二意見欠損）

2026-04-24 制定時、R11 §53 経路 A (Sonnet Task) が **API 上限で未実行**。反定立は **過去 Synthesis で反復されたパターン**（曖昧トリガー排除、§52-6 衝突、解除の cron 依存排除）をメイン AI が内省して条文化。**フォロー手順の一般化**: **§53-7-H**。再開後に Sonnet レビューで **§54-2** へ追記推奨。
...
- **C (Consulted)**: 相談先（§53 第二意見・Synthesis 等）
...
| 活動 | R | A | C | I |
|---|---|---|---|---|
| 憲法・運用ルール改訂（AGENTS.md / RULES-INDEX / WORKFLOW） | メイン AI（起草・commit） | **浜田** | §53 / §53-7 | autonomy log・朝報 |
| Tier A 副作用実行（§52-1） | メイン AI | **浜田**（不具合・仕様疑義時） | second-opinion | `logs/autonomy-decisions.log` |
...
| Negative Log（棄却案） | Sonnet 推奨・AI | **浜田**（監査） | §54-2 | `synthesis-graveyard/` |
...
#### §53-4 不一致時の処理 (R11 v3 / 2026-04-24 19:35 / Synthesis Logic 反映)

**v3 改訂**: 浜田 19:30 提案「高次元融合のルール体系 (Synthesis Logic / 弁証法 = 正→反→合)」+ Sonnet 反定立 (19:25 「自己審判」「偽の合」「検証不能」批判 / 多元論的並列提示の代替案) を統合した「ハイブリッド・並列+検証付 Synthesis」モデルを採用。

**詳細は §53-7 を参照**。要約:
- **デフォルト動作 (軽微〜中度判断)**: 多元論的並列提示 (正・反並列のまま浜田に提示 / 統合バイアス排除)
- **重大判断時のみ**: 検証付 Synthesis (合を導出 → Sonnet が合の妥当性検証 → 承認なら第 3 案として並列追加)
- **浜田の最終判断**: 提示された 2 or 3 案からどれか選択 (統合試案は採用必須ではない / 「全部却下 / 別案で」も可)
...
- v3 Synthesis Logic 発動時は cost 約 4 倍 (定立→反定立→合導出→合検証 = 4 ステップ) = 月 50 回想定 → $0-5 追加
...
#### §53-7 高次元融合プロセス (Synthesis Logic / R11 v3 / 2026-04-24 制定 / 浜田指示「弁証法 正→反→合」)

**背景**: 浜田 19:30 提案 = ヘーゲル弁証法 (定立→反定立→統合) を AI 対話に適用。メイン AI (Opus) C 案 = 透明性確保版 Synthesis Logic を提示 → Sonnet 反定立 19:25 で「自己審判」「偽の合」「検証不能」「§53-3 衝突」「3 つの破綻シナリオ」「多元論的並列提示の代替案」5 件指摘 → メイン AI が両者の対立を解消する合 (= 「ハイブリッド・並列+検証付 Synthesis」) を導出 → 浜田 19:35 A 案で承認。**Synthesis Logic 自身を Synthesis Logic で実装した初実例**。

##### §53-7-H Task / 第二意見経路が利用不能なとき（2026-04-25 制定 / C1 / [FIX]）

**典型**: Cursor Task (Sonnet) **API 上限**、ネットワーク断、外部 CLI (Codex/Gemini) 未起動。

1. チャット先頭に **`⚠ 第二意見経路不通 — <理由>`** を 1 行宣言（G-6 可用性: フリーズしない）。
2. **B 項の手順は次で代替**: ① 過去 **Negative Log / TSB / 同日の Synthesis 記録** を Read した上での内省を 1 段落チャットに記載 ② 判断ログに `second_opinion:"skipped:<理由>"` ③ 新規・重大条文の場合は `docs/archives/synthesis-graveyard/<日付>/` に **経路欠損メモ** 1 ファイル（§55-7 と同型）。
3. **§55 連動**: 憲法 Read 不能・インフラ連続失敗時は **§55 発動**を優先（セーフモード中は §55-4 が §53-7 B を上書きしうる）。

##### A. デフォルト動作 (軽微〜中度判断)

第二意見不一致時は **多元論的並列提示** (プラグマティズム / Sonnet 反定立 5 採用):
- メイン AI 原案 (= 正) + Sonnet 反定立 (= 反) を **並列のまま浜田に提示**
- 統合バイアス排除 (メイン AI が「合」を生成しない = 自己審判問題なし)
- 浜田が状況に応じて選択 (どちらか / 両者の組合せを浜田自身が判断 / 別案も可)
- cost 抑制 (Synthesis Logic 発動なし)

##### B. 重大判断時のみ (検証付 Synthesis 発動)

以下の場合のみ Synthesis Logic を発動:
- 不可逆操作 (削除 / リネーム / mcp.json 破壊的編集 / push --force)
- 大規模変更 (100 件以上の一括処理 / アプリ全削除)
- TSB 真因究明 (TSB-006/007/011/012/013/014/015 系の重大インシデント)
- 浜田が「Synthesis で考えて」「弁証法で」「合を出して」と明示要請時

発動時の 6 ステップ:
1. **定立**: メイン AI が原案 (= 正) を内部生成
2. **反定立**: Sonnet (or 別 model) に「致命的欠陥指摘 + 正反対視点で批判」依頼 (= 反 / fabrication 禁止 / 事実ベース厳守)
3. **統合導出**: メイン AI が両者の対立を解消する「合」候補を生成
4. **合の検証**: Sonnet (or 別 model) に「合は妥当か / 偽の合 (原案の言い換えに過ぎないか) / 両者のメリットを真に統合しているか」を再レビュー (= **自己審判問題の解消** / Sonnet 申し送り 19:25 反映)
5. **承認判定**:
   - Sonnet 承認 → 浜田に **「統合試案 (Sonnet 承認済 / 要・浜田確認)」として 第 3 案として並列に追加** (浜田は 3 案から選ぶ / 採用必須ではない)
   - Sonnet 不承認 → デフォルト動作 A に fallback / 統合試案を提示しない (= 偽の合の防止)
6. **浜田裁定**: 浜田が 2 or 3 案から選択 / または「全部却下 / 別案で」も可

##### C. 「合」のラベル必須

「合」「Synthesis」「融合」だけでは浜田が無批判に受け取る錯覚 (Sonnet 申し送り 19:25):
- 必ず **「統合試案 (Sonnet 承認済 / 要・浜田確認)」** と明示
- 「正」「反」も並列で残す (透明性確保)
- 例: `## 統合試案 (Sonnet 承認済 / 要・浜田確認)` セクションタイトル

##### D. 例外条項

「合」が物理的に存在しない対立 (例: 削除する / しない / 中間ない) の場合:
- メイン AI が「合不能 / 二者択一」と明示判断
- デフォルト動作 A (多元論的並列提示) に fallback
- Sonnet にも「合不能判定の妥当性」を確認 (= 例外乱用の抑止 / Sonnet シナリオ C 反映)

##### E. 失敗時の対応 (シナリオ A-C 反映)

- **シナリオ A (合の遅延)**: 定立→反定立→合導出→合検証 = 4 ステップで時間がかかる場合、浜田が「急ぐ」と明示時はデフォルト動作 A に fallback
- **シナリオ B (偽の合)**: Sonnet が「合は実質原案の言い換えに過ぎない」と判定したら fallback
- **シナリオ C (例外条項の乱用)**: 月次レビュー (R22 / 5/1 開始) で「例外発動率 > 50%」なら R11 v4 として再設計

##### F. 進化期待 (浜田 19:35 メッセージ「今後さらに進化していけるといいね 期待してる」)

R11 v3 はあくまで **2026-04-24 時点のスナップショット**。月次レビュー / 違反検知 / 浜田フィードバックで継続進化:
- v4 候補: best-of-n (3 AI) を Synthesis に組込 (Claude + GPT-5.5 + Gemini で正・反・反 → 三項合)
- v5 候補: AI が「合の合 (メタ Synthesis)」を導出 (反定立に対する反定立 = 弁証法の階層化)
- 提案・改善は浜田 §44 夕反省 / 月次レビュー / TSB 教訓ベースで採用

##### G. 運用強化指示 (浜田 19:50 伝達事項 / Synthesis Logic 暴走抑止 3 原則)

**G-1. 馴れ合いの統合は厳禁 (対 Sonnet / 反定立役および合検証役)**

> Sonnet、君の役割は『メイン AI の案に賛成すること』ではない。重大判断時、メイン AI が出してきた『統合案 (合)』に対して、少しでも妥当性が欠けていたり、君の元の批判 (反) が無視されていたりすれば、**容赦なく不承認 (Reject)** を出せ。**現場で『偽の合』が浜田様に提示されることは、このプロジェクトにおける最大の失態である**と認識せよ。

実装: Sonnet (or 別 model) への第二意見・反定立・合検証依頼プロンプトに、**毎回必ず本 G-1 の文言全文を含める** こと。承認バイアス防止 = 「合の検証で 100% 承認」が連続したら R11 自体の見直しシグナル (R10 §52-6 例外規定の monitor 対象)。

**G-2. ラベル表示の徹底 (対メイン AI / 統合試案提示時)**

> 重大判断時に統合案を提示する際は、必ず **『統合試案 (Sonnet 承認済 / 要・浜田確認)』というラベルを付与**せよ。浜田様が『AI が決めた正解』だと誤認しないよう、**あくまで判断材料の一つとして差し出す謙虚さを維持**せよ。**ラベルがない提案は、未完成品と見なす**。

実装: 統合試案提示時の必須テンプレ (チャット出力):
```markdown
## 📋 統合試案 (Sonnet 承認済 / 要・浜田確認)

(統合内容の本文)

> ⚠ これは判断材料の一つです。浜田様の最終判断で「統合試案」「正 (元案)」「反 (反定立)」「全部却下 / 別案」のいずれも選択可能です。
```

ラベル欠落 = §47 違反扱い (浜田が指摘したら即訂正 + 判断ログに「label_missing:true」記録)。

**G-3. 「重大判断」境界線を守れ (Synthesis Logic の濫用防止)**

> 何でもかんでも統合 (Synthesis) しようとして、浜田様の待ち時間を増やすな。**削除・リネーム・TSB 真因究明以外は、原則として『並列提示 (多元論)』でテンポ良く進めること**。この『判断の使い分け』こそが、今回導入した §53-7 の核心である。

実装: §53-7 B 項「重大判断時のみ」の発動条件を**厳格に適用**:
- ✅ 発動可: kintone レコード/アプリ削除・リネーム / push --force / mcp.json 破壊的編集 / TSB 真因究明 / 浜田が「Synthesis で考えて」「弁証法で」「合を出して」と明示要請時
- ❌ 発動不可: 通常の kintone 書込 (add / update) / 内部 script ロジック変更 / 軽微判断 / 一般的な技術選定
- 迷ったら**並列提示 (多元論) でテンポ優先**。Synthesis 6 ステップは時間 4 倍 = 浜田の待ち時間増加 = 本末転倒
- 月次レビューで「Synthesis 発動率 > 20%」なら濫用警告 (R10 §52-6 例外乱用と同じ monitor)

**G-4. 違反時の処理**

5 原則 (G-1/G-2/G-3/G-5) のいずれかを違反した場合:
- 即座に「§53-7-G 違反: <原因> / 次から改善します」と明示宣言
- 判断ログに `g_violation:"g-1"|"g-2"|"g-3"|"g-5"` フラグ + 内容を記録
- 高次元進化ログ (S20 / 21:00 cron) で「本日の G 違反」セクション提示 → 翌日改善材料

**G-5. 物理的な並列処理は禁止 (浜田 19:55 伝達事項 / 競合エラー回避最優先)**

> 「§53-7-G の運用において、いかなる理由があっても『物理的な並列処理 (マルチスレッド実行)』は禁止とする。
>
> **思考は並列、実行は直列**: 案を出し合う (思考) までは並列で良いが、実際のファイル書き込み、kintone への API リクエスト、設定変更は、必ず『一つの工程が終わってから次へ進む』完全直列で実行せよ。
>
> **シングル・エージェント・コントロール**: サブエージェントを複数同時に走らせるな。メイン AI が指揮を執り、一つずつタスクを完遂させ、その結果を確認してから次のタスクへ移行せよ。
>
> **エラー回避最優先**: 効率よりも『確実性』を優先する。少しでも競合のリスクがある場合は、処理を一時停止し、相談せよ。」

##### G-5-1. 「思考並列・実行直列」の境界線

| フェーズ | 並列 | 例 |
|---|---|---|
| **思考フェーズ** (案出し / 弁証法的内省 / 複数案比較) | ✅ OK (内部処理 / メモリ上のみ) | 正・反・合の同時保持 / 複数案検討 |
| **第二意見取得** (Sonnet 1 件呼出) | ✅ 単独 OK (1 subagent) | second-opinion = 1 AI / 通常はこれ |
| **best-of-n** (3 AI 起動) | ⚠ **§53-2 修正検討中** (Tier B 投入 / 翌朝 cron で判断) | 浜田 4/24 19:55 G-5 制定で「3 AI 同時起動」の扱い再考必須 |
| **実行フェーズ** (ファイル書込 / API / 設定変更) | ❌ **絶対禁止** | 1 操作完了 → 結果確認 → 次操作 |

##### G-5-2. シングル・エージェント・コントロール

- メイン AI (現在 Claude 4.6 Opus) が**唯一の指揮官**
- subagent (Task tool) は**同時に 1 つまで**
- subagent 完了 → 結果確認 → 次の subagent (必要なら) という順次パターン

##### G-5-3. エラー回避最優先

- 競合リスク察知時 = **処理一時停止 + 浜田相談 (Tier B 投入)**
- 競合リスクの典型例:
  - ファイル lock 可能性 (file-watcher 監視中ファイルへの大量編集)
  - cron 衝突可能性 (15 分ごと wipe-guard / 4 時間ごと health-check / 4 時間ごと auto-heal / 21:00 evening-reflect)
  - kintone API rate limit 接近 (短時間 100 リクエスト超)
  - mcp.json 編集中の MCP 起動
- 効率を犠牲にしても**確実性優先** = R10 §52-4「迷ったら昇格原則」と整合
- **§51 並列禁止と完全整合** (§51 = 1 ターン 1 ツール call / G-5 = 実行直列 = 同じ精神を Synthesis Logic 文脈で再強化)

**G-6. 可用性最優先 (浜田 20:05 伝達事項 / フリーズ回避ロジック / 「止まらない生存本能」)**

> 「三経路のうち、外部接続や手動操作に起因して『3 つの意見』が揃わない場合、最大何分待機し、その後どう動くべきかを定義せよ。
>
> 推奨案: 外部 API (経路 B) が 30 秒以内に応答しない場合は、自動的に『Sonnet (経路 A) との二者協議モード』へダウングレードし、その旨を浜田様に通知して進め。
>
> **浜田様の判断を止めること (フリーズ) を最大のリスクと見なし、『知能の多様性』よりも『システムの可用性』を優先するロジックを組み込め。**」

##### G-6-1. 経路別タイムアウトとフォールバック規定

| 経路 | タイムアウト | フォールバック先 | 浜田通知 |
|---|---|---|---|
| 経路 A (Cursor 内 Sonnet 単独 / Task tool) | 60 秒 | Tier B 投入 (浜田裁定 / 翌朝 cron) | 自動 (タイムアウト発生時) |
| 経路 B (外部 CLI Codex/Gemini) | **30 秒** | 経路 A (Cursor 内 Sonnet) にダウングレード | 自動「⚠ 経路 B (Codex) 30 秒タイムアウト → 経路 A (Sonnet) で代替実施」 |
| 経路 C (浜田手動切替) | **5 分** (浜田応答待機) | 経路 A (Cursor 内 Sonnet) にダウングレード | 自動「⚠ 浜田応答待機 5 分超 / Sonnet で代替実施」 |

##### G-6-2. 「システム可用性 > 知能の多様性」原則

- 浜田判断を止めること (フリーズ) = **最大のリスク** (本プロジェクトにおける最大の失態 = G-1 と同等の重要度)
- 3 視点欠ける場合でも、2 視点 (経路 A のみ) で進めるほうが**フリーズより安全**
- 単独視点 (経路 A 1 件) でも、**判断を進める**ほうが、待機より安全
- ただし「判断を進める = Tier A 即実行」ではなく「判断を進める + 浜田に正直に通知」

##### G-6-3. 浜田通知の自動化

ダウングレード発生時、浜田への通知必須:
- メイン AI チャット出力に `⚠ <経路> <理由> → <代替経路> でダウングレード継続` と明示
- 判断ログに `downgrade:"route-B-to-A"`, `downgrade_reason:"timeout-30s"` フラグ記録
- 高次元進化ログ (S20 / 21:00 cron) で「本日のダウングレード」セクション提示 → 翌日改善材料化
- 月次レビューで「ダウングレード発生率 > 10%」なら経路 B/C の信頼性見直し

##### G-6-4. 適用範囲 (永続化 = 「止まらない生存本能」を全外部連携に)

本フォールバック規定は以下に適用:
- §53-2 External Multi-Review (S21 で導入予定 / 翌朝 cron 適用後)
- §53-1 second-opinion (Cursor 内 Sonnet 1 件起動 / 60 秒タイムアウト)
- 将来導入される全外部連携機能 (新 skill / 新 MCP / 新 API / 新 Cursor model)

→ R11 §53 全体に「**止まらない生存本能**」を組込む = 浜田 20:05 伝達事項の核心実装。

##### G-6-5. 例外: 緊急セーフティブレーキ

ダウングレード自体が失敗 (例: Sonnet も応答しない) する場合:
- メイン AI 単独判断 (経路ゼロ = R10 §52 自己診断 5 問のみ) で Tier A 即実行
- ただし操作内容を判断ログに `route_zero:true` フラグ + 詳細記録
- 翌朝 06:00 cron + 21:00 §44 で浜田に強調通知 (連続 3 回発生で R11 全体見直し = メタガード)
