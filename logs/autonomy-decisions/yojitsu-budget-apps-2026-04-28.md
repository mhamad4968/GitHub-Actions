...
**「一旦すべて削除」**に対し、**kintone 公開 REST API ではアプリ本体の削除ができない**（`DELETE /k/v1/app.json`・`/k/v1/apps.json` は **405 Method Not Allowed**）。**テナント上の削除**は次のいずれかで実施する必要がある。

1. 各アプリの **設定（歯車）→ アプリの設定タブ → アプリの削除** で **675**・**676** を順に削除する。  
2. 参照アプリが無いことを確認のうえ **一括削除**（kintone ヘルプの「アプリの削除」に従う）。

**リポジトリ側**: `kintone-apps.md` の 675/676 行を **（未定）** に戻し、`SPEC.md` 冒頭を **アプリ未作成** に戻した。

**追記（浜田・削除完了報告）**: テナント上の **675 / 676** は浜田により削除済み。`GET /k/v1/apps.json?ids[0]=675&ids[1]=676` 相当の確認で **該当アプリなし**。
