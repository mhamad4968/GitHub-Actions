 ...
 *   --json          JSON 出力モード
 *   --target=PATH   検証対象ファイル (default AGENTS.md)
 *   --verbose       詳細 diff を表示
 */
...
const TARGET = ARG('target', 'AGENTS.md');
...
for (const breaking of breakingCommits) {
  ...
  for (const sub of subsequent) {
    ...
    if (undoneHeaders.length > 0) {
      ...
      if (alreadyResolved.length > 0 && VERBOSE) {
        issues.push({
          severity: 'info',
          breaking_commit: breaking.hash.slice(0, 7),
          breaking_subject: breaking.subject,
...
console.log('### post-BREAKING 削除 復活検知 (TSB-016 #20 / I-1)');
console.log('');
console.log(`- 対象ファイル: \`${TARGET}\``);
console.log(`- 走査範囲: 直近 ${SINCE} commit (実走査 ${commits.length} 件)`);
console.log(`- [BREAKING] commit 検出: ${summary.breaking_commits} 件`);
console.log('');

if (summary.passed) {
  console.log('✅ pass: 現在の HEAD に「ゾンビ復活した削除済章/節」は存在せず');
  if (VERBOSE) {
    ...
    if (resolved.length > 0) {
      ...
      for (const issue of resolved) {
        console.log(`- \`${issue.breaking_commit}\` 削除 → \`${issue.suspicious_commit}\` で復活 → 既に修復済 (resolved): ${issue.undone_headers.join(' / ')}`);
      }
    }
  }
} else {
  ...
  for (const issue of issues) {
    ...
    console.log(`#### ${issue.breaking_commit} の削除を ${issue.suspicious_commit} で復活 (現在も残存)`);
  ...
  console.log('');
  console.log('**対応**:');
  console.log('1. 残存している章/節を AGENTS.md から削除する新 commit を作成');
