...
const checks = [
  ...
  ['給与名称listOnly', /SALARY_ROLE_MASTER/],
  ['給与氏名列', /氏名（選択）/],
  ...
  ['計バッジ', /jy2-subtotal-badge/],
  ['計バッジは金額列直前', /label\.colSpan = 9/],
  ['総括整合チェックがrow_keyを読む', /function jy2SummaryCostLinesFromRecord\([\s\S]*?summary_row_key[\s\S]*?return rows\.map/],
  ['印刷ボタン', /jy2-print-button/],
  ['印刷ポータル', /jy2-print-portal/],
  ['印刷計バッジ', /jy2-pr-sub-badge/],
  ['印刷は切らず次頁', /html,body\{[^}]*overflow:visible!important/],
  ['印刷セル折り返し', /overflow-wrap:anywhere/],
  ['印刷列見出し繰り返し', /function jy2PrStartTable[\s\S]*?thead\.appendChild\(jy2PrHeadRow/],
  ['印刷thead繰り返し', /\.jy2-pr-table thead\{display:table-header-group\}/],
  ['印刷計行は分割しない', /\.jy2-pr-table tr\.jy2-pr-sub[\s\S]*?page-break-inside:avoid/],
  ['印刷中央揃え', /td\.jy2-pr-center\{text-align:center/],
  ['印刷数量は中央', /jy2-pr-center", line\.quantity/],
  ['印刷金額は右', /td\.jy2-num[\s\S]*?text-align:right/],
  ['合計行備考保存', /summary_total_notes/],
  ['合計行備考セル', /function jy2TotalNoteCell/],
  ['10800種別レンタル', /仮設材･鉄道器材レンタル/],
  ['諸経費自動計算(R-11)', /overheadFromDetails/],
  ['諸経費率10%', /OVERHEAD_RATE_PERCENT/],
  ['諸経費根拠の行内表示(案B)', /jy2-footer-basis/],
  ['諸経費単価注意書き', /諸経費の単価は外注費の明細合計です/],
  ['諸経費根拠文言', /外注費明細合計 ×\$\{footerRow\.ratePercent\}%/],
  ['費目列ラベル', /費目（選択）/],
  ['種別列ラベル', /種別（選択）/],
  ['詳細列ラベル', /詳細（入力）／材料（選択）/],
  ...
  ['タブ名は工事原価管理', /工事原価管理/],
  ['原価管理タブv2表示', /JY2_HIDE_COST_MGMT_TAB\s*=\s*false/],
  ['原価管理v2置換', /JY2_COST_MGMT_V2\s*=\s*true/],
