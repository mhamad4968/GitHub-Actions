/**
 * 部署予実 入力アプリ 677: レコード直下フィールド batch1 をプレビューに追加し deploy。
 * 正本: templates/yojitsu-budget-lite/docs/yojitsu-master-and-field-plan.md §3
 * DeepSeek/CIO: CALC `variable_budget_total_display` は batch2（数値フィールド作成後）。
 *
 *   node scripts/yojitsu-677-add-batch1-preview.mjs --dry-run
 *   node scripts/yojitsu-677-add-batch1-preview.mjs
 */
...
const APP = 677;
const FRAGMENT_PATH = path.join(path.dirname(fileURLToPath(import.meta.url)), 'data', 'yojitsu-677-batch1-properties.json');
...
async function main() {
  ...
  if (cur.properties?.work_type_name) {
    console.log('677: work_type_name は既に存在。POST スキップ。');
    return;
  }

  if (dryRun) {
    console.log(JSON.stringify({ app: APP, properties }, null, 2));
    console.error('[677] dry-run: POST していません');
    return;
  }

  const rev = await postPreviewFields(properties);
  console.log('677: preview revision', rev);
  await deployPreview(rev);
  await waitDeploy();
  console.log('677: deploy SUCCESS');
}
