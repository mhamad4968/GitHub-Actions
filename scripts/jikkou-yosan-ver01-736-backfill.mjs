#!/usr/bin/env node
/**
 * 【Ver.01 専用】App 736 — 版管理 v2a 既存データ移行（version_seq / status / row_key）
 *   npm run jikkou-yosan:ver01-736-backfill -- --dry-run
 *   npm run jikkou-yosan:ver01-736-backfill -- --apply
 *
 * Phase5 hardening (H1): このスクリプトは Ver.01 の App 736 フィールドパッチであり、
 * Ver.02（3アプリ構成・jikkou-yosan-v2-*）とは無関係。Ver.02 の CLI 名前空間から
 * 隔離するため jikkou-yosan-v2-backfill.mjs から改名した。旧パスは HARD ABORT スタブ。
 */
