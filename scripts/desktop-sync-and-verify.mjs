...
// mid-session / Desktop 同期では締め用の hard NG を出さない。
// #D-CLOSE-02 日付は --wake-context、Git/bridge ラグは --warn-only。
// 締め本体は cio:session:close-git が先に strict（両フラグなし）を実行する。
runStep('verify-session-close-git-warn', () =>
  runNodeScriptSync(
    root,
    'scripts/verify-session-close-git-warn.mjs',
    ['--wake-context', '--warn-only'],
    { stdio: 'inherit' },
  ),
);
