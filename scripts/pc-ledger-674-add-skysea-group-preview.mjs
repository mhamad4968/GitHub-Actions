/**
 * App 674: `scripts/data/pc-ledger-674-add-skysea-group-properties.json` の properties を
 * プレビュー `POST /k/v1/preview/app/form/fields.json` でマージし、preview deploy まで実行。
 *
 *   npm run pc-ledger:674:add-skysea-group-preview -- --dry-run
 *   npm run pc-ledger:674:add-skysea-group-preview
 */
...
const APP = 674;
const FRAGMENT_PATH = path.join(path.dirname(fileURLToPath(import.meta.url)), 'data', 'pc-ledger-674-add-skysea-group-properties.json');
...
async function main() {
  ...
  if (cur.properties?.skysea_system_meta) {
    console.log('674: skysea_system_meta は既に存在。POST スキップ。');
    return;
  }
  ...
  await waitDeploy();
  console.log(`674: skysea_system_meta 追加 + preview deploy SUCCESS revision=${rev}`);
}
