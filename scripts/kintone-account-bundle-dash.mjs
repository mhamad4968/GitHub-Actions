...
const dir = path.join(root, 'customize', 'kintone-account-dash');

const sync = spawnSync('node', ['scripts/kintone-account-sync-dash-db-id.mjs'], {
  cwd: root,
  encoding: 'utf8',
  shell: false,
});
...
const banner =
  ...
  xlsxVersion +
  ' + customize/kintone-account-dash/desktop.src.js */\n';
writeFileSync(path.join(dir, 'desktop.js'), banner + xlsx + '\n' + src, 'utf8');
console.log('bundled customize/kintone-account-dash/desktop.js');

const lint = spawnSync('npm', ['run', 'lint:customize', '--silent'], { cwd: root, encoding: 'utf8', shell: true });
if (lint.status !== 0) {
  console.error('[kintone-account:bundle-dash] lint NG');
  process.exit(1);
}
console.log('[kintone-account:bundle-dash] lint OK');
