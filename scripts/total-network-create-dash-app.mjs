#!/usr/bin/env node
/**
 * トータルネットワーク — Dash app create (Space 48 / thread 52)
 */
import {
  DASH_APP_NAME,
  SPACE_ID,
...
async function main() {
  ...
  if (!dbAppId) {
    console.error('dbAppId missing. Run total-network:create-db first.');
    process.exit(1);
  }
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DASH_APP_NAME,
    'トータルネットワーク ネットワーク管理台帳 — 一覧表・IPマトリックス・設定の唯一の入り口。',
  );
