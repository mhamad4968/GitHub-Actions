#!/usr/bin/env node
/**
 * 拠点の ip_count を ip_range_start〜end から再計算して kintone に同期
 *
...
async function fetchSites(baseUrl, headers, appId) {
  ...
  while (true) {
    ...
    const fields = ['$id', 'location_name', 'ip_count', 'ip_range_start', 'ip_range_end'];
...
async function main() {
  ...
  for (const rec of sites) {
    ...
    const start = rec.ip_range_start?.value || '';
    const end = rec.ip_range_end?.value || '';
    const expected = computeIpCountFromRange(start, end);
    if (!expected) continue;
    const current =
      rec.ip_count?.value == null || rec.ip_count?.value === '' ? '' : String(rec.ip_count.value);
  ...
  console.log(JSON.stringify({ updates: updates.length, report }, null, 2));
