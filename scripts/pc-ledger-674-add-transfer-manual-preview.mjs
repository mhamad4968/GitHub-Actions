/**
 * App 674: 手動転用チェックボックス `npl_transfer_manual` をプレビューへ POST し deploy まで実行。
 *
 *   npx dotenv -e .env -e .env.proxy -- node scripts/pc-ledger-674-add-transfer-manual-preview.mjs --dry-run
 *   npx dotenv -e .env -e .env.proxy -- node scripts/pc-ledger-674-add-transfer-manual-preview.mjs
 */
...
const FRAGMENT_PATH = path.join(
  ...
  'data',
  'pc-ledger-674-add-transfer-manual-properties.json',
);
...
async function main() {
  ...
  if (cur.properties?.npl_transfer_manual) {
    console.log('674: npl_transfer_manual は既に存在。POST スキップ。');
    return;
  }
  ...
  await waitDeploy();
  console.log('[674] deploy SUCCESS (npl_transfer_manual 追加)');
}
