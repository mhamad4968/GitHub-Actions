/**
 * App 674: 転用で廃棄したPCの転記用 `npl_disposed_pc_copy`（SINGLE_LINE_TEXT）をプレビューへ POST し deploy まで実行。
 *
 *   npx dotenv -e .env -e .env.proxy -- node scripts/pc-ledger-674-add-npl-disposed-pc-field-preview.mjs --dry-run
 *   npx dotenv -e .env -e .env.proxy -- node scripts/pc-ledger-674-add-npl-disposed-pc-field-preview.mjs
 */
...
const FRAGMENT_PATH = path.join(
  ...
  'data',
  'pc-ledger-674-add-npl-disposed-pc-field.json',
);
...
async function main() {
  ...
  if (cur.properties?.npl_disposed_pc_copy) {
    console.log('674: npl_disposed_pc_copy は既に存在。POST スキップ。');
    return;
  }
  ...
  await waitDeploy();
  console.log('[674] deploy SUCCESS (npl_disposed_pc_copy 追加)');
}
