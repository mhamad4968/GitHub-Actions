#!/usr/bin/env node
/**
 * Storage media ledger — Dash app create (Space 21 / thread 23)
 * 正本: docs/plans/2026-06-13-storage-media-ledger-kintone-spec.md §4.1 / §9
 */
import {
  DASH_APP_NAME,
  SPACE_ID,
  ...
  loadAppIds,
  saveAppIds,
} from './lib/storage-media-ledger-kintone.mjs';
...
async function main() {
  ...
  if (!dbAppId) {
    console.error('dbAppId missing. Run storage-media-ledger:create-db first.');
    process.exit(1);
  }
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DASH_APP_NAME,
    '記憶媒体等管理台帳 v1 — 新規・修正・廃止・削除・リスト出力の唯一の入り口。',
  );
