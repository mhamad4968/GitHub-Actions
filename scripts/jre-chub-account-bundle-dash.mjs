...
const dir = path.join(root, 'customize', 'jre-chub-account-dash');

const sync = spawnSync('node', ['scripts/jre-chub-account-sync-dash-db-id.mjs'], {
  cwd: root,
  encoding: 'utf8',
  shell: false,
});
...
const banner =
  ...
  xlsxVersion +
  ' + customize/jre-chub-account-dash/desktop.src.js */\n';
writeFileSync(path.join(dir, 'desktop.js'), banner + xlsx + '\n' + src, 'utf8');
console.log('bundled customize/jre-chub-account-dash/desktop.js');

const lint = spawnSync('npm', ['run', 'lint:customize', '--silent'], { cwd: root, encoding: 'utf8', shell: true });
if (lint.status !== 0) {
  console.error('[jre-chub:bundle-dash] lint NG');
  process.exit(1);
}
console.log('[jre-chub:bundle-dash] lint OK');
