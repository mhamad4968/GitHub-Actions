#!/usr/bin/env node
/** JRE-C_Hub — DB app create (Space 34) */
import {
  DB_APP_NAME,
  deployApp,
  fetchJson,
...
async function main() {
  ...
  if (dryRun) {
    console.log(
      JSON.stringify({ name: DB_APP_NAME, space: 34, thread: threadId, fieldCount: Object.keys(properties).length }, null, 2),
    );
    return;
  }
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DB_APP_NAME,
    'JRE-C_Hub アカウントの正本。日常の書込は「JRE-C_Hubアカウント台帳」から REST のみ。標準 UI は閲覧のみ（IT管理者）。',
  );
