#!/usr/bin/env node
/**
 * NAS管理台帳 — Dash app create (Space 48, no thread)
 */
import {
  DASH_APP_NAME,
  SPACE_ID,
...
async function main() {
  ...
  if (!dbAppId) {
    console.error('dbAppId missing. Run nas-ledger:create-db first.');
    process.exit(1);
  }
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DASH_APP_NAME,
    'NAS管理台帳 — 一覧・編集・印刷・xlsx 出力の唯一の入り口。',
  );
  ...
  const aclRev = await setEveryoneAcl(baseUrl, headers, appId).catch(function (e) {
    console.warn(`[warn] ACL skipped: ${e.message || e}`);
    return deployRev;
  });
  if (aclRev !== deployRev) {
