/**
 * App 674: 内部メタフィールドを kintone 標準フィールドグループ `internal_system_meta` 内へ移動する。
 *
...
async function main() {
  ...
  if (existing && groupLayoutIsComplete(existing)) {
    console.log(
      `674: "${GROUP_CODE}" 内に内部メタ ${CHILD_CODES.length} 件が既に配置済み。スキップ。`,
    );
    return;
  }
  ...
  await waitDeploy();
  console.log(
    `674: preview layout 更新 + deploy SUCCESS（GROUP "${GROUP_CODE}" 内に ${CHILD_CODES.length} フィールドを収容）revision=${rev}`,
  );
}
