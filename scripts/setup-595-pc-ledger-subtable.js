...
/**
 * 595 に PC 台帳（594）複数台対応のサブテーブルを追加する。
 * - 親: pc_ledger_list
 * - 子: pc_594_record_id（NUMBER）, pc_ledger_entry_done（CHECK_BOX・作成済み）
 * ledger_created / ledger_record_id（627 用）は変更しない。
 */
...
if ((current.properties || {})[FC_TABLE]) {
  console.log('[setup595 pc_ledger] already exists (no changes)');
  process.exit(0);
}
    ...
    properties: {
      [FC_TABLE]: {
        ...
        fields: {
          ...
          pc_ledger_entry_done: {
            type: 'CHECK_BOX',
            code: 'pc_ledger_entry_done',
            label: 'PC台帳登録',
            noLabel: false,
            required: false,
            options: {
              作成済み: { label: '作成済み', index: '0' },
            },
          },
        },
      },
...
console.log(`[setup595 pc_ledger] created revision=${created.revision}`);
...
for (let i = 0; i < 60; i++) {
  ...
  if (status === 'SUCCESS') {
    console.log('[setup595 pc_ledger] deploy SUCCESS — run: npm run app:fields 595');
    process.exit(0);
  }
