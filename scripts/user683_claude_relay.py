...
PROMPT_WEEK = (
    ...
    "会計年度四半期は固定対応のみ（暦年の1–3月=Q1等は禁止）: 5–7月=第1四半期、8–10月=第2四半期、11–1月=第3四半期、2–4月=第4四半期。"
    "週内に**年末年始・ゴールデンウィーク・秋分の日前後の連休（いわゆるシルバーウィーク）**など稼働日が少ない期間が含まれる場合は、件数・負荷の変動をその文脈で読むこと。"
    "コーパス行頭の**[土日祝対応・…]**は土・日・祝日に実際に対応した内容（本来は不要だが行った対応）である。平日業務と同列に書かず、該当があれば要約の1点で区別して触れよ。件数0の土日祝は扱わない。"
...
PROMPT_MONTH = (
    ...
    "コーパス行頭の**[土日祝対応・…]**は土・日・祝日に実際に対応した内容（本来は不要だが行った対応）である。平日と同列の通常業務として書くな。"
