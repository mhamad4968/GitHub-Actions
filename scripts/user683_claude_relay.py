...
RELAY_BUILD = "2026-05-16-user683-claude-relay-fiscal-holidays"
...
PROMPT_WEEK = (
    ...
    "週内に**年末年始・ゴールデンウィーク・秋分の日前後の連休（シルバーウィーク）**など稼働日が少ない期間が含まれる場合は、件数・負荷の変動をその文脈で読むこと。"
    "日本語で、箇条書き3点以内・合計200字以内で要約してください。コーパスに無い数値は書かないこと。余計な前置きは不要。\n\n"
)
PROMPT_MONTH = (
    "【前月の月次要約】と【当月の対応メモ抜粋】が続きます。"
    "当社の会計年度は**4月末が期末・5月が期首**である（暦月ベースの前月比に加え、必要ならその暦月が会計年度上どの位置かに一言触れてよい。例: 4月＝期末月、5月＝新年度期首、1〜3月＝期末に向けた後半、5〜12月＝期中。コーパスに無い変形会計カレンダーや四半期名は付け加えない）。"
    "当月のコーパスに**年末年始・ゴールデンウィーク・秋分の日前後の連休（シルバーウィーク）**など大型休暇が含まれる場合は、稼働日減による件数・相談量の変動を疑い、コーパスに根拠がある範囲で言及すること（根拠が無いときは断定しない）。"
    "日本語で、冒頭1〜3文で前月と比べた所感（件数・相談内容・負荷の増減など）を必ず書き、その後に箇条書き3点以内で当月の要点をまとめてください。"
    "コーパスに明示されていない数値は書かないでください。"
    "全体は350字程度を目安（やや超過可）。前月要約が空または「要約キャッシュなし」のときは「前月比:（初月または欠）」の1行でよいです。余計な前置きは不要。\n\n"
)
