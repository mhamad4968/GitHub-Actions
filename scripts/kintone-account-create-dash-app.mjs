#!/usr/bin/env node
/** Kintoneアカウント — Dash app create (Space 48) */
import {
  DASH_APP_NAME,
  deployApp,
  fetchJson,
...
async function main() {
  ...
  if (!dbAppId) {
    console.error('dbAppId missing. Run kintone-account:create-db first.');
    process.exit(1);
  }
  ...
  console.log(`作成: "${DASH_APP_NAME}" thread=${threadId} db=${dbAppId}`);
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DASH_APP_NAME,
    'Kintoneアカウント管理台帳 — 一覧・新規・編集・集計・出力の唯一の入り口。',
  );
