#!/usr/bin/env node
/** SheetJS + dash source → desktop.js */
...
const dir = path.join(root, 'customize', 'total-network-dash');

const sync = spawnSync('node', ['scripts/total-network-sync-dash-db-id.mjs'], {
  cwd: root,
  encoding: 'utf8',
  shell: false,
});
...
const banner =
  ...
  xlsxVersion +
  ' + customize/total-network-dash/desktop.src.js — do not edit by hand */\n';
writeFileSync(path.join(dir, 'desktop.js'), banner + xlsx + '\n' + src, 'utf8');
console.log('bundled customize/total-network-dash/desktop.js');
...
if (lint.status !== 0) {
  console.error('[total-network:bundle-dash] lint:customize NG');
  process.exit(1);
}
console.log('[total-network:bundle-dash] lint:customize OK');
