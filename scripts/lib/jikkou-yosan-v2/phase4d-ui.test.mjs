...
test("App 1 actual tab renders the jy2-* 予実 matrix wired to editActuals", () => {
  ...
  assert.match(source, /@JY_V2_BUILD 2026-09-12-ver02-overhead-gaichu-only/);
  ...
  assert.match(source, /jy2CostMgmtPlaceManagerWageAfterKamagaya/);
  assert.match(source, /Phase2c-excel-10800-after-10700/);
  assert.match(source, /jy2CostMgmtPlaceKamagayaAfterPaintAncillary/);
  ...
  assert.match(
    source,
    /"11200":\s*Object\.freeze\(\["列車見張員賃金"\]\)/,
  );
  assert.match(
    source,
    /"11200":\s*Object\.freeze\(\{\s*"列車見張員賃金":\s*Object\.freeze\(\["昼間",\s*"夜間"\]\)/,
  );
  assert.match(
    source,
    /shortName:\s*"列車見張員賃金",\s*workTypeCode:\s*"11200"/,
  );
  assert.match(
    source,
    /"11300":\s*Object\.freeze\(\["交通整理員賃金"\]\)/,
  );
  assert.match(
    source,
    /"11300":\s*Object\.freeze\(\{\s*"交通整理員賃金":\s*Object\.freeze\(\["昼間",\s*"夜間"\]\)/,
  );
  assert.match(
    source,
    /shortName:\s*"交通整理員賃金",\s*workTypeCode:\s*"11300"/,
  );
  {
    ...
    assert.equal(omitCodes[1].includes('"11200"'), false);
    assert.equal(omitCodes[1].includes('"11300"'), false);
    ...
    assert.equal(omitNames[1].includes('"列車見張員"'), false);
    assert.equal(omitNames[1].includes('"外注列車見張員"'), false);
    assert.equal(omitNames[1].includes('"交通整理員"'), false);
    assert.equal(omitNames[1].includes('"外注交通整理員"'), false);
    assert.equal(omitNames[1].includes('"交通整理員等"'), false);
  }
  ...
  assert.match(source, /Phase2c-excel-12800-compensation/);
  assert.match(source, /Phase2c-excel-12800-col-widths/);
  ...
  assert.match(
    source,
    /"12800":\s*Object\.freeze\(\["補償費"\]\)/,
  );
  assert.match(
    source,
    /"12700":\s*Object\.freeze\(\["建退共証紙購入費"\]\)/,
  );
  assert.match(
    source,
    /"12600":\s*Object\.freeze\(\["履行保証保険料"\]\)/,
  );
  assert.match(
    source,
  ...
  assert.match(source, /Phase2c-fix-flat-plus-strip/);
  assert.match(source, /手入力行（＋で reveal 済み）は name1 だけでも残す/);
  assert.match(source, /jy2ActualReuseEmptyDetailIfSole/);
  assert.match(
    source,
    /jy2CostMgmtEnsureTypeOnlyFrames\(detailModel,\s*costDetailVisibility\)/,
  );
  ...
  assert.match(source, /jy2CostMgmtPlaceDayNightFramesAfterManagerWage/);
  assert.match(source, /jy2CostMgmtFindPaintAncillaryAnchor/);
  assert.match(source, /jy2CostMgmtFindManagerWageAnchor/);
  assert.match(source, /JY2_COST_MGMT_ENSURE_TYPE_ONLY_FRAMES/);
  assert.match(source, /JY2_COST_MGMT_ENSURE_DAY_NIGHT_FRAMES/);
  assert.match(source, /JY2_COST_MGMT_TYPES_OVERRIDE/);
  assert.match(source, /JY2_COST_MGMT_TYPES_OVERRIDE_BY_HIMOKU/);
  ...
  );
  {
    const typelessMatch = source.match(
      /JY2_COST_MGMT_TYPELESS_HIMOKU = Object\.freeze\(\[([\s\S]*?)\]\);/,
    );
    assert.ok(typelessMatch, "TYPELESS list present");
    assert.doesNotMatch(typelessMatch[1], /建設機械オペレーター/);
  }
  ...
  // 自動確保時に name1 を載せない（費目枠のみ）
  assert.match(source, /name1 は載せない/);
  ...
  assert.match(source, /jy2CostMgmtShouldOmitHimoku/);
  ...
  assert.match(
    source,
    /\.jy2-actual-table \.jy2-actual-parent-himoku,\.jy2-actual-table \.jy2-actual-himoku-group-label\{font-weight:700;text-align:left;padding-left:6px/,
  );
  assert.match(source, /#R-EXCEL-UI-07/);
  assert.match(source, /#R-EXCEL-UI-08/);
