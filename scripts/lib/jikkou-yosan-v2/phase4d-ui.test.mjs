...
test("App 1 actual tab renders the jy2-* 予実 matrix wired to editActuals", () => {
  ...
  assert.match(source, /@JY_V2_BUILD 2026-08-01-ver02-actual-omit-sonota-himoku/);
  assert.match(source, /JY2_ACTUAL_DETAIL_MANUAL_ONLY/);
  assert.match(source, /JY2_COST_MGMT_TYPE_DENY/);
  assert.match(source, /JY2_COST_MGMT_HIMOKU_DENY/);
  assert.match(source, /jy2CostMgmtIsDeniedHimoku/);
