/**
 * Storage media ledger kintone — shared REST helpers (Space 21).
...
export const DB_APP_NAME = '記憶媒体等台帳DB';
export const DASH_APP_NAME = '記憶媒体等管理台帳ver.1';
export const SPACE_ID = Number(process.env.STORAGE_MEDIA_LEDGER_SPACE_ID || process.env.APPLE_ID_SPACE_ID || 21);
export const THREAD_ID = Number(process.env.STORAGE_MEDIA_LEDGER_THREAD_ID || process.env.APPLE_ID_THREAD_ID || 23);
export const STATUS_ACTIVE = '利用中';
export const STATUS_RETIRED = '廃止';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
export const FIELDS_PATH = path.join(__dirname, '..', 'data', 'storage-media-ledger-db-fields.json');
export const STATE_PATH = path.join(__dirname, '..', 'data', 'storage-media-ledger-app-ids.json');
...
export function loadFieldProperties() {
  ...
  if (!raw.properties) throw new Error('storage-media-ledger-db-fields.json: missing properties');
