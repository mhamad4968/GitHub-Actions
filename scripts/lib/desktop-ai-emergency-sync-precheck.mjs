 ...
 * Soft-tune（§50-3-8）: RAM INFO 80–89 / WARN ≥90; Notepad WARN は 24/25 ロック時のみ
...
/** WARN / strict fail 閾値（%） */
export const RAM_WARN_PERCENT = 90;

/** INFO 帯の下限（%）— [RAM_INFO_PERCENT, RAM_WARN_PERCENT) は INFO のみ */
export const RAM_INFO_PERCENT = 80;
...
export function runDesktopSyncPrecheck(destDir, { strict = false } = {}) {
  ...
  } else if (ram >= RAM_INFO_PERCENT) {
    // soft-tune: INFO only（warn にしない）
    infos.push(
      `RAM ${ram}%（INFO ${RAM_INFO_PERCENT}–${RAM_WARN_PERCENT - 1}%）— LITE 推奨・WARN 未満`,
    );
  }
  ...
  if (notepadCount > 0) {
    if (lockedNames.length > 0) {
      warns.push(
        `Notepad ${notepadCount} 件稼働 — 24/25 .md ロック中は Application Hang リスク（C2）`,
      );
    } else {
      // soft note once: WARN にせず INFO 1 行のみ
      infos.push(
        `Notepad ${notepadCount} 件稼働 — 24/25 は未ロック。メモ帳は **34/35-LITE.txt** のみ推奨（C2 soft）`,
      );
    }
  }
  ...
  for (const info of infos) {
    console.log(`[desktop-sync-precheck] INFO ${info}`);
  }
  for (const w of warns) {
    console.warn(`[desktop-sync-precheck] WARN ${w}`);
  }
  if (warns.length > 0 || infos.some((i) => i.includes('Notepad'))) {
    console.log(
  ...
  if (strict && warns.length > 0) {
    console.error('[desktop-sync-precheck] NG — --precheck-strict（WARN 解消まで sync 中止）');
    return false;
  }
  return true;
}
