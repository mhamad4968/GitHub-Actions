/**
 * Kintoneアカウント管理台帳 — shared REST helpers (Space 48).
 * 正本: docs/plans/2026-07-05-kintone-account-ledger-spec.md
 */
import { readFileSync, writeFileSync, existsSync } from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

export const DB_APP_NAME = 'Kintoneアカウント管理台帳DB';
export const DASH_APP_NAME = 'Kintoneアカウント管理台帳';
export const SPACE_ID = Number(process.env.KINTONE_ACCOUNT_SPACE_ID || 48);
export const THREAD_NAME = process.env.KINTONE_ACCOUNT_THREAD_NAME || 'Kintoneアカウント管理';
...
export const STATE_PATH = path.join(__dirname, '..', 'data', 'kintone-account-app-ids.json');
export const ORGS_PATH = path.join(__dirname, '..', 'data', 'kintone-account-orgs.json');
export const DEPTS_BY_ORG_PATH = path.join(__dirname, '..', 'data', 'kintone-account-depts-by-org.json');
...
export function loadOrgs() {
  return loadJson(ORGS_PATH);
}

export function loadDeptsByOrg() {
  const raw = loadJson(DEPTS_BY_ORG_PATH);
  const { _comment, ...rest } = raw;
  return rest;
}

export function flattenDepts(deptsByOrg) {
  const set = new Set();
  Object.values(deptsByOrg).forEach((arr) => {
...
export async function resolveThreadId(baseUrl, headers) {
  if (process.env.KINTONE_ACCOUNT_THREAD_ID) return Number(process.env.KINTONE_ACCOUNT_THREAD_ID);
  try {
    ...
    if (id) {
      console.log(`[kintone-account] created thread id=${id} name=${THREAD_NAME}`);
      return id;
    }
  } catch (e) {
    console.warn('[kintone-account] thread/add skipped:', e.message || e);
  }
  const fallback = Number(process.env.KINTONE_ACCOUNT_THREAD_FALLBACK || 52);
  console.log(`[kintone-account] using thread fallback=${fallback}`);
  return fallback;
}

export function loadFieldProperties() {
  const raw = JSON.parse(readFileSync(FIELDS_PATH, 'utf8'));
  if (!raw.properties) throw new Error('kintone-account-db-fields.json: missing properties');
  return raw.properties;
}

export function loadAppIds() {
  if (!existsSync(STATE_PATH)) return { dbAppId: null, dashAppId: null, spaceId: SPACE_ID, threadId: null };
...
export async function recordCount(baseUrl, headers, appId) {
  const url = `${baseUrl}/k/v1/records.json?app=${appId}&query=${encodeURIComponent('order by login_id asc limit 1')}&totalCount=true`;
...
export function paySiteFromAccountType(accountType) {
  if (accountType === '首都圏支店個人') return '首都圏支店';
  return '本社';
}
