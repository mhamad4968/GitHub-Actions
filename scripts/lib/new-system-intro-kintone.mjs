/**
 * 新規システム導入ヒアリング — kintone REST helpers (Space 48).
 */
import { readFileSync, writeFileSync, existsSync } from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

export const DB_APP_NAME = '新規システム導入ヒアリング用DB';
export const DASH_APP_NAME = '新規システム導入ヒアリング記録';
export const SPACE_ID = Number(process.env.NEW_SYSTEM_INTRO_SPACE_ID || 48);

const __dirname = path.dirname(fileURLToPath(import.meta.url));
export const FIELDS_PATH = path.join(__dirname, '..', 'data', 'new-system-intro-db-fields.json');
export const STATE_PATH = path.join(__dirname, '..', 'data', 'new-system-intro-app-ids.json');
...
export async function resolveDefaultThreadId(baseUrl, headers, spaceId) {
  const override = process.env.KINTONE_SPACE_DEFAULT_THREAD || process.env.NEW_SYSTEM_INTRO_THREAD_ID;
  if (override && /^\d+$/.test(String(override))) return Number(override);
  ...
  if (!dt || String(dt).trim() === '') throw new Error(`Space ${spaceId} has no defaultThread`);
  return Number(dt);
}

export function loadFieldProperties() {
  const raw = JSON.parse(readFileSync(FIELDS_PATH, 'utf8'));
  if (!raw.properties) throw new Error('new-system-intro-db-fields.json: missing properties');
  return raw.properties;
}

export function loadAppIds() {
  if (!existsSync(STATE_PATH)) return { spaceId: SPACE_ID, threadId: null, dbAppId: null, dashAppId: null };
