/**
 * Software ledger kintone — shared REST helpers (Space 21).
 * 正本: docs/plans/2026-06-13-software-ledger-kintone-spec.md
 */
import { readFileSync, writeFileSync, existsSync } from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

export const DB_APP_NAME = 'ソフトウエア台帳DB';
export const DASH_APP_NAME = 'ソフトウエア管理台帳ver.1';
export const SPACE_ID = Number(process.env.SOFTWARE_LEDGER_SPACE_ID || process.env.APPLE_ID_SPACE_ID || 21);
export const THREAD_ID = Number(process.env.SOFTWARE_LEDGER_THREAD_ID || process.env.APPLE_ID_THREAD_ID || 23);
export const STATUS_ACTIVE = '利用中';
export const STATUS_RETIRED = '廃止';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
export const FIELDS_PATH = path.join(__dirname, '..', 'data', 'software-ledger-db-fields.json');
export const STATE_PATH = path.join(__dirname, '..', 'data', 'software-ledger-app-ids.json');
...
export function loadFieldProperties() {
  ...
  if (!raw.properties) throw new Error('software-ledger-db-fields.json: missing properties');
