/**
 * 不適合管理台帳 — kintone REST helpers (Space 48).
...
export const DB_APP_NAME = '不適合管理台帳用DB';
export const DASH_APP_NAME = '不適合管理台帳';
export const SPACE_ID = Number(process.env.NONCONFORMANCE_SPACE_ID || 48);

const __dirname = path.dirname(fileURLToPath(import.meta.url));
export const FIELDS_PATH = path.join(__dirname, '..', 'data', 'nonconformance-db-fields.json');
export const STATE_PATH = path.join(__dirname, '..', 'data', 'nonconformance-app-ids.json');
...
export async function resolveDefaultThreadId(baseUrl, headers, spaceId) {
  const override = process.env.KINTONE_SPACE_DEFAULT_THREAD || process.env.NONCONFORMANCE_THREAD_ID;
...
export function loadFieldProperties() {
  ...
  if (!raw.properties) throw new Error('nonconformance-db-fields.json: missing properties');
