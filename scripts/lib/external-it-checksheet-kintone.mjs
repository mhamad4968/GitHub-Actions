/**
 * 外部 IT サービス導入チェックシート — kintone REST helpers (Space 48).
...
export const DB_APP_NAME = '外部ITサービス導入チェック用DB';
export const DASH_APP_NAME = '外部ITサービス導入チェックシート';
export const SPACE_ID = Number(process.env.EXTERNAL_IT_CHECKSHEET_SPACE_ID || 48);

const __dirname = path.dirname(fileURLToPath(import.meta.url));
export const FIELDS_PATH = path.join(__dirname, '..', 'data', 'external-it-checksheet-db-fields.json');
export const STATE_PATH = path.join(__dirname, '..', 'data', 'external-it-checksheet-app-ids.json');
...
export async function resolveDefaultThreadId(baseUrl, headers, spaceId) {
  const override = process.env.KINTONE_SPACE_DEFAULT_THREAD || process.env.EXTERNAL_IT_CHECKSHEET_THREAD_ID;
...
export function loadFieldProperties() {
  ...
  if (!raw.properties) throw new Error('external-it-checksheet-db-fields.json: missing properties');
