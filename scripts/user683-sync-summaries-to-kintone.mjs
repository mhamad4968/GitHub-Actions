...
const CLAUDE_PROMPT_WEEK =
  '以下は社内ユーザサポートの週次対応メモの抜粋です。当社の会計年度は**4月末が期末・5月が期首**（暦月のみの比較だけで終わらせず、必要ならその週が会計年度のどの位置かに触れてよい。コーパスに無い「21日締め」等の変形会計月は捏造しない）。週内に**年末年始・ゴールデンウィーク・秋分の日前後の連休（いわゆるシルバーウィーク）**など稼働日が少ない期間が含まれる場合は、件数・負荷の変動をその文脈で読むこと。コーパス行頭の**[土日祝対応・…]**は土・日・祝日に実際に対応した内容（本来は不要だが行った対応）である。平日業務と同列に書かず、該当があれば要約の1点で区別して触れよ。件数0の土日祝は扱わない。日本語で、箇条書き3点以内・合計200字以内で要約してください。コーパスに無い数値は書かないこと。**対応件数やメモが1日・少数件でも、コーパスに書かれた内容は必ず1行以上で要約すること。空欄で返さないこと。**余計な前置きは不要。\n\n';
