/**
 * App 674: `scripts/data/pc-ledger-674-add-ux-section-groups-properties.json` の GROUP 3 件を
 * プレビュー `POST /k/v1/preview/app/form/fields.json` でマージし、preview deploy まで実行。
 *
 *   npx dotenv -e .env -e .env.proxy -- node scripts/pc-ledger-674-add-ux-section-groups-preview.mjs -- --dry-run
 *   npm run pc-ledger:674:add-ux-section-groups-preview
 */
...
const FRAGMENT_PATH = path.join(
  ...
  'data',
  'pc-ledger-674-add-ux-section-groups-properties.json',
);
...
async function main() {
  ...
  if (cur.properties?.pc_ledger_g_identity) {
    console.log('674: UX セクション用 GROUP（pc_ledger_g_*）は既に存在。POST スキップ。');
    return;
  }
  ...
  await waitDeploy();
  console.log(`674: UX セクション GROUP 3 件追加 + preview deploy SUCCESS revision=${rev}`);
}
