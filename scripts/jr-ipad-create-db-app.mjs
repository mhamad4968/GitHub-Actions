#!/usr/bin/env node
/**
 * JR iPad — DB app create (Space 34 / thread 38)
 * 正本: docs/plans/2026-06-15-jr-ipad-ledger-kintone-spec.md §4
 */
import {
  DB_APP_NAME,
  SPACE_ID,
...
async function main() {
  ...
  if (dryRun) {
    console.log(JSON.stringify({ name: DB_APP_NAME, space: SPACE_ID, thread: THREAD_ID, fieldCount: Object.keys(properties).length }, null, 2));
    return;
  }
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DB_APP_NAME,
    'JRシステム用 iPad 端末の正本。日常の書込は「JRシステム用iPad管理台帳 ver.1」から REST のみ。標準 UI は閲覧のみ。',
  );
