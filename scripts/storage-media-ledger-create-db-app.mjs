#!/usr/bin/env node
/**
 * Storage media ledger — DB app create (Space 21 / thread 23)
 * 正本: docs/plans/2026-06-13-storage-media-ledger-kintone-spec.md §4.1 / §7
 */
import {
  DB_APP_NAME,
  SPACE_ID,
...
async function main() {
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DB_APP_NAME,
    '記憶媒体・周辺機器の正本。日常の書込は「記憶媒体等管理台帳ver.1」から REST のみ。標準 UI は閲覧のみ。',
  );
