#!/usr/bin/env node
/**
 * Software ledger — Dash app create (Space 21 / thread 23)
 * 正本: docs/plans/2026-06-13-software-ledger-kintone-spec.md §4.1 / §9
 */
import {
  DASH_APP_NAME,
  SPACE_ID,
...
async function main() {
  ...
  if (!dbAppId) {
    console.error('dbAppId missing. Run software-ledger:create-db first.');
    process.exit(1);
  }
  ...
  const settingsRev = await setAppSettings(
    ...
    appId,
    DASH_APP_NAME,
    'ソフトウェア管理台帳 v1 — 新規・修正・廃止・削除・リスト出力の唯一の入り口。',
  );
