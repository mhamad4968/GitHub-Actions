  ...
  modelLoop: for (const modelId of geminiModelCandidates()) {
    const model = genAI.getGenerativeModel({
      ...
      systemInstruction: [
        ...
        "次の 4 見出しを**この順・この表記**で各行の先頭に付ける（全角括弧・コロンなし）:",
        "【事象】",
        "【脆弱性関連】",
        "【修正・対策】",
        "【見解】",
        "**digest の整形:** 各ブロックのあいだに**空行を 1 行**入れる（前の見出しの本文の直後で改行し、空行を挟んでから次の `【脆弱性関連】` 等を開始）。JSON の digest 文字列内に `\\n\\n` を含めてよい。",
        "各見出しの直後は空にしない。**最低 1 文・おおよそ 15 文字以上**。抜粋に無い事項は「記事・公式で要確認」「抜粋に明示なし」と明示する。",
