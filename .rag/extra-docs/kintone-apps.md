...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23** / customize **BUILD=`2026-05-05-pc-ledger-fixed-ip-nas-doc-note`**（閲覧=detail **PC買替・印刷のみ**／共有・JR **最小表示**＋所属モーダル（**680**）／**固定IPはサーバーNASのみ・任意**／個人＋保管＝個人同様・VPN個人のみ）/ fileKey **`17d911a5-ed04-40fb-a875-daa90b6a89fc`** / preview revision **`92`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
