...
- **本番で実行中の JS**（kintone アプリ設定に表示されるソース）の **`BUILD`** は **`2026-05-04-678-manual-app-guide-name`**（fileKey **`e901ad5f-d7fb-4016-97f1-84d6f871f723`** / revision **`109`**）。リポの **`customize/678/desktop.js`** の `var BUILD` と一致することを 2026-05-05 に確認済み。
- **上表の 678 行のうち**、`**続（2026-05-05）**` から **「続（会社）… `BUILD=`2026-05-07-678-partner-select-nfkc`」**までの deploy 連記は **リポ上の先行ログ**であり、**当該時点の本番 customize には未反映**（本番 BUILD は上記のまま）。
- **整合手順**: それらを本番へ載せるときは **`npm run deploy:678`** を実施し、成功後に **fileKey / revision / BUILD** を本表へ追記し、**本番と一致しない先行分は削除するか「本番反映済」に置換**する。
