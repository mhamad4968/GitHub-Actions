...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23** / customize **BUILD=`2026-05-05-pc-ledger-index-skysea-chips`**（**一覧**: キーワード＋種別に加え **SKYSEA 状態チップ**（`skysea_status` 複数 in）§4.8a）／**PC名＝管理番号・全種別保存必須**／**個人: JBIS 重複は赤バナー＋室長確認モーダル**／閲覧=detail **PC買替・印刷のみ**／共有・JR **最小表示**＋所属モーダル（**680**）／**671 行番号は新規・編集で手入力不可**／固定IPはNASのみ任意／備考任意／モバイル想定なし）/ fileKey **`7d1b1263-d94d-4d1e-8c92-3f456664925c`** / preview revision **`98`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
