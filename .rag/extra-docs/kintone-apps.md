...
| **681（運用終了・旧 PC台帳簡単ガイドライン）** | **681** | `customize/681/desktop.js`（**リポに参照用で残置**・**deploy 対象外**） | **2026-05-06 CEO 方針**: **kintone アプリは削除**（スペース管理から **アプリ削除**を実施。台帳上は **運用対象外**）。担当者向け案内は **別手段を後日設計**。`scripts/*quick-guide*`・`npm run pc-ledger:quick-guide:*` は **当面存置**（再利用前提なし）。旧 deploy 連記は **git 履歴**および本行に集約。 |
