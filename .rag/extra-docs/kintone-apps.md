...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23** / customize **BUILD=`2026-05-05-pc-ledger-dept-master-app-680`**（閲覧=detail **PC買替・印刷のみ**／共有・JR **最小フィールド表示**＋**所属候補モーダル**（マスタ **680**）＋保存前必須・API 失敗時は埋め込み候補／個人＋保管は個人と同表示・VPN は個人のみ・NAS/その他は全表示）/ fileKey **`d2ecf7dc-8f7b-4666-b626-73d04b75d5ae`** / preview revision **`90`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
