...
| 新・PC台帳ver.1（本体・674 customize **本実装進行中**・**運用開始前**） | **674** | `customize/new-pc-ledger-v1/desktop.js` | `npm run deploy:674` / Space **21** / thread **23**（スペースでアクセス権）/ customize **BUILD=`2026-05-05-pc-ledger-field-focus-assist`**（**入力支援**: 未入力で **利用者名／所属名／所属グループ** にフォーカス→個人（非保管）は **595 検索**、共有・JR は **680 所属モーダル**）／**所属ヘルプ** `<details>` 既定閉じ §4.2.0b／**一覧** §4.8a＋URL `query` 復元／SKYSEA 計画後再検討／**PC名**必須・個人 **JBIS** 室長確認／閲覧=detail PC買替・印刷のみ／共有・JR 最小表示＋**680**／**671** 手入力不可／備考任意）/ fileKey **`59de672f-5015-4268-94ca-d5f1cc598c15`** / preview revision **`101`** / field-spec **44/44** / **運用開始予定 2026-05-13**（仕様書 §1） |
