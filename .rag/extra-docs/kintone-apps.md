...
- **本番 live**（**2026-05-07T12:14:21Z** GitHub Actions `kintone-customize-deploy` SUCCESS 時点・直前の **`npm run deploy:678` rev=125** から自動再デプロイで **rev=126** に更新）: fileKey **`20260507121419E8BC8237900E4FF883E1A9D0E0E9EA3F041`** / preview revision **`126`** / **`var BUILD`** = **`2026-05-07-678-partner-presets-canonical-confirm`**（リポ **`customize/678/desktop.js` HEAD** と一致）。**変更内容**: `PARTNER_DROPDOWN_PRESETS` を **正典 16 社**（大塚商会・FBJ・KDDI・その他・あさかわシステムズ・KCS・NTTコミュニケーションズ・NTTファイナンス・NTT・TCリース・NTT東日本・ソフトバンク・三菱HCビジネスリース・蔵衛門・オフィスバスター・クロネコヤマト・佐川急便）に確定。「会社を新規登録する」ボタンに **B3 確認ダイアログ**（株式会社・㈱は付けない／全角カタカナ・漢字・半角アルファ混在可）を追加。`submitPayment` 内で **NFKC＋㈱・株式会社・（株）削除＋空白圧縮**を保存時自動適用。**実データ**: REST `records.json` PUT 1 回で **26 件・8 種**を一括正規化（`KDDI㈱`→`KDDI` ×10／`㈱大塚商会`→`大塚商会` ×8／`KCS㈱`→`KCS` ×2／`あさかわｼｽﾃﾑｽﾞ㈱`→`あさかわシステムズ` ×2／他 ×4）。**「主候補＋未確定」3 件**（`クロネコヤマト、佐川急便` ほか）は datalist 載せず維持。`SPEC.md` §6g・`yojitsu-master-and-field-plan.md` §3 を追随更新。
