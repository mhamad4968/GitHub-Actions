...
| 756 | `2026-09-12-ver02-workdesc-wider` | **367** | `3303929f-0d1c-4672-992b-4d5ed8fad221` | Ver.02 App1 JS shell／ACL everyone 書込（2026-09-13 CB_NO02 是正） |
...
| 757 | `2026-07-21-ver02-phase6-app2-readonly-guard` | **33** | `ca97ae0c-856f-496f-95fb-08276fe63f6a` | 2026-09-05 内訳4フィールド ADD（name_detail/name_item/line_vendor_name/line_person_name） |
...
| 749 | `2026-08-29-749-ux-toolbar-copy-pill-print` | **18** | `53f7ec7c-ddc1-442c-a8c7-95b34be384b9` | 2026-06-28 NAS 台帳 v1 初回 deploy |
...
| 683 | `2026-09-02-683-wiring-print-box` | **117** | `df4cd13b-b963-4916-8ba4-52864918219f` | 2026-05-16 registry 整合 |
...
>
> **継続棚卸**: `npm run audit:kintone-app-inventory`（読み取り専用）を月次 `cio:periodic:monthly` の先頭で実行する。対象は **AIチームと作成・管理したアプリのみ**で、appId範囲の単一正本は `data/kintone-ai-team-app-registry.json`（現役／削除済みを明示。**2026-09-05**: 社員名簿 **776** を active に登録）。一般部門・利用者作成アプリは対象外。このregistryは棚卸範囲の正本であり、PC台帳の業務データ正本が674だけであることとは区別する。基準更新は `npm run audit:kintone-app-inventory:write`。現役消失／削除済み再出現は NG、AIチーム管理証跡あり・一覧未掲載／追加／削除／名称変更は要確認（自動削除なし）。latest は `docs/reports/kintone-app-inventory-latest.md`。**削除済アプリへの稼働コード参照**は `npm run verify:retired-app-refs`（月次＋**pre-push**＋**constitution-gates**・596 買替事故の再発防止）。**一覧 ID が registry 未登録**は `npm run verify:kintone-ai-team-registry-parity`（**pre-push**＋**constitution-gates**・オフライン。LIVE API なし・776 漏れの再発防止）。**RULE-2 / ORG-2（2026-08-11）**: テナント削除・統廃合と**同ターン**で `retiredAppIds` 登録し、稼働コードに退役 appId を残さない。
...
| **NAS管理台帳**（日常 UI・748 へ REST） | **749** | `customize/nas-ledger-dash/desktop.js` \| `npm run deploy:749` | [https://jbis-kintone.cybozu.com/k/749/](https://jbis-kintone.cybozu.com/k/749/) **Space 48 / thread 52**・**v1 完成 — CLOSED**／**UX 改善レーンクローズ 2026-08-29**（ツールバー枠・件数チップ・sticky・IP/ホスト/管理者ID/PW コピー・状態ピル・行色・モーダル保持・印刷機密1行）・一覧印刷（PW 除く）・xlsx（PW 含む）・浜田目視 OK **2026-08-29**・`closures JSON 不触`・SPEC `docs/plans/2026-06-28-nas-ledger-kintone-spec.md` §7.5・**BUILD=`2026-08-29-749-ux-toolbar-copy-pill-print` rev **18** / fileKey **`53f7ec7c-ddc1-442c-a8c7-95b34be384b9`** |
...
**状態（2026-08-23）**: **v1 + P0/P1 + P2-1/VUX（2026-08-23 目視OK）** — CLOSED（`docs/plans/2026-08-23-jr-ipad-721-p2-vux-spec.md`）。P2-2 / C〜I / 694・674 連携は見送り。
