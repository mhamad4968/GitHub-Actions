...
| 役割 | モデル | 制約 |
|---|---|---|
| **CIO（本体・本チャット）** | **Claude Opus 4.7 固定**（ピッカー上書き） | 判断・統合・GO 前の最終統合 |
...
**正本**: `chat-sessions/session-starter-parts/part-A-constitution-kernel.md`（🎖️ 表）・`.cursor/rules/deepseek-cursor-spec-division.mdc`・`.cursor/rules/cursor-generate-image-assets.mdc`（画像 MCP は見送り・内蔵 GenerateImage のみ）。
...
**開発時の絶対3ステップ**（`customize/**`・仕様書・`deploy:*` に触れる **前**）:
