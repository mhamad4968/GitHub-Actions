...
- **硬拒否フレーズ（2026-07-31 浜田承認・止め役固定）**:  
  `4h超・新チャットのみ。deploy/実装はしない`  
  （機械: `cio-deploy-preflight-guard` が SESSION-CLOCK 4h 超で deploy 拒否。緊急のみ `SKIP_CIO_SESSION_CLOCK_DEPLOY=1`）
...
- **日終わり反省（2026-07-31）**: handoff 末尾に **改善候補 1〜3 件を1段落**（運用・ルール・性能）。実装は翌日可。
