...
8. **§1-2-3-1 ティア（再掲・モデル可視化）**: **§1-1 と同一内容**なら **`項目8: §1-1 と同一`** と **1 行**でよい。**Cursor でモデル切替**した本ターンだけは、**更新後のティア**を **1 行で再掲**（`AGENTS.md` §1-2-3-1・§1-2-3-2）。Tier B なのに L1/L2 だけ → L3 要否を自己点検。
8b. **🎖️ 割当（再掲）**: **§1-3 と同一内容**なら **`項目8b: §1-3 と同一`** と **1 行**でよい。**割当を変えた**ときだけ **§1-3 に足りない差分をここに 1 行**追記。**新チャット第 1 応答**の詳細運用は `NEW-SESSION-STARTER.md`・**read-pack `08-READ-06.txt` §10**。
