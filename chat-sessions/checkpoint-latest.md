...
## 凍結（毎セッション先頭・**2026-06-08 まで**）

| 禁止 | 許可 |
|------|------|
| 業務改善 **kintone customize/deploy** の着手（前倒し） | **2026-06-08** **案B1**（Space 57 skeleton + 設定マスタ Excel）— AI チームと確認の上 |
| Q36 GO 済みでも **〜6/7** の実装レーン | 仕様・runbook・683 要約微調整・v5 ドキュレーン |

---

## 2026-06-06 JST — **セッション締め**
...
| 項目 | 内容 |
|------|------|
| **本日完了** | 予実 **41601→¥70,000**（677/678）／683 月次 **max_tokens 1024**・5月再生成／683 **週6**（`user683_week_6` フィールド追加＋要約）／v5 目次 **71項**（Ｃ－５ 生成AI）／v5 **第８章 Ａ－３** 担当者まとめ本文復元 — **浜田目視 OK** |
| **凍結** | 業務改善 **customize/deploy** — **6/8 まで着手しない** |
| **次回 1 手** | **6/8** 案B1（Space 57 skeleton + 設定マスタ Excel）— AI チームと確認の上 |
| **683 運用** | 毎月 sync 後の **文言微調整**は浜田依頼時（夕反省 **P4** 案は承認待ち） |
| **doc-lane** | `verify_v5_ch3_c5` に **Ａ－３ 空欄検知**・`patch_v5_a3_staff_summary.py`・`npm run doc-lane:patch-v5-a3` |
| **Git** | 6/1 締め + **Ａ－３/doc-lane** 追記 commit 済（本ターン） |
...
| 項目 | 内容 |
|------|------|
| **本日追加** | v5 目次 **70 項**（Ｃ－２端末管理・Ｃ－３棚卸）・**LITE mirror**（24/25 `.txt` 要約）・Notepad Hang 調査 |
| **浜田メモ帳** | **`24-handoff-log-LITE.txt` / `25-checkpoint-latest-LITE.txt` のみ** — `.md` 全文は開かない |
| **午前済** | Q36 GO・案A1・壁時計 manual-desktop |
| **実装** | **6/8** まで **着手しない** |
| **Git** | `84d80be`（LITE）+ 本締め commit → **push 実施予定** |
| **次回 1 手** | **6/8** 案B1（AI チームと確認の上） |
| **v5 目次** | 浜田 **目視 OK**（Ｃ－２／Ｃ－３ 問題なし） |
| **承認待ち** | ~~C1–C4 / B4 / S3 / D3 / E1~~ → **2026-06-01 全 GO 実施済** |
| **締め** | `SESSION-CLOSE-REPORT-20260531.txt`（§4 夜追記） |

---

## 2026-05-31 JST — **セッション締め（午前）**
...
| 項目 | 内容 |
|------|------|
| **Q-GUIDE-08** | **確定** — ガイド執筆時の **下書きヒント**＝制度導入資料（目的・導入理由・旧83/84利用方法） |
| **正本配置** | `C:\tmp\業務改善\導入資料保管` — **2026-05-31 保管済み**（追加資料は **6/8 まで** 同フォルダへ可） |
...
## 2026-05-30 JST — **終盤締め（595 本番・PC ログ修復・承認案）**

| 項目 | 内容 |
|------|------|
| **595 退職→674** | **rev 84** — 検収 OK — **クローズ**（追加作業なし） |
| **v5** | **69項**・verify OK — **クローズ** |
| **壁時計** | spawn 競合修復・`session:clock:ensure` |
| **PC masa** | Event **3503**（Killer）・**BTHUSB 5**（HCI）— 修復後 **新規 0** |
| **未 commit** | **本ターン B1 で解消予定** |
| **夕反省改善** | A1–A3 / B1–B3 / R1–R3 / S1–S2 / D1 — **全 GO・実装済** |
| **触らない** | v5・595→674（**クローズ**）・業務改善実装（Q36 GO 前） |
| **前日に決めない** | 業務改善・第1手・レーン — **当日 -0 で聞く** |

---

## 2026-05-30 JST — **セッション締め（v5 目次最終・壁時計 hooks 復旧）**

| 項目 | 内容 |
|------|------|
| **v5 目次** | **65項**・第１章 **p4**・第８章＋**Ａ－１〜Ａ－３** — `verify_toc_completeness_v5.py` **合格** |
| **v5 正本** | `C:\tmp\マニュアル\【改訂版】新情報管理運用マニュアルv5.docx`（backup `…155902`） |
| **ヘルス** | `npm run health-check` **100%**（本セッション前半） |
| **壁時計** | `session:clock:stop` 実施 / **`cursor:hooks:install-user-windows`** で `~/.cursor/hooks.json` 復旧 |
| **次回 1 手** | 打合せ v5 持参 / **§41 案A1** |
| **触らない** | kintone customize/deploy（Q36 GO 前） |
| **未 push** | `check-mcp-dormancy`・debug-tips・hooks install 等（ローカルのみ） |
...
| 項目 | 内容 |
|------|------|
| **実施** | 4行単一窓口化（`mode-b-mdc-canonical-linter.mdc`）・金曜 MCP 定例 runbook・Opus4.7固定ゾンビ表記パージ・`every-turn` 一本化 |
| **verify** | 全 checker **exit 0**（`verify:cio-four-ai-governance` 含む） |
| **履歴** | **憲法・ルールの大整理、および自律自動化ルールの追加環境充実化完了** |
| **触らない** | kintone customize / deploy（**Q36 GO 前・実装レーン凍結**） |
| **次回 1 手** | **§41 案A1**（Q36 報告 1 問） |
...
| 項目 | 内容 |
|------|------|
| **仕様** | **Q-GUIDE-01〜07**・**Q-IMPL-01〜05**・**Q-DATA-01**・**Q-ANN-09**・**Q-DEMO-01**・**Q-SCHED-02** 確定。**仕様確定日: 2026-05-30（浜田）** |
| **チェック** | Pass4 **14/14**・`business-improvement:compare-83` **20/20** |
| **保留** | ガイド dropdown 名・FAQ 本文（6/9〜6/20 可）・**Q36 GO**（6/8 着手前） |
| **次回 1 手** | **明日 §41** — **案A1**（Q36・1問）→ **案B1**（実装準備・1問）。**案Cはクローズ** |
| **承認（5/28 夜）** | 案C **クローズ** / 案A・B **明日各1問** / 反省改善 **CIO自律可**（customize禁止維持） |
| **スケジュール** | **6/8** 実装着手 → **7/1** デモ → **8/1** 本番（スペース 5） |
| **触らない** | kintone customize / deploy（**Q35–Q36 GO 前**） |
| **夕反省** | `docs/reports/2026-05-28-evening-reflection.md` |
| **Desktop** | `19-SESSION-ONE-REPORT-2026-05-28.md`・`26-evening-reflection-2026-05-28.md` |

---

## 2026-05-27 JST — **予実 678 承認用差異・683 印刷・677 修正（セッション締め）**

| 項目 | 内容 |
|------|------|
| **678** | 予算増減差異タブ — 承認用・工種×摘要初期・差異のみ OFF・年額＝予算修正のみ（rev **187**） |
| **683** | 印刷 1 枚目文字拡大・グラフ可読・日次ラベル 7pt（rev **83**）— **浜田 OK** |
| **677** | ﾒｰﾙｻｰﾊﾞｰ利用料 6月 **73900**（id 66） |
| **次回 1 手** | **業務改善 Q-GUIDE-01（G1）** — 浜田「明日からメイン」 |
| **待ち** | 678 担当明日レビュー・追加依頼 |
| **Git** | 678/683 等 **commit 済**（H3 GO）— push は本ターン |
| **夕反省** | `docs/reports/2026-05-27-evening-reflection.md` |
| **承認** | D6・18 追記 **GO** / G 明日相談 / H 依頼都度 / H3 **GO** |

---

## 2026-05-26 JST — **予実 678（5A）セッション締め**

| 項目 | 内容 |
|------|------|
| **本日** | 支払同期（行 id）・月次孤児実績（TSB-036 + reconcile）・備考→`payment_memo` のみ・工種コード集計 3 列（円・%） |
| **最終 BUILD** | `2026-05-26-678-pivot-copy-yen-pct-format` |
| **浜田** | 予実 **一旦 OK**。集計フォーマット **OK** |
| **担当（明日）** | 備考・工種コード集計の実データ確認 |
| **次回 1 手** | **G 保留**（レーンは明日決定）。**E・F 保留**（担当から依頼なし）。**H 承認**（運用継続） |
| **レーン注意** | 5/25 checkpoint の **683・業務改善 GO** は未消化 — **明日 G を検討** |
| **案の承認（5/26 夜）** | **H=GO** / **E・F=保留**（担当依頼なし） / **G=明日判断** |

---

## 2026-05-25 JST — **次回セッション（浜田 GO: A〜D 全件）**

| 順 | 内容 |
|----|------|
| **1** | **683 B1/B2** — page1 印刷文字拡大 → deploy → A4 実機（4AIゲート必須） |
| **2** | **業務改善 C1** — §41 **Q-GUIDE-01**（ご利用ガイド文字サイズ） |
| **3以降** | C2/C3・A2〜A5・D1/D2（夕反省参照） |
| **GO** | 業務改善 customize は Q35–Q36 まで未着手 |

**運用**: 毎ターン §1四行（A3）・仕様GO即正本（A2）

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**§7.2 観点5 完了** — Q-VIS-03）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-VIS-03** — Q58バッジ **緑/橙/灰** 確定。観点5＝Q60＋Q-VIS-01/02/03 **完了** |
| **観点1〜5** | **✅ 完了** |
| **次回 1 手** | 上記 **次回セッション第1手** 表を参照 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-VIS-02 確定** — §7.2 観点5）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-VIS-02** — 新⑤年次＝**薄い緑グラデ**（申請青／評価茶／年次緑）。表は白カード |
| **観点1〜4** | **✅ 完了** |
| **観点5** | **一部 ✅**（Q60・Q-VIS-01/02） |
| **次回 1 手** | **§7.2 観点5 続き** — **Q58バッジ色**（緑/橙/灰）の最終確認 §41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-VIS-01 確定** — §7.2 観点5）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-VIS-01** — 文字サイズ **標準16px／大18px** を **画面トグル**で切替。**localStorage** 記憶（拠点共有IDは端末ごと） |
| **観点1〜4** | **✅ 完了** |
| **観点5** | **一部 ✅**（Q60＋Q-VIS-01） |
| **次回 1 手** | **§7.2 観点5 続き** — **新⑤年次画面**の背景テーマ §41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**§7.2 観点4 完了** — Q-ANN-08）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-ANN-08** — 明細 **行クリック**→新①詳細。観点4＝**表1/表2/明細**プレビュー＋新①遷移＋手動メール＋部署並び **一式完了** |
| **観点1〜4** | **✅ 完了** |
| **次回 1 手** | **§7.2 観点5** — **ビジュアル**（申請=薄青／評価=茶系・文字サイズ等）§41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-ANN-07 確定** — §7.2 観点4）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-ANN-07** — 表2の **A/B/C件数**クリック→新①（社員+表彰ランク最終）。**目視確認用** |
| **観点1〜3** | **✅ 完了**（Q-UX-08〜11） |
| **観点4** | **一部 ✅**（Q-ANN-03〜07） |
| **次回 1 手** | **§7.2 観点4 続き** — **明細プレビュー**（行クリックで新①へ）§41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-ANN-06 確定** — §7.2 観点4）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-ANN-06** — 表1プレビュー＝**1段目月・2段目業務改善｜アイデア**（Q-ANN-03通り）。**件数クリック**→新①該当レコード一覧→詳細 |
| **観点1〜3** | **✅ 完了**（Q-UX-08〜11） |
| **観点4** | **一部 ✅**（Q-ANN-03〜06） |
| **次回 1 手** | **§7.2 観点4 続き** — **表2プレビュー**（A/B/C件数もクリック遷移するか）§41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-ANN-04/05 確定** — §7.2 観点4）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-ANN-04** — プレビュー後、**浜田が手動でメール**（xlsx/PDF ダウンロード→Outlook）。送付前 **xlsx 加工可**。**Q-ANN-05** — 表2の所属並び＝本社9部・支店営業所20拠点（固定順・`scripts/data/business-improvement-annual-department-order.json`） |
| **観点1〜3** | **✅ 完了**（Q-UX-08〜11） |
| **観点4** | **一部 ✅**（Q-ANN-03〜05） |
| **次回 1 手** | **§7.2 観点4 続き** — **表1プレビュー**（2段ヘッダの画面レイアウト）§41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-ANN-04 確定** — §7.2 観点4）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-ANN-04** — 浜田＝新⑤**画面プレビュー**で突合。人事へ**メール**で **xlsx または PDF** |
| **観点1〜3** | **✅ 完了**（Q-UX-08〜11） |
| **観点4** | **一部 ✅** |
| **次回 1 手** | **§7.2 観点4 続き** — 表1プレビュー表示・メール送信手段 §41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**§7.2 観点3 完了** — Q-UX-08〜11）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-08〜11**（経過・履歴／差戻しアラート／再申請／確認モーダル／一覧ハイブリッド） |
| **観点1〜3** | **✅ 完了** |
| **次回 1 手** | **§7.2 観点4** — **帳票・年次**（Q-ANN-03）§41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-08〜10 確定** — §7.2 観点3）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-08** 経過・履歴＋差戻しアラート／**Q-UX-09** 再申請ボタン／**Q-UX-10** 初回・再申請とも確認モーダル（はい/キャンセル） |
| **観点1〜2** | **✅** |
| **観点3** | **ほぼ ✅**（Q-UX-08〜10）。**一覧表示** 等は未確定 |
| **次回 1 手** | **§7.2 観点3 続き** または **観点4（帳票・年次）** — **一覧のステータス** §41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-08/09 確定** — §7.2 観点3）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-08** 経過・履歴＋差戻しアラート／**Q-UX-09** 下部「再申請する」（Q58・Q-UX-02 同一判定） |
| **観点1〜2** | **✅** |
| **観点3** | **一部 ✅**（Q-UX-08/09）。**再申請確認** 等は未確定 |
| **次回 1 手** | **§7.2 観点3 続き** — **再申請確認モーダル** §41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-08 確定** — §7.2 観点3）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-08** — 下部「経過・履歴」（A）。**申請者修正待ち**＝画面上部に **差戻し理由＋再申請促し** |
| **観点1** | **✅**（Q-UX-01/02） |
| **観点2** | **✅**（Q-UX-03〜07） |
| **観点3** | **一部 ✅**（Q-UX-08）。**再申請操作 UI** 等は未確定 |
| **次回 1 手** | **§7.2 観点3 続き** — **再申請ボタン・操作の見せ方** §41 1問 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**§7.2 観点2 完了** — Q-UX-07）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-07** — 完結評価者＝評価後 **ダイアログ等**で **表彰ランク（最終）確定を必ず促す**。未確定は承認不可 |
| **観点1** | **✅ 完了**（Q-UX-01/02） |
| **観点2** | **✅ 完了**（Q-UX-03〜07） |
| **整備** | RAG ミラー同期・浜田一覧 §3 追記・§7.2 観点1 ✅・**git commit 済** |
| **次回 1 手** | **§7.2 観点3** — **差戻し→再申請→履歴の見え方** から **§41 1問** |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-06 追記 — 支店長判断**）

| 項目 | 内容 |
|------|------|
| **確定** | **`支店長判断` ON**（**部長評価のみ**）→ **評価項目未完了でも** WF **支店長承認中**へ。それ以外（完結者の最終ランク必須等）は Q-UX-06 確定のまま |
| **次** | **§7.2 観点3** — 申請～評価の流れ |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-06 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | 承認表示＝**評価項目すべて選択済**。**表彰ランク（最終）**＝完結評価者のみ（C/B/A） |
| **次** | **§7.2 観点3** — 申請～評価の流れ |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-05 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-05** — 評価完了後 **承認** 表示→「承認しますか？」**はい/差戻し/キャンセル** |
| **次** | **Q-UX-06** — 「評価完了」の判定条件（§41 1問） |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-04 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-04（B）** — 差戻し理由＝**確認モーダル** |
| **次** | **§7.2 観点2** 続き — 承認ボタン確認 等 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-03 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-03（B）** — 評価画面 ≥1280px=2ペイン／未満=縦積み |
| **次** | **§7.2 観点2** 続き — 差戻し理由UI 等 |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-02 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-02（A）** — 「次の項目へ」+ blur・入力済のみ次展開＋スクロール |
| **次** | **§7.2 観点1** 続き or **観点2**（評価者 UI） |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UX-01 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-UX-01（C）** — 上段3完了→目的展開・入力完了で次ブロック連鎖 |
| **次** | **§7.2 観点1** 続き（自動スクロール等） |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-HIST-03 / UX-01 確定**）

| 項目 | 内容 |
|------|------|
| **確定** | **Q-HIST-03（A）** — 差戻し後もスナップショット **すべて残す**・再評価は行追加 |
| **次** | **§7.2 観点1** — 申請者 UI 摺合せ |
| **残 ⚠** | UX-02（年次）・UX-03（人事部長配置） |

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**§7.1 Pass 3 完了**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — §7.1 厳格チェック（浜田 GO） |
| **結果** | **14/14 ＋ 拡張8/8 ✅**／compare-83 **20/20**／**⚠ 3件**（UX-01〜03） |
| **正本** | `2026-05-23-business-improvement-proposal-spec-checklist.md` Pass 3 |
| **触らない** | customize/**・deploy（**Q36 実装 GO 前**） |

### 次の 1 手

| 順 | 作業 |
|----|------|
| **1** | **§7.2 観点1** — 申請者 UI 摺合せ（§41 1問ずつ） |
| **2** | **UX-02** — 差戻し・年次（観点4で） |

**注**: 本日 GO＝§7.1 厳格チェック。**kintone 実装 GO** は §7.2 完了後

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-PHASE-UX 確定**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` §7.1/§7.2 |
| **本日確定** | **Q-PHASE-UX** — §7.1 後・GO 前に **UX/UI/帳票/デザイン摺合せ**（1件ずつ・時間余裕あり）。Q-UI-06 / Q-WF-05 / Q-HIST-02 も同日確定済 |
...
| 順 | 作業 |
|----|------|
| **1** | **§7.1** — AIチーム仕様反映チェックシート |
| **2** | **§7.2 観点1** — 申請者 UI から摺合せ開始（§41 1問ずつ） |
| **3** | 全観点完了後 **Q36 浜田 GO** |

**実装 GO 前**: §7.1 → §7.2 UX摺合せ → DeepSeek §50-3-8 → 浜田 GO

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-HIST-02 確定**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` §4.9/§4.10 |
| **本日確定** | **Q-HIST-02** — 評価スナップショット・操作履歴とも **新①レコード内サブテーブル**（別アプリなし）。Q-UI-06 / Q-WF-05 も同日確定済 |
| **触らない** | スペース57・customize/**・deploy |

### 次の 1 手

| 順 | 作業 |
|----|------|
| **1** | **Q35–Q36 GO 仰ぎ** — §7.1 AIチーム中身確認 → 浜田 GO |
| **2** | 評価 code 命名・新⑤物理設計（実装 GO 後 or GO と並行） |

**実装 GO 前**: DeepSeek §50-3-8 ＋ `cio:guard:5038` ＋ 浜田 GO

---

## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-WF-05 確定**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` §4.9/§4.10 |
| **本日確定** | **Q-WF-05** — 差戻し＝申請やり直し・部長/支店長/人事が差戻し・理由必須・再申請→部長評価から再開（Q-UI-06 も同日確定済） |
| **触らない** | スペース57・customize/**・deploy |
...
## 2026-05-25 JST — 業務改善 ver.02 仕様（**Q-UI-06 確定**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` §4.9/§4.10 |
| **本日確定** | **Q-UI-06** — 上段＝代表提案者・一覧＝全員・1行目↔上段連動・2行目以降手動/検索・年次付与正本＝一覧 |
| **触らない** | スペース57・customize/**・deploy |
...
## 2026-05-24 JST — 業務改善 ver.02 仕様（**セッション締め・§2/§3/年次 浜田確認済**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` §4.8/§4.9/§4.10 |
| **浜田確認用** | `2026-05-24-business-improvement-proposal-01-fields-hamada-review.md` — **§2・§3・年次 ✅** |
| **本日確定** | Q-UI-01〜05, Q-WF-04, Q-HIST-01, Q-AUDIT-01, **Q-ANN-03**（年次全文）, 新⑤ |
| **OPEN-17** | **✅**（実装設計: 年度マスタ物理・履歴保存先のみ残） |
| **触らない** | スペース57・customize/**・deploy |
...
| 順 | 作業 | 備考 |
|----|------|------|
| **0** | checkpoint + `19-SESSION-ONE-REPORT-2026-05-24.md` Read | |
| **1** | **Q-UI-06** — 上段「社員名」vs 提案者サブテーブル | **高** |
| **2** | Q-WF-05 差戻し再申請 / Q-HIST-02 履歴保存先 | |
| **3** | **Q35–Q36 GO 仰ぎ** | 仕様 §4.10 参照 |
...
**git/deploy（2026-05-24 夜）**: **main 完全同期** — `6b34409` customize push → GHA **success** → `6cba124` 674監査マーカー修正＋checkpoint（**全件完了・未コミットなし**）

**実装 GO 前（業務改善）**: DeepSeek §50-3-8 ＋ `cio:guard:5038` ＋ 浜田 GO

---

## 2026-05-24 JST — 業務改善 ver.02 仕様（**§4.8 途中・Q-ANN-01/02 確定**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` |
| **チェックリスト** | `docs/plans/2026-05-23-business-improvement-proposal-spec-checklist.md`（§7.1 **14/14 ✅×2**） |
| **本セッション確定** | **Q-FLD-01〜03**／**Q-ANN-01**（ランク+付与 A5000/B1000/C100）／**Q-ANN-02**（按分なし・全員同額） |
| **§4.3.1** | 【評価ランクについて】正本反映。アイデア B＝**7～9点**（離散評点では8点） |
| **触らない** | スペース57・customize/**・deploy |

### 次セッション — **AI が先頭で実施する順（浜田合意 2026-05-24）**

| 順 | 作業 | 備考 |
|----|------|------|
| **0** | 本 checkpoint ＋ `handoff-log.md` 末尾を Read | 忘れ防止 |
| **1** | ~~軽微反映 Q-ANN-02~~ | **✅ 2026-05-24 締め時**（浜田確認用一覧） |
| **2** | **フィールド表 OK** — 浜田確認（§2〜4） | **次回先頭** |
| **3** | **Q-ANN-03** — 4月年次処理の **操作手順・検算・件数突合** | **最重要** |

### Q-ANN-03 設計方針（浜田指示 — 次回必須）

- 旧運用より **シンプル**で **集計ミスに気づける**仕組み
- 過去問題: **提案数と合わない** 等 → 社員付与後のトラブル防止
- 合わせて詰める: **Q-ANN-04** 年度マスタ／**Q-ANN-05/06** 年次締め状態（旧取込ステータス・年度処理 GROUP）
...
| 1 | 付与ポイント表 | ✅ Q-ANN-01 |
| 2 | 複数提案者 | ✅ Q-ANN-02 |
| **3** | **4月処理 UI・手順・検算** | **次回本題** |
| 4 | 年度マスタ・締めフラグ | 未 |
| 5–6 | 旧年度処理／取込ステータス代替 | 未 |

**実装 GO 前（変更なし）**: フィールド表 OK ＋ AI「問題なし」＋ 浜田 GO ＋ DeepSeek §50-3-8 ＋ `cio:guard:5038`

---

## 2026-05-23 JST — 業務改善提案 ver.02 仕様（**セッション3 終了 → Q70 承認**）

| 項目 | 内容 |
|------|------|
| **本題** | **業務改善提案システム ver.02** — 仕様策定（**実装 GO 前**） |
| **正本** | `docs/plans/2026-05-23-business-improvement-proposal-spec.md` |
| **本日確定** | **Q45–Q69**（UI Q58/Q67、評価 Q63–Q65、84突合 Q68、code Q69） |
| **Q70 承認** | 83突合5フェーズ **GO**／AI主導可／ギャップ=新仕様正・**§4.3.1固定**／自動ランク+最終決定+部長→支店長 **維持**／実装話題禁止／**△→◎命令** |
| **成果物** | `scripts/data/app-84-fields-snapshot.json`・評価基準 Excel 読込 |
| **§7.1** | 1回目 14/14 ✅ — **2回目は 83突合後** |

**次の 1 手（明日）**: **旧83×§4.3.1 フェーズ3** — 突合表 **20/20 ✅済**・**Q57 確認候補4件**（メタ3+余剰4）。`docs/reports/2026-05-24-app83-spec431-crosswalk.md`

**準備済（2026-05-23 夜 GO）**: `npm run business-improvement:prep-83` — snapshot/compare/RAG 一括

**触らない**: スペース57・customize/**・deploy・**kintone実装の話題**（仕様完全確定まで）

**規律（CEO命令）**: §1四行毎ターン・83前DeepSeek・締めreport-verify・日終わりsync — **△禁止・◎必達**

---

| 項目 | 内容 |
|------|------|
| **絶対** | **明日のブリーフィング／最初の実装ターンから** D1〜D4 ゲートを **必ず実施**（失敗の繰り返し禁止） |
| **手順** | `npm run cio:pre-implement-gate` → DeepSeek §50-3-8 → 突合3行 → `cio:guard:5038 --stamp` |
| **4AI** | customize/**=Composer のみ・毎ターン§1四行・締め=report-verify exit 0 |
| **正本** | `docs/runbooks/cio-four-ai-violation-remediation.md`・`18-重要確認.txt` |
| **D5** | 674 §10.5 目視 — **未承認・実施しない** |
| **CIO** | AIチーム連携の責任者として指揮（①割振→②DeepSeek→③Composer→④Kimi→⑤要約） |

**次セッション先頭 1 手**: ブリーフィング応答の **§1 四行** の直後に `cio:pre-implement-gate` を実行してから本題へ。

---

## 2026-05-22 JST — セッション締め（浜田）

| 項目 | 内容 |
|------|------|
| **674** | 本番運用中・B-1/B-2 移行完了・先祖返り監査 **OK**（`npm run cio:audit:674`） |
| **文書** | 仕様 §1.6/§9/§10/§12・`kintone-apps.md`・§10.5（5/23〜 目視 1件/日） |
| **目視済** | 新規・自動生成・M365・印刷（§10.1） |

**次セッションの 1 手**: **5/23** §10.5 — **#11 PC買替**の画面確認（`docs/plans/2026-04-21-new-pc-ledger-spec.md` §10.5）

**残（急ぎでない）**: `pc-ledger:verify-labels-spec`・681 代替案内・594 整理

---

## 2026-05-22 JST — §10.5 目視スケジュール（5/23〜 1項目/日）

| 項目 | 内容 |
|------|------|
| **予定** | **5/23（金）から** §10.5 表どおり **1 日 1 項目**（浜田・明日以降で可） |
| **明日** | **#11 PC買替** |
| **正本** | `docs/plans/2026-04-21-new-pc-ledger-spec.md` **§10.5**・§9 表 **5/23〜6/7** 行 |

---

## 2026-05-22 JST — §10 画面確認（一部・浜田）

| 項目 | 内容 |
|------|------|
| **確認済** | 新規登録（個人・共有）・自動生成（個人・共有）・共有/JR の M365 登録・**印刷** — いずれも **問題なし** |
| **文書** | `docs/plans/2026-04-21-new-pc-ledger-spec.md` §10.1 を **[目視] OK** 反映 |

---

## 2026-05-22 JST — B-2 完了（共有・JR すべて登録済み）

| 項目 | 内容 |
|------|------|
| **B-2** | **完了** — 共有 + JR 端末は **すべて 674 に登録済み**（浜田確認） |
| **移行** | **B-1 + B-2** とも計画どおり完了扱い（§7.4.6） |

**残**: §10.1/§10.4 の **[目視] 未**・`pc-ledger:verify-labels-spec`・681 代替案内・594 整理

---

## 2026-05-22 JST — PC台帳 仕様・スケジュール実態突合

| 項目 | 内容 |
|------|------|
| **674 運用** | **本番運用中**（担当者 **5/11〜**・システム切替 **5/13**） |
| **live** | **BUILD** `2026-05-21-list-create-modal-clear-btn` / rev **224** / 61 フィールド |
| **文書** | `docs/plans/2026-04-21-new-pc-ledger-spec.md` §1.6・§9 実施状況・§10/§12 更新、`kintone-apps.md` 674 行 |
| **先祖返り** | `npm run cio:audit:674` **OK** |

**次の 1 手**: **§10 [目視]** の優先確認。定期は `cio:audit:674`。
...
**次の 1 手**: **A1**（DeepSeek→5038）。GitHub pending は Cursor/Mintlify 見直し。
...
| 項目 | 内容 |
|------|------|
| **承認** | 明日案 **全件 GO**（B1 は CIO 自律） |
| **施行** | A1/B3→18・13-READ-05・mode-b-canonical／B1→19履歴を archive／sync prune |
| **反省対策** | §50-3-8 着手前必須・Composer 大量Diff 禁止を条文化 |

**次の 1 手**: 次の実装ターンで **A1 チェックリストどおり DeepSeek→5038** を実行。

---

## 2026-05-21 JST — セッション締め（単一報告・Desktop 同期）

| 項目 | 内容 |
|------|------|
| **報告 1 本** | `19-SESSION-ONE-REPORT-2026-05-21.md`（成果・反省・明日案 §3） |
| **夕反省** | `docs/reports/2026-05-21-evening-reflection.md` → sync で **26-evening-reflection-2026-05-21.md** |
| **19 履歴** | `19-SESSION-ONE-REPORT-2026-05-19.md` **維持**（5/19 セッション控え） |
| **Desktop** | sync + verify 済み・旧番号 prune 済み |

**次の 1 手**: 浜田が §3 案（A1/A2/A3/B2 等）の承認可否を判断。
...
| レーン | 状態 |
|--------|------|
| **688/687 稼働日数** | v4-syntax-fix deploy・Space56・ACL 完了。**CEO: 後日まで保留** |
| **685/686 ICT** | v2.1 push `567ed2c`・**明日〜運用**・カスタマイズ依頼待ち |
| **GHA ICT collect** | 次回 10:00/20:00 JST から v2.1 適用 |
...
- **#8 RSS**: **A 確定** — **27 本**（JPCERT/IPA alert 除外 + `scan.netsecurity.ne.jp/rss/index.rdf`）。`npm run ict-digest:rss:verify` で件数・代表4本を検証。
- **#9 カテゴリ**: **A** — 7種。**685 ドロップダウン差替え済**（2026-05-17）。686 は旧→新マッピング、既存685は旧値維持。
...
| 論点 | 結果 |
|------|------|
| **GHA `ICT_RSS_FEED_URLS`** | 未設定＝`config.ts` DEFAULT **27 本** |
| **685 カテゴリ7種** | `update-685-category-dropdown.mjs` 適用済 |
| **本番収集** | dry-run → dispatch SUCCESS → `ICT_DRY_RUN=false`・cron 継続 |
...
- **683 ブラウザ AI 生成**: **CEO #4 不要**（定時 `apply-prev-month` 正・`USER683_RELAY_ZERO_MODE=true` 継続）。
- **683 未完了リスト #1〜#6**: **2026-05-17 全クローズ**（#5 PDF serve 廃止済・#6 632 別レーン）。当月途中再生成・ブラウザ中継は **都度相談**。
