...
**Git**: **`8d2e4455`** = `origin/main` — push 済（本ターン SPEC commit 後に heal）
...
**継続メモ**:
1. **SKYSEA**: 手動インストール方針。SPEC 確認後に 674 実装。実配信・GPO・SG追加禁止。
2. **674**: 採番 max+1 済。SKYSEA台帳は SPEC 確認後のみ customize。
3. **719/721**: closed-v1 微小 UI／印刷は明示依頼で再開可（O-CLOSED-01）。
**GO待ち**: SKYSEA 674 実装＝**SPEC AIチーム確認後**（完了済を出さない）。新アプリ＝相談・GO後のみ。
...
| **688** | WBGT 以外触らない |
| **677–679** | 触らない |
| **SKYSEA** | **active（2026-08-06）** — **手動インストール＋674台帳**（SPEC確認待ち）。**実PC配信・GPO・SG追加はしない** |
