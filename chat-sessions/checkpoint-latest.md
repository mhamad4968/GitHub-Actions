...
**最終更新**: 2026-07-25 14:45 JST — 内訳列見出しUI反映（分類／品目／規格＝補助）。756 LIVE deploy 済。

**Git**: **dcc26a3b** = origin/main — 756 labels deploy R63

**本日状態（要約）**: App **756** LIVE BUILD=`2026-07-25-ver02-bunrui-hinmoku-kikaku-aux` rev**104**。内訳見出し **分類（選択）／品目（選択）／規格（補助）**。規格は薄い赤・必須対象外。確認パック3問は7/27送付想定。735/736書込禁止。

**継続メモ（次セッション -0）**:
1. ~~列見出しUI反映・756 deploy~~ → **済**
2. 浜田: 確認パック「送ってよい／直す」（送付は7/27以降）
3. Excel案件追加は依頼者データ待ち / H9 評価記録

**GO待ち**: 依頼者送付は **7/27以降**。リストJSON追記は回答 or 浜田GO後。

**次の1手**: 756画面で分類／品目／規格（補助）タグを目視。確認パック送付判断。735/736 WRITE 禁止。

**調査正本**: requester-confirm-pack-pre-0727 ＋ SPEC U4/U17 ＋ unit-and-kindlong-diff。
