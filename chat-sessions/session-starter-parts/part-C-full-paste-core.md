...
【今やってる主タスク（2026-08-30 自動更新）】
- 次の1手: 浜田の実装合図 → G0 **§15＋§16** の **S0**（既存値スキャン）。個人資産月次は 9/13–17 必須
- checkpoint: 2026-08-30 17:15 JST — **夜セッション**。§15＋**§16**（listOnly祖父・S0〜S5）を仕様確定。コードなし。**実装合図待ち**。
- 進行中 plan: docs/plans/2026-08-30-jikkou-yosan-v2-s0-existing-value-scan.md
- 当日コミット (上位 3):
  - chore(checkpoint): sync Git line after commit
  - fix(jikkou-yosan): keep ditto type from leaking into other himoku menus.
  - fix(jikkou-yosan): hide vendor clear option when value is set.