...
**方針（v3.36）**: 浜田は **ハブ**（Desktop の **`00-NEW-SESSION-STARTER_yyyymmdd.txt`**）を **新チャット 1 通目**に貼れば **項番 -1 の入口**は足りる（短く読める）。**旧「単一ファイル全文」と同値の細則**は **Part A〜F**（`session-starter-parts/*.md`／Desktop `00p01`〜`00p06`）にあり、**AI が順に Read** して復元する（下記手順 2）。任意で **Part C「■ フル版」ブロック**を追加貼付してもよい。`12-HANDOFF-HUMAN.txt` 5 行は **任意**。
...
**浜田が毎回最初に開く場所（運用上いちばん大事）**: `C:\Users\mhamada202408224\Desktop\AI緊急用\` の **`00-NEW-SESSION-STARTER_yyyymmdd.txt`**（**JST 8 桁・ハブ正本**。旧名 `NEW-SESSION-STARTER_*.txt` は同期で掃除）。**貼付推奨**は **`npm run verify:desktop-ai-emergency-sync` の最終行**（または `session-starter:sync-desktop` の「貼付推奨」行）。フォルダ説明は **`13-README.txt`**。**セッション切替時はハブを 1 通目に貼る**（細則は AI が Part A〜F で Read）。貼ったあと **本日の一手**を **§41 一問**で確認し、**浜田 OK 後**に項番 0（**`npm run session:bootstrap`**）。**CIO 義務**: スターター系をリポで触った **同一ターン**で **`npm run session-starter:sync-desktop`**（§35-1・TSB-024）。**日終わり**にも sync → verify。

**補足（2026-04-28 / v3.36 で追補）**: Desktop に集約 txt のみある場合も、**先に `session-starter:sync-desktop`** で **ハブ＋00p01〜00p06＋儀式 4 本**を復元してから **ハブ貼付 → 項番 -0 → bootstrap**。Git 正本: **ハブ** `chat-sessions/NEW-SESSION-STARTER.md` と **Part** `chat-sessions/session-starter-parts/part-*.md`。`/mnt/c` 不可時は CIO がチャットに 1 行（復帰後に npm 再実行）。
