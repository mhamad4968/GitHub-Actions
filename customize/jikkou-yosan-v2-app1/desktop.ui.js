  ...
  // @JY_V2_BUILD 2026-07-31-ver02-actual-excel-phase2c-c-excel-detail-pm
  ...
  function jy2InstallStyle(documentRef) {
    ...
    style.textContent = [
      ...
      ".jy2-actual-child-ops{display:inline-flex;gap:2px;flex-shrink:0;align-items:center;order:-1}",
      ...
      ".jy2-actual-table .jy2-freeze-3{left:15.2rem;min-width:7.5rem;max-width:11rem;overflow:visible;box-shadow:2px 0 5px rgba(15,23,42,.1)}",
  ...
  function jy2ActualChildRow(
    ...
    if (childCanEditBudget && childDetailModel) {
      ...
      ops.setAttribute("aria-label", "詳細行の追加・削除");
  ...
  // Phase2c-c-excel-flat-detail: Excel「原価管理明細」は種別の下に詳細行が
  // あり手入力する。旧 hide（name3 あり非表示）は入力欄ごと消えていたため廃止。
  // reveal は＋詳細行直後のフォーカス維持用に残す。shouldShow は常に true。
  ...
  function jy2RenderActualPane(
    ...
    if (canEditBudget) {
      ...
      detailAddNotice.textContent =
        "原価管理は Excel 同様に費目→種別→詳細。詳細列の横の＋／－で行の追加・削除、詳細・単価は手入力、一時保存で App757 へ。「予実を保存」では構造は保存されません";
          ...
          body.appendChild(
            jy2ActualTypeGroupRow(
              ...
              months,
              {
                ...
                detailQuickAdd: typeChildren.length === 0,
              },
            ),
          );
