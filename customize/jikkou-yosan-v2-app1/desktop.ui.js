  ...
  // @JY_V2_BUILD 2026-08-01-ver02-actual-excel-nameless-typeless
  ...
  function jy2CostMgmtEnsureTypeOnlyFrames(detailModel) {
    ...
    for (const frame of JY2_COST_MGMT_ENSURE_TYPE_ONLY_FRAMES) {
      ...
      try {
        ...
        const after = detailModel.snapshot().blocks.find(
          (block) => block && block.stableBlockId === id,
        );
        const firstRow =
          after && Array.isArray(after.detailRows) ? after.detailRows[0] : null;
        if (
          firstRow &&
          firstRow.rowKey &&
          typeof detailModel.updateDetailRow === "function"
        ) {
          detailModel.updateDetailRow(id, firstRow.rowKey, {
