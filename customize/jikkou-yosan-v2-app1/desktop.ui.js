  ...
  // @JY_V2_BUILD 2026-08-02-ver02-actual-sticky-totals-bar
  ...
  function jy2InstallStyle(documentRef) {
    ...
    style.textContent = [
      ...
      ".jy2-actual-totals-bar{position:sticky;top:0;z-index:45;display:flex;flex-wrap:wrap;align-items:baseline;gap:10px 22px;margin:8px 0 10px;padding:10px 12px;background:#fffbeb;border:1px solid #f59e0b;border-radius:8px;box-shadow:0 2px 8px rgba(15,23,42,.08);box-sizing:border-box}",
      ...
      ".jy2-actual-totals-bar .jy2-actual-totals-note{flex:1 1 100%;margin:0;font-size:11px;color:#78716c}",
  ...
  // Phase2c-actual-sticky-totals-bar: 表直上の仮置き全合計バー（施工計+保安計と同趣旨）。
  function jy2ActualCostMgmtStickyTotalsBar(documentRef, opts) {
    ...
    appendItem(
      "実行予算額（全合計）",
      anyFinalBudget ? jy2AmountDisplay(finalBudgetTotal) : "－",
    );

    let monthlyAmountTotal = "0";
    let anyMonthlyAmount = false;
    for (const category of ACTUAL_COST_CATEGORY_KEYS) {
      const monthly = totals && totals[category] && totals[category].monthly;
      if (!monthly) continue;
      for (const month of months) {
        const addend = jy2ActualDecimalAddend(monthly[month]);
        if (addend !== null) {
          monthlyAmountTotal = add(monthlyAmountTotal, addend);
          anyMonthlyAmount = true;
        }
      }
    }
    appendItem(
      "月次金額（全合計）",
      anyMonthlyAmount ? jy2AmountDisplay(monthlyAmountTotal) : "－",
    );
    ...
    for (const row of rows) {
      ...
      const kids = jy2ActualChildrenForBudgetSum(row.children, shouldShowDetail);
    ...
    appendItem(
      "月次数量（全合計）",
      anyQty ? jy2ActualMonthQtySumDisplay(qtyTotal) : "－",
    );
