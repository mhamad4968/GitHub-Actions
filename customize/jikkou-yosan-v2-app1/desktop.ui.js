  ...
  function jy2InstallStyle(documentRef) {
    ...
    style.textContent = [
      ...
      ".jy2-actual-table .jy2-actual-child-row td.jy2-actual-child-name{padding-left:22px}",
  ...
  function jy2ActualChildRow(
    ...
    const name3Resolved = jy2IsDitto(name3Raw)
      ...
      : jy2HasText(name3Raw)
        ...
        : "";
    // Excel: 費目枠→種別枠の下では詳細列に品名のみ。ホバーにフルパス。
    ...
    // Phase2c-c: 種別は種別枠ヘッダに出すため、明細の種別列は "－"（単位は title）。
    const typeCell = jy2Cell(documentRef, "td", "", "－");
    if (child.unit) {
      typeCell.title = `種別: ${name2Resolved || "－"}／単位: ${child.unit}`;
    } else if (name2Resolved) {
      typeCell.title = `種別: ${name2Resolved}`;
    }
  ...
  function jy2ActualTypeGroupRow(
    ...
    for (const month of months) {
      ...
      qtyCell.title = "種別単位の数量は表示しない（明細の月次数量を参照）";
      tr.appendChild(qtyCell);
      const monthSum = jy2ActualSumMonth(childrenInGroup, month);
      const monthCell = jy2Cell(
        documentRef,
      ...
      monthCell.title = "種別合計（表示専用・入力不可）";
      tr.appendChild(monthCell);
    }
  ...
  function jy2RenderActualPane(
    ...
    for (const row of rows) {
      ...
      if (row.hasChildren && expandState.isExpanded(row.stableBlockId)) {
        ...
        // Phase2c-a/c: 費目(name1)→種別(name2)の視覚入れ子。並びは既存順の
        // まま。書き込み経路は変更しない。
        ...
        const peekGroupChildren = (startIndex, matchFn) => {
          const group = [];
          for (let j = startIndex; j < row.children.length; j += 1) {
            const peekChild = row.children[j];
            const peekIndex = detailRows.findIndex(
              (candidate) =>
                candidate && candidate.rowKey === peekChild.rowKey,
            );
            if (!matchFn(peekChild, peekIndex)) break;
            group.push(peekChild);
          }
          return group;
        };
        let previousHimokuLabel = null;
        let previousTypeLabel = null;
        row.children.forEach((child, index) => {
          ...
          if (index === 0 || himokuLabel !== previousHimokuLabel) {
            const himokuChildren = peekGroupChildren(
              index,
              (peekChild, peekIndex) =>
                resolveHimokuLabel(peekChild, peekIndex) === himokuLabel,
            );
            ...
            previousTypeLabel = null;
          }
          if (
            previousTypeLabel === null ||
            typeLabel !== previousTypeLabel
          ) {
            const typeChildren = peekGroupChildren(
              index,
              (peekChild, peekIndex) =>
                resolveHimokuLabel(peekChild, peekIndex) === himokuLabel &&
                resolveTypeLabel(peekChild, peekIndex) === typeLabel,
            );
            const lastChildRowKeyInGroup =
