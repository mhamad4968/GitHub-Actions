  ...
  // @JY_V2_BUILD 2026-08-01-ver02-actual-hide-blank-worktype
  // Phase2c-hide-blank-worktype: システム工種が空/「－」の親行は原価管理に出さない。
  // 親の欠落コード表示も「－」ではなく空欄。ダッシュ種別・（未分類）費目も抑止。
  ...
  function jy2CostMgmtHimokuHasVisibleChild(typeMap, shouldShowDetail) {
    if (!(typeMap instanceof Map)) return false;
    for (const entries of typeMap.values()) {
      for (const entry of entries || []) {
        const key = entry && entry.child && entry.child.rowKey;
        if (!key) continue;
        if (typeof shouldShowDetail !== "function") return true;
        if (shouldShowDetail(key)) return true;
      }
    }
    return false;
  }
  function jy2CostMgmtShouldOmitHimoku(
    himokuLabel,
    templateHimoku,
    typeMap,
    shouldShowDetail,
  ) {
    if (jy2CostMgmtIsNoiseHimoku(himokuLabel)) {
      // （未分類）等: 手入力で reveal した子があるときだけ残す
      return !jy2CostMgmtHimokuHasVisibleChild(typeMap, shouldShowDetail);
    }
    const allowed = Array.isArray(templateHimoku) ? templateHimoku : [];
    if (allowed.length > 0 && !allowed.includes(himokuLabel)) {
      return !jy2CostMgmtHimokuHasVisibleChild(typeMap, shouldShowDetail);
    }
    return false;
  }
