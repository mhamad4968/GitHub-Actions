  ...
  function jy2RenderActualPane(
    ...
    for (const row of rows) {
      ...
      if (row.hasChildren && expandState.isExpanded(row.stableBlockId)) {
        ...
        // Phase2c-c-template-types: コード表 typesByHimoku の種別はデータが
        // 無くても空の種別枠として出す（Excel 原価管理明細と同じ）。
          ...
          for (const typeLabel of typeOrder) {
            ...
            body.appendChild(
              jy2ActualTypeGroupRow(
                ...
                typeChildren,
                months,
                { ...groupOpts, lastChildRowKeyInGroup: lastTypeKey },
              ),
            );
