(function () {
  ...
  function aggChipBarHtml(values, allActive) {
    return values
      .map(function (v) {
        return (
          '<button type="button" class="jca-agg-chip' +
          (allActive ? " active" : "") +
          '" data-value="' +
          esc(v) +
          '">' +
  ...
  function selectedAggChipValues(containerEl) {
    ...
    return Array.prototype.slice
      .call(containerEl.querySelectorAll(".jca-agg-chip.active"))
      .map(function (btn) {
  ...
  function setAggChipsAll(containerEl, on) {
    if (!containerEl) return;
    containerEl.querySelectorAll(".jca-agg-chip").forEach(function (btn) {
  ...
  function open595SearchModal(onPick) {
    ...
    bg.innerHTML =
      ...
      '<button type="button" id="jca-595-clear" class="kintoneplugin-button-normal">クリア</button>' +
    ...
    bg.querySelector("#jca-595-clear").onclick = function () {
      var qEl = document.getElementById("jca-595-q");
      var box = document.getElementById("jca-595-results");
      if (qEl) qEl.value = "";
  ...
  function readAggSelections() {
    var fromEl = document.getElementById("jca-agg-from");
    var toEl = document.getElementById("jca-agg-to");
    var orgBox = document.getElementById("jca-agg-org");
    var deptBox = document.getElementById("jca-agg-dept");
    return {
  ...
  function injectCss() {
    ...
    st.textContent =
      ...
      ".jca-agg-chip.active{background:#059669;color:#fff;border-color:#059669;font-weight:600;}" +
      ".jca-agg-chip:hover:not(.active){background:#f1f5f9;}" +
      ".jca-agg-chip-link{font-size:12px;color:#2563eb;background:none;border:none;cursor:pointer;padding:5px 0 0;white-space:nowrap;text-decoration:underline;}" +
      ...
      ".jca-filter-label{font-size:14px;font-weight:600;color:#475569;}" +
  ...
  function buildShell() {
    ...
    root.innerHTML =
      ...
      "</div>" +
      '<button type="button" id="jca-agg-all-dept" class="jca-agg-chip-link">すべて</button>' +
      "</div></div></div></details>" +
    ...
    document.getElementById("jca-agg-clear").addEventListener("click", function () {
      clearAggConditions();
    });
